...
fn main() -> Result<()> {
    ...
    // The architectures these archives actually carry SASS for, handed to the
    // crate as a constant so `CudaDevice` can refuse a card that has no image
    // rather than let every launch fail silently. Generated from
    // `KERNEL_ARCHES`, the same list the `-gencode` flags come from, so the
    // check can never describe a build that was not performed.
    let arches = KERNEL_ARCHES
        .iter()
        .map(|sm| sm.to_string())
        .collect::<Vec<_>>()
        .join(", ");
    let out_dir = PathBuf::from(std::env::var("OUT_DIR").context("OUT_DIR unset")?);
    fs::write(
        out_dir.join("built_arches.rs"),
        format!(
            "/// GPU architectures (`sm_NN`) this build carries native SASS for.\n\
             ///\n\
             /// Generated by `build.rs` from `KERNEL_ARCHES`. There is no PTX, so a\n\
             /// device outside this set has nothing to run and nothing to JIT from.\n\
             pub const BUILT_ARCHES: &[u32] = &[{arches}];\n"
        ),
    )
    .context("Failed to write built_arches.rs")?;
