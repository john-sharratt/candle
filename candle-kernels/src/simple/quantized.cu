...
static __device__ __forceinline__ int ggml_cuda_dp4a(const int a, const int b, int c) {
    // Native __dp4a intrinsic (available on SM61+, always present on SM80+)
    return __dp4a(a, b, c);
}
...
// Q8_1 MMQ config - same structure as Q8_0 since both are 8-bit quantization
#define  MMQ_X_Q8_1_RDNA2  64
#define  MMQ_Y_Q8_1_RDNA2  128
#define NWARPS_Q8_1_RDNA2  8
#define  MMQ_X_Q8_1_RDNA1  64
#define  MMQ_Y_Q8_1_RDNA1  64
#define NWARPS_Q8_1_RDNA1  8
#if defined(CUDA_USE_TENSOR_CORES)
#define  MMQ_X_Q8_1_AMPERE 4
#define  MMQ_Y_Q8_1_AMPERE 32
#define NWARPS_Q8_1_AMPERE 4
#else
#define  MMQ_X_Q8_1_AMPERE 128
#define  MMQ_Y_Q8_1_AMPERE 64
#define NWARPS_Q8_1_AMPERE 4
#endif
...
// Q8_K MMQ config - similar to Q6_K (both K-quant types with 256 elements per block)
#define  MMQ_X_Q8_K_RDNA2  64
#define  MMQ_Y_Q8_K_RDNA2  128
#define NWARPS_Q8_K_RDNA2  8
#define  MMQ_X_Q8_K_RDNA1  32
#define  MMQ_Y_Q8_K_RDNA1  64
#define NWARPS_Q8_K_RDNA1  8
#if defined(CUDA_USE_TENSOR_CORES)
#define  MMQ_X_Q8_K_AMPERE 4
#define  MMQ_Y_Q8_K_AMPERE 32
#define NWARPS_Q8_K_AMPERE 4
#else
#define  MMQ_X_Q8_K_AMPERE 64
#define  MMQ_Y_Q8_K_AMPERE 64
#define NWARPS_Q8_K_AMPERE 4
#endif
...
// Q8_K: 8-bit K-quant (256 elements per block, full 8-bit precision)
#define QR8_K 1
#define QI8_K (QK_K / (4*QR8_K))  // 64 ints per block
typedef struct {
...
// VDR constants for Q8_K
// VDR=2 means each thread processes 2 int32s (8 elements)
// With qi=64 and 32 threads, each thread handles 64/32=2 int32s
#define VDR_Q8_K_Q8_1_MMVQ 2
...
static __device__ __forceinline__ void dequantize_q8_1(const void * vx, const int ib, const int iqs, dfloat2 & v){
    const block_q8_1 * x = (const block_q8_1 *) vx;

    const dfloat d = __low2half(x[ib].ds);  // delta is in .x
...
// Q8_1 dequantization - similar to Q8_0 but uses ds.x for delta
template<typename dst_t>
static __device__ void dequantize_block_q8_1(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    const int i = blockIdx.x;
    ...
    const block_q8_1 * x = (const block_q8_1 *)vx + ib;
    const float d = __low2float(x->ds);  // ds.x = delta
...
// =============================================================================
// KS (attention-sink sub-block) block types and dequantization kernels
// Block types must match blocks.cuh exactly.
// =============================================================================

#define QK_Q4_KS 32
typedef struct { half d; uint8_t sa; uint8_t sb; uint8_t qs[16]; } block_q4_ks;

#define QK_Q8_KS 32
typedef struct { half d; uint8_t sa; uint8_t sb; int8_t  qs[32]; } block_q8_ks;

// Q4_KS dequantization
// Layout: qs[k] = low_nibble(k) | (high_nibble(k+16) << 4), biased +8
// Sub-block A: elements 0-3 (attention sinks), sub-block B: elements 4-31
// actual_d_A = d * sa / 255.0,  actual_d_B = d * sb / 255.0
template<typename dst_t>
static __device__ void dequantize_block_q4_ks(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    const int i   = blockIdx.x;
    const int tid = threadIdx.x;
    const int il  = tid / 8;    // sub-group index [0,3] — 4 groups of 8 threads, each outputting 4 elements
    const int ir  = tid % 8;    // within-group index [0,7] — selects which of 8 blocks in this blockIdx
    ...
    const block_q4_ks * x = (const block_q4_ks *)vx + ib;
    const float cd = __half2float(x->d);
    const float da = cd * (x->sa / 255.0f);
    const float db = cd * (x->sb / 255.0f);

    const uint8_t * q = x->qs + 4 * il;

    for (int l = 0; l < 4; ++l) {
        // Low nibble: elements 0-15. Scale by da if il==0 (elems 0-3 are sub-block A).
        const float d_lo = (il == 0) ? da : db;
        y[l]      = (dst_t)(d_lo * (float)((int)(q[l] & 0xF) - 8));
        // High nibble: elements 16-31. Always sub-block B scale.
        y[l + 16] = (dst_t)(db   * (float)((int)(q[l] >> 4)  - 8));
    }
}

// Q8_KS dequantization
// Sub-block A: elements 0-3 (il==0, l<4), sub-block B: remainder
template<typename dst_t>
static __device__ void dequantize_block_q8_ks(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    const int i   = blockIdx.x;
    const int tid = threadIdx.x;
    const int il  = tid / 8;
    const int ir  = tid % 8;
    ...
    const block_q8_ks * x = (const block_q8_ks *)vx + ib;
    const float cd = __half2float(x->d);
    const float da = cd * (x->sa / 255.0f);
    const float db = cd * (x->sb / 255.0f);
    ...
    for (int l = 0; l < 8; ++l) {
        const float d = (il == 0 && l < 4) ? da : db;
        y[l] = (dst_t)(d * (float)q[l]);
    }
}

// Q8_K dequantization - K-quant 8-bit (256 elements per block)
template<typename dst_t>
static __device__ void dequantize_block_q8_K(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    // assume 64 threads - 4 elements per thread, 256 elements total
    const int64_t tid = threadIdx.x;
    const int64_t il = tid * 4;  // each thread handles 4 consecutive elements

    dst_t * y = yy + i * QK_K + il;

    const float d = x[i].d;
    const int8_t * q = x[i].qs + il;
...
// =============================================================================
// Q2_0 and Q3_0: simple 2-bit and 3-bit symmetric quantization formats
// Block types must match blocks.cuh exactly.
// =============================================================================

// Q2_0: 2-bit symmetric, scale only (10 bytes per 32 elements)
#define QK2_0 32
typedef struct { half d; uint8_t qs[8]; } block_q2_0;

// Q3_0: 3-bit symmetric, scale only (14 bytes per 32 elements)
#define QK3_0 32
typedef struct { half d; uint8_t qh[4]; uint8_t qs[8]; } block_q3_0;

// Q2_0 dequantization
// Layout: qs[j] = q0|(q1<<2)|(q2<<4)|(q3<<6), levels 0..3
// Value: d * (q - 1.5)  where d = amax/1.5
template<typename dst_t>
static __device__ void dequantize_block_q2_0(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    const int i   = blockIdx.x;
    const int tid = threadIdx.x;
    const int il  = tid / 8;    // 0..3: which 8-element group within the 32-element block
    const int ir  = tid % 8;    // 0..7: which of 8 blocks in this CTA
    ...
    const block_q2_0 * x = (const block_q2_0 *)vx + ib;
    const float d  = __half2float(x->d);
    const float dm = -1.5f * d;  // decode: d*(q - 1.5) = d*q + dm

    // il=0: qs[0..1] => elements 0..7
    // il=1: qs[2..3] => elements 8..15
    // il=2: qs[4..5] => elements 16..23
    // il=3: qs[6..7] => elements 24..31
    const uint8_t * q = x->qs + 2 * il;
    for (int b = 0; b < 2; ++b) {
        uint8_t byte = q[b];
        for (int bit = 0; bit < 4; ++bit) {
            y[4 * b + bit] = (dst_t)(__fmaf_rn(d, (float)((byte >> (2 * bit)) & 3), dm));
        }
    }
}

// Q3_0 dequantization
// Layout: qs[j] = lo0|(lo1<<2)|(lo2<<4)|(lo3<<6), qh[j] = high bits
// Value: d * ((hi<<2|lo) - 3.5)  where d = amax/3.5
template<typename dst_t>
static __device__ void dequantize_block_q3_0(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    const int i   = blockIdx.x;
    const int tid = threadIdx.x;
    const int il  = tid / 8;    // 0..3: which 8-element group within the 32-element block
    const int ir  = tid % 8;    // 0..7: which of 8 blocks in this CTA
    ...
    const block_q3_0 * x = (const block_q3_0 *)vx + ib;
    const float d  = __half2float(x->d);
    const float dm = -3.5f * d;  // decode: d*(q - 3.5) = d*q + dm

    const uint8_t * qs = x->qs + 2 * il;
    for (int b = 0; b < 2; ++b) {
        uint8_t byte_qs = qs[b];
        for (int bit = 0; bit < 4; ++bit) {
            int e    = 8 * il + 4 * b + bit;  // element index within block
            int lo   = (byte_qs >> (2 * bit)) & 3;
            int hi   = (x->qh[e >> 3] >> (e & 7)) & 1;
            float qf = (float)((hi << 2) | lo);
            y[4 * b + bit] = (dst_t)(__fmaf_rn(d, qf, dm));
        }
    }
}


// R16 dequantization: extract d[] (K values stored as F16) from block_r16
// block_r16 layout: half d[32] (K), uint16_t q[32] (Q) = 128 bytes total
// Only extracts the K (d[]) values; Q capture data is discarded.
#define QK_R16 32
typedef struct { half d[QK_R16]; uint16_t q[QK_R16]; } block_r16_simple;

template<typename dst_t>
static __device__ void dequantize_block_r16(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    const int i   = blockIdx.x;
    const int tid = threadIdx.x;
    const int il  = tid / 8;    // 0..3: which 8-element group within the 32-element block
    const int ir  = tid % 8;    // 0..7: which of 8 blocks in this CTA
    ...
    const block_r16_simple * x = (const block_r16_simple *)vx + ib;
    const half * d = x->d + 8 * il;

    for (int l = 0; l < 8; ++l) {
        y[l] = (dst_t)(__half2float(d[l]));
    }
}
...
// =============================================================================
// K/128 NATIVE TYPES - Dequantization kernels for AWQ quant formats
// =============================================================================
// These types use 128 elements per block (K/128 format) and are designed for
// the GEMX tensor-core matmul path. They can also be dequantized to f32/f16/bf16.
//
// Block types match canonical definitions in block_compact.cuh:
// - Q_AWQ:    80B (64B qs + scale/zero + padding) - asymmetric 4-bit g128
// - Q_AWQ_G64:80B (64B qs + 2×scale/zero) - asymmetric 4-bit g64
// =============================================================================

// Q_AWQ: 16 threads × 8 elements, 1 scale/zero pair per 128 elements
struct __align__(16) block_q_awq_k128 {
    int qs[16];       // 16 ints = 64 bytes
    half scale;       // scale for entire block
    half zero;        // zero point for entire block
    int _pad[3];      // 12 bytes padding to reach 80 bytes
};

// Q_AWQ_G64: 2 groups of 64 elements, each with own scale/zero
struct __align__(16) block_q_awq_g64_k128 {
    int qs[16];       // 16 ints = 64 bytes
    half scales[2];   // scale per 64-element group
    half zeros[2];    // zero per 64-element group
    int _pad[2];      // 8 bytes padding to reach 80 bytes
};

// Q_AWQ: Asymmetric 4-bit with scale and zero (g128)
// value = scale * (nibble - zero)
// Using int[16] layout
template<typename dst_t>
static __device__ void dequantize_block_q_awq(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    const block_q_awq_k128 * x = (const block_q_awq_k128 *) vx;
    const int i = blockIdx.x;
    const int tid = threadIdx.x;  // 0-31
    
    dst_t * y = yy + i * 128;
    const float scale = __half2float(x[i].scale);
    const float zero = __half2float(x[i].zero);
    
    // Each thread processes 4 elements (half an int = 4 nibbles)
    const int int_idx = tid / 2;
    const int q = x[i].qs[int_idx];
    const int half_select = tid & 1;
    
    const int shift = half_select * 16;
    const int nibble0 = (q >> (shift + 0)) & 0xF;
    const int nibble1 = (q >> (shift + 4)) & 0xF;
    const int nibble2 = (q >> (shift + 8)) & 0xF;
    const int nibble3 = (q >> (shift + 12)) & 0xF;
    
    const int out_base = tid * 4;
    y[out_base + 0] = (dst_t)(scale * (nibble0 - zero));
    y[out_base + 1] = (dst_t)(scale * (nibble1 - zero));
    y[out_base + 2] = (dst_t)(scale * (nibble2 - zero));
    y[out_base + 3] = (dst_t)(scale * (nibble3 - zero));
}

// Q_AWQ_G64: Asymmetric 4-bit with 2 groups of 64 elements each
// Using int[16] layout, threads 0-15 handle group 0, threads 16-31 handle group 1
template<typename dst_t>
static __device__ void dequantize_block_q_awq_g64(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    const block_q_awq_g64_k128 * x = (const block_q_awq_g64_k128 *) vx;
    const int i = blockIdx.x;
    const int tid = threadIdx.x;  // 0-31
    
    dst_t * y = yy + i * 128;
    
    // First 16 threads handle group 0 (elements 0-63), last 16 handle group 1 (64-127)
    const int group = tid / 16;
    const int local_tid = tid % 16;
    
    const float scale = __half2float(x[i].scales[group]);
    const float zero = __half2float(x[i].zeros[group]);
    
    // Each thread in the group processes 4 elements
    // Group 0: ints 0-7, Group 1: ints 8-15
    const int int_idx = group * 8 + local_tid / 2;
    const int q = x[i].qs[int_idx];
    const int half_select = local_tid & 1;
    ...
    const int out_base = group * 64 + local_tid * 4;
    y[out_base + 0] = (dst_t)(scale * (nibble0 - zero));
    y[out_base + 1] = (dst_t)(scale * (nibble1 - zero));
    y[out_base + 2] = (dst_t)(scale * (nibble2 - zero));
    y[out_base + 3] = (dst_t)(scale * (nibble3 - zero));
...
// Generate global kernel wrappers for K/128 AWQ types
DEQUANTIZE_K(q_awq)
DEQUANTIZE_K(q_awq_g64)
...
extern "C" __global__ void dequantize_mul_mat_vec_q8_1_cuda(const void * vx, const dfloat * y, float * dst, const int ncols, const int nrows) {
...
// Q8_K dequantize_mul_mat_vec kernel
// Q8_K: 256 int8 values per block with float scale and per-group sums
extern "C" __global__ void dequantize_mul_mat_vec_q8_k(const void * __restrict__ vx, const float * __restrict__ yy, float * __restrict__ dst, const int ncols, int nrows) {
    ...
    // Each block has 256 int8 values
    // With 32 threads, each thread handles 8 values per block
    for (int i = 0; i < num_blocks_per_row; ++i) {
        const float d = x[i].d;
        const float * y = yy + i * QK_K;
        
        // Each thread processes 8 consecutive values
        const int base = tid * 8;
        float sum = 0.0f;
        
        #pragma unroll
        for (int j = 0; j < 8; ++j) {
            sum += y[base + j] * (float)x[i].qs[base + j];
        }
...
static __device__ __forceinline__ float vec_dot_q8_1_q8_1(
    ...
    const block_q8_1 * bq8_1_v = (const block_q8_1 *) vbq;
...
#pragma unroll
    for (int i = 0; i < VDR_Q8_1_Q8_1_MMVQ; ++i) {
        v[i] = get_int_from_int8(bq8_1_v->qs, iqs + i);
        u[i] = get_int_from_int8_aligned(bq8_1->qs, iqs + i);
    }

    // Use Q8_0-style simple formula: d_v * d_u * sum(v[i] * u[i])
    // The sum field in Q8_1 is only used for affine quantization optimizations,
    // not for basic Q8_1 x Q8_1 matmul
...
// Q8_K x Q8_1 dot product for mul_mat_vec
// With VDR=2, iqs is 0, 2, 4, ..., 62 (each thread processes 2 consecutive int32s = 8 elements)
// Q8_K block has 256 elements = 64 int32s, Q8_1 has 32 elements = 8 int32s per block
static __device__ __forceinline__ float vec_dot_q8_K_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & iqs) {

    const block_q8_K * bq8_K = (const block_q8_K *) vbq;
    const float d = bq8_K->d;

    // iqs is 0, 2, 4, ..., 62 (int32 index, processing 2 at a time)
    // Each int32 contains 4 int8 values, so iqs*4 is the byte offset
    // Q8_1 blocks: block i covers elements [i*32, (i+1)*32)

    float sumf = 0.0f;

#pragma unroll
    for (int l = 0; l < VDR_Q8_K_Q8_1_MMVQ; ++l) {
        const int idx = iqs + l;  // int32 index (0-63)
        const int elem_idx = idx * 4;  // element index (0-252 in steps of 4)

        // Which Q8_1 block does this fall into? (each has 32 elements)
        const int q8_1_block = elem_idx / 32;  // 0-7
        const int q8_1_offset = elem_idx % 32; // 0-28 in steps of 4

        // Get the 4 Q8_K values as int32
        const int vq = get_int_from_int8(bq8_K->qs, idx);

        // Get the corresponding 4 Q8_1 values as int32
        const int uq = get_int_from_int8(bq8_1[q8_1_block].qs, q8_1_offset / 4);

        // Q8_1 scale for this block
        const float d8 = __low2float(bq8_1[q8_1_block].ds);

        // Dot product of 4 int8 pairs
        int sumi = ggml_cuda_dp4a(vq, uq, 0);
...
// Q8_1 kernels (batch sizes 1-8)
extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda1(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<1, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda2(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<2, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda3(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<3, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda4(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<4, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda5(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<5, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda6(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<6, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda7(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<7, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_1_q8_1_cuda8(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<8, QK8_1, QI8_1, block_q8_1, VDR_Q8_1_Q8_1_MMVQ, vec_dot_q8_1_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}
...
// Q8_K mul_mat_vec kernels (batch sizes 1-8)
extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda1(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<1, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda2(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<2, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda3(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<3, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda4(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<4, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda5(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<5, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda6(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<6, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda7(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<7, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}

extern "C" __global__ void mul_mat_vec_q8_K_q8_1_cuda8(
    const void * vx, const void * vy, float * dst,
    const int ncols_x, const int nrows_x, const int nrows_y, const int nrows_dst) {

    mul_mat_vec_q<8, QK_K, QI8_K, block_q8_K, VDR_Q8_K_Q8_1_MMVQ, vec_dot_q8_K_q8_1>
        (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
}
...
// =============================================================================
// Q8_1 MMQ tile functions - for Q8_1 x Q8_1 matrix multiplication
// =============================================================================
// Q8_1 has both delta (scale) and sum (for min) in half2 ds

template <int mmq_y> static __device__ __forceinline__ void allocate_tiles_q8_1(int ** x_ql, half2 ** x_dm, int ** x_qh, int ** x_sc) {
    GGML_UNUSED(x_qh); GGML_UNUSED(x_sc);

    __shared__ int   tile_x_qs[mmq_y * (WARP_SIZE)       + mmq_y];
    __shared__ half2 tile_x_ds[mmq_y * (WARP_SIZE/QI8_1) + mmq_y/QI8_1];

    *x_ql = tile_x_qs;
    *x_dm = tile_x_ds;
}

template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_q8_1(
    const void * __restrict__ vx, int * __restrict__ x_ql, half2 * __restrict__ x_dm, int * __restrict__ x_qh,
    int * __restrict__ x_sc, const int & i_offset, const int & i_max, const int & k, const int & blocks_per_row) {
    GGML_UNUSED(x_qh); GGML_UNUSED(x_sc);
    ...
    const int kbx  = k / QI8_1;
    const int kqsx = k % QI8_1;

    const block_q8_1 * bx0 = (const block_q8_1 *) vx;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
        ...
        const block_q8_1 * bxi = bx0 + i*blocks_per_row + kbx;

        x_ql[i * (WARP_SIZE + 1) + k] = get_int_from_int8(bxi->qs, kqsx);
    }

    const int blocks_per_tile_x_row = WARP_SIZE / QI8_1;
    const int kbxd = k % blocks_per_tile_x_row;

#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI8_1) {
        int i = i0 + i_offset * QI8_1 + k / blocks_per_tile_x_row;

        if (need_check) {
            i = min(i, i_max);
        }

        const block_q8_1 * bxi = bx0 + i*blocks_per_row + kbxd;

        x_dm[i * (WARP_SIZE/QI8_1) + i / QI8_1 + kbxd] = bxi->ds;
    }
}

static __device__ __forceinline__ float vec_dot_q8_1_q8_1_mul_mat(
    const int * __restrict__ x_ql, const half2 * __restrict__ x_dm, const int * __restrict__ x_qh, const int * __restrict__ x_sc,
    const int * __restrict__ y_qs, const half2 * __restrict__ y_ds, const int & i, const int & j, const int & k) {
    GGML_UNUSED(x_qh); GGML_UNUSED(x_sc);

    // Both x and y are Q8_1, so we use half2 for both delta+sum pairs
    return vec_dot_q8_1_q8_1_impl<VDR_Q8_1_Q8_1_MMQ>
        (&x_ql[i * (WARP_SIZE + 1) + k], &y_qs[j * WARP_SIZE + k],
         x_dm[i * (WARP_SIZE/QI8_1) + i/QI8_1 + k/QI8_1],
...
// Q8_K MMQ tile functions
// Q8_K is simpler than Q6_K: no bit packing, just 8-bit values with scale
template <int mmq_y> static __device__ __forceinline__ void allocate_tiles_q8_K(int ** x_ql, half2 ** x_dm, int ** x_qh, int ** x_sc) {
    GGML_UNUSED(x_qh);
    GGML_UNUSED(x_sc);

    // Q8_K: 256 int8 values = 64 int32s per block, but QR8_K=1 so we need WARP_SIZE ints per row
    __shared__ int   tile_x_ql[mmq_y * (WARP_SIZE)       + mmq_y];
    __shared__ half2 tile_x_dm[mmq_y * (WARP_SIZE/QI8_K) + mmq_y/QI8_K];

    *x_ql = tile_x_ql;
    *x_dm = tile_x_dm;
}

template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_q8_K(
    const void * __restrict__ vx, int * __restrict__ x_ql, half2 * __restrict__ x_dm, int * __restrict__ x_qh,
    int * __restrict__ x_sc, const int & i_offset, const int & i_max, const int & k, const int & blocks_per_row) {
    GGML_UNUSED(x_qh);
    GGML_UNUSED(x_sc);
    ...
    const int kbx  = k / QI8_K; // == 0 if QK_K == 256 (since QI8_K = 64)
    const int kqsx = k % QI8_K; // == k if QK_K == 256

    const block_q8_K * bx0 = (const block_q8_K *) vx;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
        ...
        const block_q8_K * bxi = bx0 + i*blocks_per_row + kbx;

        // Load 4 int8 values as one int32 (kqsx selects which group of 4)
        x_ql[i * (WARP_SIZE + 1) + k] = get_int_from_int8(bxi->qs, kqsx * 4);
    }

    const int blocks_per_tile_x_row = WARP_SIZE / QI8_K; // == 0 if QK_K == 256
    const int kbxd = k % blocks_per_tile_x_row;          // == 0 if QK_K == 256
    float * x_dmf = (float *) x_dm;

#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI8_K) {
        int i = (i0 + i_offset * QI8_K + k / blocks_per_tile_x_row) % mmq_y;

        if (need_check) {
            i = min(i, i_max);
        }

        const block_q8_K * bxi = bx0 + i*blocks_per_row + kbxd;

        x_dmf[i * (WARP_SIZE/QI8_K) + i / QI8_K + kbxd] = bxi->d;
    }
}

static __device__ __forceinline__ float vec_dot_q8_K_q8_1_impl_mmq(
    const int * __restrict__ v, const int * __restrict__ u, const float & d8_K, const float * __restrict__ d8_1) {

    float sumf = 0.0f;

#pragma unroll
    for (int i = 0; i < VDR_Q8_K_Q8_1_MMQ; ++i) {
        int sumi = ggml_cuda_dp4a(v[i], u[i], 0);
        sumf += d8_1[i] * sumi;
    }
...
static __device__ __forceinline__ float vec_dot_q8_K_q8_1_mul_mat(
    const int * __restrict__ x_ql, const half2 * __restrict__ x_dm, const int * __restrict__ x_qh, const int * __restrict__ x_sc,
    const int * __restrict__ y_qs, const half2 * __restrict__ y_ds, const int & i, const int & j, const int & k) {
    GGML_UNUSED(x_qh);
    GGML_UNUSED(x_sc);
    ...
    const int index_x = i * (WARP_SIZE + 1) + k;
    const int index_y = j * WARP_SIZE       + k;

    return vec_dot_q8_K_q8_1_impl_mmq(&x_ql[index_x], &y_qs[index_y], x_dmf[i * (WARP_SIZE/QI8_K) + i/QI8_K], &y_df[index_y/QI8_1]);
}
...
extern "C" __global__ void
    ...
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
    const int mmq_x  =  MMQ_X_Q8_1_AMPERE;
    const int mmq_y  =  MMQ_Y_Q8_1_AMPERE;
    const int nwarps = NWARPS_Q8_1_AMPERE;

    mul_mat_q<QK8_1, QR8_1, QI8_1, false, block_q8_1, mmq_x, mmq_y, nwarps, allocate_tiles_q8_1<mmq_y>,
        load_tiles_q8_1<mmq_y, nwarps, true>, VDR_Q8_1_Q8_1_MMQ, vec_dot_q8_1_q8_1_mul_mat>
        (vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
}
...
extern "C" __global__ void
    ...
    const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    const int ncols_x, const int nrows_x, const int ncols_y, const int nrows_y, const int nrows_dst) {
    const int mmq_x  =  MMQ_X_Q8_K_AMPERE;
    const int mmq_y  =  MMQ_Y_Q8_K_AMPERE;
    const int nwarps = NWARPS_Q8_K_AMPERE;
    mul_mat_q<QK_K, QR8_K, QI8_K, false, block_q8_K, mmq_x, mmq_y, nwarps, allocate_tiles_q8_K<mmq_y>,
