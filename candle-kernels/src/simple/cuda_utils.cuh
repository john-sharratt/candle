...
__device__ __forceinline__ size_t restrided(
    const size_t strided_i,
    const size_t num_dims,
    const size_t *dims,
    const size_t *strides,
...
__device__ __forceinline__ float expg(float a) { return fast_exp::exp<float>(a); }
...
__device__ __forceinline__ __half expg(__half a) { return fast_exp::exp<__half>(a); }
...
__device__ __forceinline__ __nv_bfloat16 expg(__nv_bfloat16 a) { return fast_exp::exp<__nv_bfloat16>(a); }
...
// FP8 __ldg wrapper - reads the underlying byte and reconstructs the FP8 value
__device__ __forceinline__ __nv_fp8_e4m3 __ldg(const __nv_fp8_e4m3* ptr) {
    __nv_fp8_e4m3 val;
    val.__x = __ldg(reinterpret_cast<const __nv_fp8_storage_t*>(ptr));
    return val;
}

// FP8 comparison helpers - convert to float for comparison
__device__ __forceinline__ bool fp8_lt(__nv_fp8_e4m3 a, __nv_fp8_e4m3 b) { return F8E4M3_TO_FLOAT(a) < F8E4M3_TO_FLOAT(b); }
__device__ __forceinline__ bool fp8_gt(__nv_fp8_e4m3 a, __nv_fp8_e4m3 b) { return F8E4M3_TO_FLOAT(a) > F8E4M3_TO_FLOAT(b); }
__device__ __forceinline__ bool fp8_eq(__nv_fp8_e4m3 a, __nv_fp8_e4m3 b) { return F8E4M3_TO_FLOAT(a) == F8E4M3_TO_FLOAT(b); }
...
__device__ __forceinline__ __nv_fp8_e4m3 expg(__nv_fp8_e4m3 a) { return __nv_fp8_e4m3(fast_exp::exp<float>(F8E4M3_TO_FLOAT(a))); }
