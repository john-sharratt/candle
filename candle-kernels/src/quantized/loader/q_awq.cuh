...
// =============================================================================
// Q_AWQ LOADER - ACTIVATION-AWARE WEIGHT QUANTIZATION IN K/128 FORMAT
// =============================================================================
//
// This loader processes AWQ (Activation-aware Weight Quantization) weights.
// AWQ uses 4-bit asymmetric quantization with group-wise scales and zeros.
// Dequantization: w = scale * (q - zero) = scale * q + neg_sz
//   where neg_sz = -scale * zero (precomputed for FMA efficiency)
//
// Standard AWQ group size is 128 (one scale+zero per 128 elements).
// For group size 64, use q_awq_g64.cuh.
//
// LAYOUT
// ------
// Weights: [K/128, N] block_c_q_awq_k128 (80 bytes per block)
//   - qs[16]: int32 containing packed 4-bit weights (8 weights per int32)
//   - scale: half for the group
//   - zero: half (applied as q - zero before scaling)
//
// THREAD MAPPING
// --------------
// 16 threads per K/128 block, each thread handles 8 elements.
// Uses LOP3-based nibble extraction for efficient dequantization.
//
// =============================================================================
...
// =============================================================================
// VEC_DOT LOADER FOR AWQ K/128 FORMAT
// =============================================================================

template <int vdr, typename acc_t>
struct vec_dot_q_loader_q_awq_inline {
    // -------------------------------------------------------------------------
    ...
    __device__ __forceinline__ void load_part(
        const block_c_q_awq* __restrict__ x,
        const int row,
        const int kbx,
        const int num_rows
    ) {
        static_assert(N == 0, "Q_AWQ uses 1-part interface for K/128");

        const int block_idx = kbx * num_rows + row;
        const block_c_q_awq_k128* __restrict__ blk = reinterpret_cast<const block_c_q_awq_k128*>(&x[block_idx]);

        const int lane = get_lane();
        q_packed = blk->qs[lane];
        
        // Precompute neg_sz = -scale * zero for FMA: w = scale * q + neg_sz
        const half scale = blk->scale;
        const half zero = blk->zero;
...
struct vec_dot_loader_for<block_c_q_awq, vdr, act_t> {
    using type = vec_dot_q_loader_q_awq_inline<vdr, acc_for_act_t<act_t>>;
};

// =============================================================================
// GEMX DEQUANT TRAITS - Q_AWQ (4-bit AWQ with scale and zero)
// =============================================================================
// AWQ K/128 layout (80 bytes):
//   qs[0..15]: 16 × int32 = 64 bytes (128 × 4-bit weights)
//   scale: half at offset 64
//   zero: half at offset 66
//   _pad[3]: padding to 80 bytes
//
// Dequant: w = scale * (q - zero) = scale * q + neg_sz
//   where neg_sz = -scale * zero
//
// CRITICAL DIFFERENCE FROM Q4_K:
// - AWQ has ONE scale/zero pair per entire K/128 block
// - Q4_K has 4 scale/min pairs per K/128 block
// - This means AWQ is SIMPLER - same scale for all 8 k_iter slices
// =============================================================================

template <typename compute_t, typename scale_t>
struct gemx_dequant_traits<block_c_q_awq, compute_t, scale_t> {
    static constexpr bool implemented = true;
    static constexpr bool has_min = true;  // AWQ has zero point (treated like neg_min)
    static constexpr int scales_per_ktile = 1;  // One scale per K/128
    ...
    // =========================================================================
    // AWQ layout is simpler than Q4_K:
    // - qs[0..15]: 64 bytes of weights (16 threads × 8 nibbles each)
    // - scale at offset 64, zero at offset 66
    // - ONE scale/zero pair for entire K/128 block (not 4 like Q4_K)
    //
    // For MMA m16n8k16, thread t provides B[k, n] where:
    //   n = t / 4 (row 0-7)
    //   k positions: {(t%4)*2, (t%4)*2+1, (t%4)*2+8, (t%4)*2+9}
    //
    // k_iter (0-7): which K/16 slice within K/128
    // =========================================================================
    ...
    __device__ __forceinline__ static void dequant_for_mma_k16_runtime(
        ...
        // AWQ simple layout: qs[0..15] are contiguous 64 bytes, no gaps
        // k_iter selects which pair of qs ints to use
        const int qs_offset = k_iter * 8;  // Each k_iter uses 8 bytes (2 ints)
        ...
        // Load scale and zero (same for all k_iter in this block)
        const half scale = *reinterpret_cast<const half*>(row_base + 64);
        const half zero = *reinterpret_cast<const half*>(row_base + 66);
        
        // Precompute neg_sz = -scale * zero
        const half neg_sz = __hmul(__hneg(scale), zero);
        const half2 sm = make_half2(scale, neg_sz);
    ...
    // 3. Scale loaded ONCE (same for all k_iter in AWQ)
    // 4. half_idx as template parameter (compile-time branch elimination)
    //
    // AWQ K/128 layout (80 bytes):
    //   qs[0..15] at 0-63, scale at 64, zero at 66, pad at 68-79
    //
    // k_iter → qs_offset mapping (no gaps like Q4_K):
    //   0→0, 1→8, 2→16, 3→24, 4→32, 5→40, 6→48, 7→56
    //
    // half_idx=0: k_iter 0-3 (offsets 0, 8, 16, 24)
    // half_idx=1: k_iter 4-7 (offsets 32, 40, 48, 56)
    // =========================================================================
    ...
    __device__ __forceinline__ static void dequant_for_4x_mma_k16_runtime(
        ...
        // =====================================================================
        // Load scale and zero ONCE (same for entire K/128 block)
        const half scale = *reinterpret_cast<const half*>(row_base + 64);
        const half zero = *reinterpret_cast<const half*>(row_base + 66);
        const half neg_sz = __hmul(__hneg(scale), zero);
