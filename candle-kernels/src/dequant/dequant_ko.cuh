...
// Q2_KO: 2-bit affine, min ≠ 0. value 0..3 stored as crumbs — cr0 (4 low-half) / cr1 (4
// high-half) per (lane, sub) at lane*8 + sub*2, each crumb at bit 2j. No 512 B ql region;
// dm at 256. Inverse of quantize_q2_ko_kernel, byte-identical to CPU `dequant_q2_ko`.
__global__ void dequantize_q2_ko_kernel(
    const uint8_t* __restrict__ chunk, float* __restrict__ out, int nrows, int ncols)
{
    ...
    for (int chunk_idx = (int)(((int64_t)blockIdx.x * blockDim.x + threadIdx.x) >> 5);
        ...
        for (int sub = 0; sub < 4; ++sub) {
            ...
            #pragma unroll
            for (int i = 0; i < 4; ++i) {
                const uint32_t qlo = (cr0 >> (2 * i)) & 0x3;
                const uint32_t qhi = (cr1 >> (2 * i)) & 0x3;
                lo[i] = __fadd_rn(__fmul_rn(scale, (float)qlo), mn);
                hi[i] = __fadd_rn(__fmul_rn(scale, (float)qhi), mn);
            }
            __stcs((float4*)(out + obase + sub * 32 + q3 * 4), make_float4(lo[0], lo[1], lo[2], lo[3]));
