...
// Q8_0: 32 elements, 8-bit quantization (most common for activations)
...
__device__ __forceinline__ void dequantize_q8_0_dual(
    const int8_t* __restrict__ qs0,
    const int8_t* __restrict__ qs1,
    ...
    const int8_t* qs = block_idx ? qs1 : qs0;
    const half2 d2 = __half2half2(block_idx ? d1 : d0);
    const int idx = pair_idx * 2;
    const half2 q2 = __halves2half2(
        __int2half_rn(qs[idx]),
        __int2half_rn(qs[idx + 1])
    );
...
__device__ __forceinline__ void dequantize_q8_0_dual(
    ...
    out[0] = from_f32<compute_t>(qs[idx] * d);
    out[1] = from_f32<compute_t>(qs[idx + 1] * d);
}

// Block wrappers - extract from AoS structs and call dual-block core

__device__ __forceinline__ void dequantize_block_q8_0(
    const block_q8_0* __restrict__ src,
    __half* __restrict__ dst) {
    dequantize_q8_0_dual(src[0].qs, src[1].qs, src[0].d, src[1].d, dst);
}
