...
// `resolve_pos`, trying the slice `hint` first.
//
// A caller that walks positions in order — a warp's columns of one tile, the
// glue kernel's column stream — nearly always lands in the slice it resolved
// last. Committed ranges `[rope, rope + len)` never overlap, so a position
...
__device__ __forceinline__ void resolve_pos_hinted(
    ...
    if (hint >= 0 && hint < (int)slot_hdr.n_slices) {
        ...
        const int start = (int)slice_rope(s);
        if (k_pos >= start && k_pos < start + (int)slice_len(s)) {
            slice_idx = hint;
            in_blk = (int)slice_offset(s) + (k_pos - start);
            return;
        }
    }
    resolve_pos(slot_hdr, k_pos, slice_idx, in_blk);
}
