...
use super::desc;
use super::geometry::{LatentGeometry, D512_R64_B16};

/// The geometry this build's kernels are compiled for.
///
/// Everything below is derived from it, so the shape is declared once. Changing
/// it means adding the matching kernel instantiation first — see
/// [`SUPPORTED`](super::geometry::SUPPORTED) for the full checklist.
pub const GEOMETRY: LatentGeometry = D512_R64_B16;

pub const HEAD_DIM: usize = GEOMETRY.head_dim;
pub const ROPE_DIM: usize = GEOMETRY.rope_dim;
pub const NOPE_DIM: usize = GEOMETRY.nope_dim();
/// Single-latent band count. Must equal the `NPAL` template argument the
/// kernels are instantiated with in `paged_latent_api_bf16.cu` and
/// `LATENT_N_BANDS` in candle-nn.
pub const N_BANDS: usize = GEOMETRY.n_bands;
pub const SUB_DIM: usize = GEOMETRY.sub_dim();
pub const CHUNK: usize = 32;
/// KvHead record byte size (`slot_types.cuh` layout).
pub const KVHEAD_BYTES: usize = GEOMETRY.kvhead_bytes();

/// Refuse to launch unless the model, this build's host constants, and the
/// compiled kernels all describe the SAME latent.
///
/// Four declarations have to agree and none can see the others:
///
/// 1. `cfg.head_dim` / `cfg.rope_head_dim` — what the checkpoint actually holds;
/// 2. [`Arch::geometry`] — the shape the model says its kernels need;
/// 3. [`GEOMETRY`] here — the shape this build's host code lays records out for;
/// 4. `LM_HEAD_DIM` and friends in `paged_latent_api_bf16.cu` — the shape the
///    kernels were instantiated at, read back through `run_latent_geometry`.
///
/// A divergence between any pair is invisible at runtime: the host would write
/// `KvHead` bands at one set of offsets and the kernel would read another,
/// producing **wrong attention rather than a fault**. That is why this is a load
/// error and not an assertion someone might compile out.
///
/// (2) is the check that matters once a second model exists — a successor
/// declaring a 768-dim latent against kernels built for 512 is precisely the
/// mistake the `Arch` split makes possible, and comparing only (3) against (4)
/// would let it through.
#[cfg(feature = "cuda")]
pub fn assert_geometry(cfg: &Config) -> Result<()> {
    let g = cfg.arch.geometry();
    let arch_id = cfg.arch.id();

    // (1) vs (2) — the arch's declared latent must describe its own config.
    if g.head_dim != cfg.head_dim || g.rope_dim != cfg.rope_head_dim {
        candle::bail!(
            "{arch_id} declares latent geometry (head_dim {}, rope_dim {}) but its config says \
             (head_dim {}, rope_dim {}). Arch::geometry and the config describe the same \
             tensors, so one of them is wrong",
            g.head_dim,
            g.rope_dim,
            cfg.head_dim,
            cfg.rope_head_dim
    ...
    // (2) vs the built set — is any kernel compiled for this shape at all?
    if !super::geometry::is_supported(g) {
        candle::bail!(
            "{arch_id} needs latent geometry (head_dim {}, rope_dim {}, n_bands {}), which no \
             kernel is built for. Supported: {:?}. See latent_moe::geometry::SUPPORTED for what \
             adding one requires",
            g.head_dim,
            g.rope_dim,
            g.n_bands,
            super::geometry::SUPPORTED
        );
    }

    // (2)/(3) vs (4) — and this build's kernels must be the ones that got built.
    let compiled = candle_kernels::paged_latent::latent_geometry();
    let host = (HEAD_DIM, ROPE_DIM, N_BANDS);
    let wanted = (g.head_dim, g.rope_dim, g.n_bands);
    if compiled != wanted || host != wanted {
        candle::bail!(
            "latent geometry disagreement — {arch_id} wants {wanted:?}, host constants are \
             {host:?}, compiled kernels are {compiled:?} (head_dim, rope_dim, n_bands). \
             Launching would read KvHead bands at mismatched offsets. Align paged.rs GEOMETRY \
             with LM_HEAD_DIM/LM_ROPE_DIM/LM_NPAL in paged_latent_api_bf16.cu"
        );
    }
    Ok(())
}
...
pub const ROPE_TAB_LEN: usize = GEOMETRY.rope_table_len(ROPE_HI_DIM, ROPE_LO_DIM);
...
pub const NOPE_BANDS: usize = GEOMETRY.nope_bands();
...
// NOT `clamp`: this mirrors CUDA `fminf`/`fmaxf`, which return the non-NaN
// operand, so a NaN centroid saturates to ±127 here exactly as it does on the
// device. `f32::clamp` propagates the NaN instead and would break the byte
// equality this mirror exists to prove.
...
// Same `fminf`/`fmaxf` NaN semantics as [`q0_encode_centroid`] — see its note.
#[allow(clippy::manual_clamp)]
...
/// One round-tripped chunk: the per-band arena bytes, and the decoded token rows
/// with each dim back in its natural position.
pub type MappedChunk = ([Vec<u8>; N_BANDS], Vec<[f32; HEAD_DIM]>);
...
pub fn quant_bands(v: &[f32; HEAD_DIM]) -> ([i8; HEAD_DIM], [f32; N_BANDS]) {
    ...
    for (band, scale) in scales.iter_mut().enumerate() {
        let lo = band * SUB_DIM;
        let src = &v[lo..lo + SUB_DIM];
        let mx = src.iter().fold(0.0f32, |m, x| m.max(x.abs()));
        let mut s = mx / 127.0;
        if s == 0.0 {
            s = 1.0;
        }
        *scale = s;
        ...
        for (d, &x) in src.iter().enumerate() {
            q[lo + d] = (x * inv).clamp(-127.0, 127.0).round_ties_even() as i32 as i8;
        }
    }
    (q, scales)
}
...
pub fn build_corpus_cache(
    ...
    for (band, band_scale) in scale.iter_mut().enumerate() {
        let lo = band * SUB_DIM;
        let src = &v[lo..lo + SUB_DIM];
        let mx = src.iter().fold(0.0f32, |m, x| m.max(x.abs()));
        let mut s = mx / 127.0;
        ...
        *band_scale = s;
        let inv = 1.0 / s;
        for (d, &x) in src.iter().enumerate() {
            nope[lo + d] = (x * inv).clamp(-127.0, 127.0).round_ties_even() as i32 as i8;
        }
    }
...
mod tests {
    // Host mirrors of the latent kernels. Every one of these is a transcription:
    // the kernel's own loop bounds, its float literals to full width, and its
    // `fminf`/`fmaxf` NaN behaviour. Restyling them into iterator form or
    // trimming digits would break the line-by-line diff against the `.cu` that
    // makes a mismatch findable.
    #![allow(
    ...
    /// The geometry is declared in three places that cannot import each other —
    /// [`GEOMETRY`] here, the `LM_*` macros in `paged_latent_api_bf16.cu`, and
    /// `LATENT_N_BANDS` in candle-nn (which lays out the arena records but does
    /// not depend on candle-kernels). candle-transformers is the one crate that
    /// sees all three, so this is where they get tied together.
    ///
    /// A drift between them is invisible to every numerical test: the host would
    /// write `KvHead` bands at one set of offsets and the kernel would read
    /// another, yielding wrong attention rather than a fault.
    #[test]
    #[cfg(feature = "cuda")]
    fn host_geometry_matches_the_compiled_kernels() {
        use super::super::arch::Arch;
        use candle_nn::kv_cache::{LATENT_NOPE_BANDS, LATENT_N_BANDS};

        assert_eq!(
            candle_kernels::paged_latent::latent_geometry(),
            (HEAD_DIM, ROPE_DIM, N_BANDS),
            "compiled kernel geometry differs from paged.rs GEOMETRY"
        );
        assert_eq!(
            LATENT_N_BANDS, N_BANDS,
            "candle-nn arena band count differs"
        );
        assert_eq!(
            LATENT_NOPE_BANDS, NOPE_BANDS,
            "candle-nn nope/rope band split differs"
        );
        // And the full load-time check passes for a real model, which additionally
        // ties in the arch's declared geometry and the checkpoint's own dims.
        assert_geometry(&crate::models::deepseek4::DEEPSEEK_V4.defaults())
            .expect("geometry check passes when model, host, and kernels agree");
    }

    /// A model whose declared latent no kernel implements must be REJECTED, not
    /// launched. This is the case a successor architecture introduces — the one
    /// a host-vs-kernel constant comparison alone would let through — so it is
    /// checked with a config that claims a geometry nothing is built for.
    #[test]
    #[cfg(feature = "cuda")]
    fn a_model_wanting_an_unbuilt_geometry_is_refused() {
        use super::super::arch::Arch;
        let mut cfg = crate::models::deepseek4::DEEPSEEK_V4.defaults();
        // A legal latent (1024 = 32 bands of 32 dims, rope on a band boundary)
        // that no kernel is instantiated for.
        cfg.head_dim = 1024;
        cfg.rope_head_dim = 128;
        let err = assert_geometry(&cfg).expect_err("an unbuilt geometry must not load");
        let msg = err.to_string();
        // It must fail on the ARCH/CONFIG disagreement first — the arch still
        // declares 512 — which is itself the check the review found missing.
        assert!(
            msg.contains("declares latent geometry"),
            "expected the arch/config disagreement to be named, got: {msg}"
        );
    }
    ...
    /// CPU and GPU.
    // The constants are the kernel's literals, character for character, so this
    // stays a line-by-line diff against the `.cu`. Two of them happen to be the
    // nearest f64 to TAU and 2/π (verified bit-identical to the `std` consts),
    // but 1/TAU on the line above has no `std` name — naming half of them would
    // cost the transcription and buy no precision.
