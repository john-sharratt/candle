...
fn pool_group(pools: &[&GroupPool], d: usize, generation: &Generation) -> Result<Tensor> {
...
impl IncrementalCompressor {
    ...
    pub fn emit_groups_projected(
        ...
        match self.assemble_groups(kv, score)? {
            None => Ok(None),
            Some(gp) => {
                let scope = desc::scope(self.c.device())?;
...
fn build_assemble_geom(r: usize, seqs: &[(usize, usize)], dev: &Device) -> Result<AssembleGeom> {
    let n_seq = seqs.len();
...
mod tests {
    ...
    fn pool_dense(kv: &Tensor, score: &Tensor) -> Result<Tensor> {
        ...
        let scope = desc::scope(kv.device())?;
