...
fn indexer_score_reduce(scores: &Tensor, w: &Tensor, counts: Option<&Tensor>) -> Result<Tensor> {
    ...
    if matches!(scores.device(), Device::Cuda(_)) {
        ...
        let out = Tensor::empty((b, m), DType::F32, scores.device())?;
        {
            ...
            let cnt_guard = match counts {
                Some(c) => {
                    if c.dims1()? != b {
                        candle::bail!("indexer_score_reduce: counts {:?} must be [{b}]", c.dims());
                    }
...
impl FloatGallery {
    ...
    pub fn append_batch(
        ...
        let scope = desc::scope(&self.device)?;
        append_batch_all(
            &mut [AppendSlice {
                gallery: self,
                runs: vec![(0, n)],
    ...
    pub fn gather_corpus_into(
        ...
        if self.spilled {
            ...
            let staged =
                Tensor::from_vec(staged, n_bytes, &Device::Cpu)?.to_device(&self.device)?;
    ...
    pub fn batched_causal_select_device(
        ...
        // [s, n_corpus]
        let weighted = indexer_score_reduce(&scores, weights, Some(&vis))?;
...
pub fn bdp_recall_batched(
    ...
    let sign_ptrs_t = desc::stage(&sign_ptrs, generation)?;
    {
        let (sq, _) = q_signs.storage_and_layout();
        let (sc, _) = counts.storage_and_layout();
...
pub fn topm_select_batched(
    ...
    let out = Tensor::zeros((n_sess, max_m), DType::U32, counts.device())?;
    {
        ...
        let (cp, offp, cntp, hp, mp, outp) = (ptr(&sc)?, off, cnt, ptr(&sh)?, ptr(&sm)?, ptr(&so)?);
        let code = unsafe {
...
pub fn two_stage_select_batched(
    ...
    let shortlist_deep: Option<Tensor> = if deep_idx.is_empty() {
        ...
        let off_t = desc::stage_slice(&off, generation)?;
        let cnt_t = desc::stage_slice(&cnt, generation)?;
    ...
    // [big,h,mm]
    let scores = q_all.matmul(&keys_all.transpose(1, 2)?.contiguous()?)?;
...
pub fn gather_corpus_batched(
    ...
    let ptrs_t = desc::stage(&ptrs, generation)?;
    let meta_t = desc::stage_slice(&meta, generation)?;
...
#[cfg(all(test, feature = "cuda"))]
mod tests {
    // Host mirrors of the BDP sign-pack kernel: the dim loops carry the kernel's
    // own bit positions, so they stay index-based.
