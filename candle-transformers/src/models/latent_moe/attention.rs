...
impl Attention {
    ...
    // The mask depends on the full attention geometry — window, compression
    // ratio, offsets, and the two query views — and every argument is read
    // independently. Bundling them into a struct would only move the same list
    // one level out.
    ...
    /// CUDA and the shapes clear the q8a1024 byte-row gather's alignment
    /// (`per_group % 1024`, which implies the grouped GEMM's own `% 128`, and `o_lora_rank % 32`),
...
mod tests {
    ...
    fn grouped_output_proj_matches_per_group() -> Result<()> {
        ...
        for &s in &[1usize, 5] {
            ...
            assert!(
                diff < 1e-3,
                "s={s}: grouped vs per-group max abs diff {diff}"
            );
