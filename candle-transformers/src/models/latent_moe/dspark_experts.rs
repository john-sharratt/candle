...
impl DsparkStreamingMoe {
    ...
    pub fn load(
        ...
        for b in 0..n_blocks {
            ...
            let g = m.qtensor(&w(Weight::RoutedExperts(Ffn::Gate)), cpu)?;
            let u = m.qtensor(&w(Weight::RoutedExperts(Ffn::Up)), cpu)?;
            let d = m.qtensor(&w(Weight::RoutedExperts(Ffn::Down)), cpu)?;
            let (kg, ku, kd) = (
...
mod tests {
    use super::*;
    // End-to-end gate against the real dflash drafter checkpoint.
    #[cfg(feature = "cuda")]
    ...
    fn streaming_moe_matches_reference() -> Result<()> {
        ...
        let cfg = config_from_gguf(&m, &DFLASH)?;
        let block = 0usize;
