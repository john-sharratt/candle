...
use super::arch::{Arch, Global, HyperPart};
...
/// A full model that streams its blocks from disk on each forward.
...
impl StreamingModel {
    ...
    pub fn open(
        ...
        let g = |x| arch.global(x);
        let embed = loader::dequant_f32(&mut gguf, g(Global::Embedding), device)?;
        let output_norm = loader::dequant_f32(&mut gguf, g(Global::OutputNorm), device)?;
        let lm_head = loader::qlinear(&mut gguf, g(Global::OutputHead), device)?;
        let hc = HyperConnection::new(cfg.hc_mult, cfg.hc_sinkhorn_iters, cfg.hc_eps);
        let hc_head = HyperParams {
            fn_w: loader::dequant_f32(&mut gguf, g(Global::OutputHyper(HyperPart::Fn)), device)?
                .into(),
            base: loader::dequant_f32(&mut gguf, g(Global::OutputHyper(HyperPart::Base)), device)?,
            scale: loader::dequant_f32(
                &mut gguf,
                g(Global::OutputHyper(HyperPart::Scale)),
                device,
            )?,
        };
...
mod tests {
    use super::*;
    // Streams the real DeepSeek-V4-Flash checkpoint block by block.
    ...
    fn full_model_generate_coherent() -> Result<()> {
        ...
        let mut model = StreamingModel::open(&path, &DEEPSEEK_V4, &device)?;
