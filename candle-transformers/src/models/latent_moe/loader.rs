...
use super::arch::{
    Arch, Compressor as CompressorSite, CompressorPart, Ffn, Global, Hyper as HyperSite, HyperPart,
    Meta, Weight,
};
...
/// Load a `Compressor` — its two projections, positional bias, and norm scale.
...
fn load_compressor(
    ...
    let part = |p| cfg.arch.weight(layer, Weight::Compressor(site, p));
    // wkv/wgate are matmul projections → int8-KO. `ape` is an additive positional bias (not a
    // matmul weight) and stays dense.
    let wkv = qlinear_int8(m, &part(CompressorPart::Kv), device, mode)?;
    let wgate = qlinear_int8(m, &part(CompressorPart::Gate), device, mode)?;
    ...
    let ape = dequant_native(m, &part(CompressorPart::Ape), device)?.to_dtype(DType::F32)?;
    let norm = dequant_f32(m, &part(CompressorPart::Norm), device)?;
    Ok(Compressor::new(
        wkv,
        wgate,
        ape,
...
pub fn load_attention(
    ...
    let w = |x| cfg.arch.weight(layer, x);
    ...
    let (compressor, indexer) = if kind.compresses() {
        ...
        let indexer = if kind.has_indexer() {
            ...
            Some(Indexer::new(
                qlinear_int8(m, &w(Weight::IndexerQB), device, mode)?,
                qlinear_int8(m, &w(Weight::IndexerProj), device, mode)?,
                icomp,
                cfg.index_n_heads,
                cfg.index_head_dim,
                cfg.rope_head_dim,
    ...
    let p = AttentionParams {
        wq_a: qlinear_int8(m, &w(Weight::AttnQA), device, mode)?,
        q_norm: dequant_f32(m, &w(Weight::AttnQANorm), device)?,
        wq_b: qlinear_int8(m, &w(Weight::AttnQB), device, mode)?,
        wkv: qlinear_int8(m, &w(Weight::AttnKv), device, mode)?,
        kv_norm: dequant_f32(m, &w(Weight::AttnKvANorm), device)?,
        // Keep wo_a quantized (Q8_0) in VRAM; `output_proj` dequantizes it per layer on use.
        wo_a: load_wo_a_groups(m, &w(Weight::AttnOutputA), cfg.o_groups, device, mode)?,
        wo_b: qlinear_int8(m, &w(Weight::AttnOutputB), device, mode)?,
        attn_sink: dequant_f32(m, &w(Weight::AttnSinks), device)?,
        compressor,
        indexer,
    };
...
pub fn load_moe(
    ...
    let gate_w = dequant_native(m, &w(Weight::FfnGateInp), device)?;
    let (bias, tid2eid) = if cfg.is_hash_layer(layer) {
        let t = m.read_int_tensor_u32(&w(Weight::FfnGateTid2Eid), device)?;
        (None, Some(t))
    } else {
        (
            Some(dequant_f32(m, &w(Weight::ExpProbsBias), device)?),
            None,
        )
    };
    let gate = Gate::new(
    ...
    let gate_e = load_experts_3d(
        m,
        &w(Weight::RoutedExperts(Ffn::Gate)),
        ne,
        inter,
        dim,
        device,
    )?;
    let up_e = load_experts_3d(
        m,
        &w(Weight::RoutedExperts(Ffn::Up)),
        ne,
        inter,
        dim,
        device,
    ...
    let down_e = load_experts_3d(
        m,
        &w(Weight::RoutedExperts(Ffn::Down)),
        ne,
        dim,
        inter,
        device,
    ...
    let shared = Expert::new(
        qlinear(m, &w(Weight::SharedExpert(Ffn::Gate)), device)?,
        qlinear(m, &w(Weight::SharedExpert(Ffn::Down)), device)?,
        qlinear(m, &w(Weight::SharedExpert(Ffn::Up)), device)?,
        cfg.swiglu_limit,
    );
    Ok(MoE::new(gate, experts, shared, dim))
}

/// Load one hyper-connection's mixing matrix, base, and scale.
///
/// `part` resolves a [`HyperPart`] to its tensor name — [`hc_block`] for a
/// block's two sites, [`hc_output`] for the model-level one — so both live on
/// the same path.
pub fn load_hc_params(
    m: &mut GgufModel,
    part: impl Fn(HyperPart) -> String,
    device: &Device,
    mode: Int8Mode,
    ...
    Ok(super::hyper::HyperParams {
        ...
        fn_w: qlinear_int8(m, &part(HyperPart::Fn), device, mode)?,
        base: dequant_f32(m, &part(HyperPart::Base), device)?,
        scale: dequant_f32(m, &part(HyperPart::Scale), device)?,
    })
}

/// Name resolver for one of a block's two hyper-connection sites.
pub fn hc_block(
    cfg: &Config,
    site: HyperSite,
    layer: usize,
) -> impl Fn(HyperPart) -> String + use<'_> {
    move |p| cfg.arch.weight(layer, Weight::Hyper(site, p))
}

/// Name resolver for the model-level output hyper-connection.
pub fn hc_output(cfg: &Config) -> impl Fn(HyperPart) -> String + use<'_> {
    move |p| cfg.arch.global(Global::OutputHyper(p)).to_string()
}
...
pub fn load_block(
    ...
    let hc_attn = load_hc_params(
        m,
        hc_block(cfg, HyperSite::Attn, layer),
        device,
        Int8Mode::Off,
    )?;
    let hc_ffn = load_hc_params(
        m,
        hc_block(cfg, HyperSite::Ffn, layer),
        device,
        Int8Mode::Off,
    )?;
    let attn_norm = dequant_f32(m, &w(Weight::AttnNorm), device)?;
    let ffn_norm = dequant_f32(m, &w(Weight::FfnNorm), device)?;
...
pub fn load_dspark_block(
    ...
    let mode = Int8Mode::Performance;
    let hc_attn = load_hc_params(m, hc_block(cfg, HyperSite::Attn, layer), device, mode)?;
    let hc_ffn = load_hc_params(m, hc_block(cfg, HyperSite::Ffn, layer), device, mode)?;
    let attn_norm = dequant_f32(m, &w(Weight::AttnNorm), device)?;
    let ffn_norm = dequant_f32(m, &w(Weight::FfnNorm), device)?;
...
/// Read a model's GGUF metadata into a [`Config`].
///
/// Starts from [`Arch::defaults`] and overrides only what the file actually
/// carries, so the released model's values are stated in exactly one place (its
/// arch) rather than repeated as per-key fallbacks here. A key the model bakes
/// into its weights instead of writing down — the YaRN parameters, for
/// DeepSeek-V4 — therefore needs no special case: it is simply never present.
///
/// The `general.architecture` string in the file is checked against
/// [`Arch::id`]: a GGUF for a different model would parse its metadata under the
/// wrong keys and silently yield defaults everywhere.
pub fn config_from_gguf(m: &GgufModel, arch: &'static dyn Arch) -> Result<Config> {
    let file_arch = m
        .metadata
        .get("general.architecture")
        .and_then(|v| v.to_string().ok())
        .cloned();
    if let Some(found) = &file_arch {
        if found != arch.id() {
            candle::bail!(
                "GGUF declares general.architecture = {found:?} but it is being loaded as \
                 {:?}. Loading it anyway would read every hyperparameter under the wrong \
                 metadata keys and silently fall back to {:?}'s defaults",
                arch.id(),
                arch.id()
            );
        }
    ...
    let mut cfg = arch.defaults();
    let g = |k: Meta| m.metadata.get(&arch.meta(k));
    let uset = |dst: &mut usize, v: Option<&Value>| {
        if let Some(x) = uget(v) {
            *dst = x;
        }
    };
    let fset = |dst: &mut f64, v: Option<&Value>| {
        if let Some(x) = fget(v) {
            *dst = x;
        }
    };

    uset(&mut cfg.vocab_size, g(Meta::VocabSize));
    uset(&mut cfg.dim, g(Meta::EmbeddingLength));
    uset(&mut cfg.moe_inter_dim, g(Meta::ExpertFeedForwardLength));
    uset(&mut cfg.n_layers, g(Meta::BlockCount));
    uset(&mut cfg.n_hash_layers, g(Meta::HashLayerCount));
    uset(&mut cfg.n_heads, g(Meta::AttentionHeadCount));
    uset(&mut cfg.n_routed_experts, g(Meta::ExpertCount));
    uset(&mut cfg.n_shared_experts, g(Meta::ExpertSharedCount));
    uset(&mut cfg.n_activated_experts, g(Meta::ExpertUsedCount));
    fset(&mut cfg.route_scale, g(Meta::ExpertWeightsScale));
    if let Some(x) = array_first_f32(g(Meta::SwigluClampExp)) {
        cfg.swiglu_limit = x;
    }
    uset(&mut cfg.q_lora_rank, g(Meta::QLoraRank));
    uset(&mut cfg.head_dim, g(Meta::KeyLength));
    uset(&mut cfg.rope_head_dim, g(Meta::RopeDimensionCount));
    fset(&mut cfg.norm_eps, g(Meta::LayerNormRmsEpsilon));
    uset(&mut cfg.o_groups, g(Meta::OutputGroupCount));
    uset(&mut cfg.o_lora_rank, g(Meta::OutputLoraRank));
    uset(&mut cfg.window_size, g(Meta::SlidingWindow));
    fset(&mut cfg.compress_rope_theta, g(Meta::CompressRopeFreqBase));
    fset(&mut cfg.rope_theta, g(Meta::RopeFreqBase));
    uset(&mut cfg.index_n_heads, g(Meta::IndexerHeadCount));
    uset(&mut cfg.index_head_dim, g(Meta::IndexerKeyLength));
    uset(&mut cfg.index_topk, g(Meta::IndexerTopK));
    uset(&mut cfg.hc_mult, g(Meta::HyperConnectionCount));
    uset(
        &mut cfg.hc_sinkhorn_iters,
        g(Meta::HyperConnectionSinkhornIterations),
    );
    fset(&mut cfg.hc_eps, g(Meta::HyperConnectionEpsilon));
    if let Some(r) = compress_ratios(g(Meta::CompressRatios)) {
        cfg.compress_ratios = r;
    }
    Ok(cfg)
}
...
fn uget(v: Option<&Value>) -> Option<usize> {
    let v = v?;
    v.to_u32()
        .ok()
        .map(|x| x as usize)
        .or_else(|| v.to_i32().ok().map(|x| x as usize))
}

fn fget(v: Option<&Value>) -> Option<f64> {
    v?.to_f32().ok().map(|x| x as f64)
}

/// First element of an F32 array metadata value — the per-layer
/// `swiglu_clamp_exp` is one entry per block, all identical.
fn array_first_f32(v: Option<&Value>) -> Option<f64> {
    v?.to_vec().ok()?.first()?.to_f32().ok().map(|x| x as f64)
}

/// The per-layer compression schedule, an integer array. `None` when the key is
/// absent, leaving the arch's own schedule in place.
fn compress_ratios(v: Option<&Value>) -> Option<Vec<usize>> {
    Some(
        v?.to_vec()
            .ok()?
            .iter()
...
mod tests {
    use super::*;
    // These read the real DeepSeek-V4-Flash GGUF off disk, so they name the
    ...
    /// A config parsed from a real GGUF keeps the arch's defaults for keys the
    /// file omits, and takes the file's value for keys it carries. Model-agnostic
    /// — what the parsed numbers *mean* for DeepSeek-V4 is asserted in
    /// `models::deepseek4`.
    #[test]
    fn config_from_real_gguf_overrides_only_present_keys() -> Result<()> {
        let first = first_split();
        if !first.exists() {
            eprintln!("[skip] real GGUF not present");
            return Ok(());
        ...
        let cfg = config_from_gguf(&m, &DEEPSEEK_V4)?;
        let defaults = DEEPSEEK_V4.defaults();

        // `block_count` IS in the metadata, so the parsed value came from the file.
        assert!(m.metadata.contains_key(&DEEPSEEK_V4.meta(Meta::BlockCount)));
        assert_eq!(
            cfg.n_layers,
            uget(m.metadata.get(&DEEPSEEK_V4.meta(Meta::BlockCount))).unwrap()
        );

        // The YaRN parameters are NOT stored by this arch, so they must have been
        // left at the defaults rather than zeroed.
        assert!(!m.metadata.contains_key("deepseek4.rope.scaling.factor"));
        assert_eq!(cfg.rope_factor, defaults.rope_factor);
        assert_eq!(cfg.beta_fast, defaults.beta_fast);
        assert_eq!(cfg.beta_slow, defaults.beta_slow);
        assert_eq!(cfg.original_seq_len, defaults.original_seq_len);

        // The arch travels with the config.
        assert_eq!(cfg.arch.id(), DEEPSEEK_V4.id());
