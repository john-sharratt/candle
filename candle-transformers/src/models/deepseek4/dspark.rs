...
#[cfg(feature = "cuda")]
use super::loader::{load_dspark_block, load_hc_params, qlinear};
#[cfg(feature = "cuda")]
...
impl DsparkConfig {
    /// Parse the drafter's config from its GGUF metadata.
    pub fn from_gguf(m: &GgufModel) -> Result<Self> {
        let base = config_from_gguf(m)?;
        let block_size = m
...
impl DsparkDrafter {
    ...
    pub fn load(model_path: &std::path::Path, device: &Device) -> Result<Self> {
        ...
        let cfg = DsparkConfig::from_gguf(&m)?;
        ...
        let output_hc = load_hc_params(&mut m, "output_hc", device, Int8Mode::Off)?;
...
mod tests {
    ...
    fn compare_dspark_vs_target_experts() -> Result<()> {
        ...
        let n_tgt = super::super::loader::config_from_gguf(&tm)?.n_layers;
        for db in 0..3usize {
    ...
    fn real_dspark_loads_and_runs_on_gpu() -> Result<()> {
        ...
        let mut drafter = DsparkDrafter::load(&path, &device)?;
        eprintln!(
