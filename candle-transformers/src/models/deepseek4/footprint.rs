...
mod tests {
    ...
    /// Raw expected bytes for the tiny config (head_dim=32, index_head_dim=16,
    ...
    fn footprint_raw_bytes_tiny_config() {
        ...
        // Window: 6 layers × min(1000, 8)=8 rows × 32 dims × 1 B (FP8).
        assert_eq!(fp.window_bytes, 6 * 8 * 32);
        // Corpus: r4 layers (2, 4): G=250 → 250×(32+16)×4 B = 48_000 each.
        //         r128 layers (3, 5): G=7 → 7×(32+16)×4 B = 1_344 each.
        assert_eq!(fp.corpus_bytes, 2 * 48_000 + 2 * 1_344);
        // Tails: 1000 % 4 == 0 → r4 layers carry none. 1000 % 128 == 104 → the
        // two r128 layers each hold 3×(32+16)×4 B = 576.
        assert_eq!(fp.tail_bytes, 2 * 576);
        assert_eq!(fp.total(), 1_536 + 98_688 + 1_152);
    }

    #[test]
    fn baseline_raw_bytes_tiny_config() {
        let cfg = Config::tiny();
        // 1000 tokens × 32 dims × 2 (K,V) × 2 B (F16) × 6 layers.
        assert_eq!(fp16_linear_baseline_bytes(1000, &cfg), 768_000);
    }
    ...
    fn ratio_grows_toward_exact_asymptote() {
        ...
        // Tiny config: numerator 32·2·2·6 = 768 B/token; denominator
        // 2×(48·4/4) + 2×(48·4/128) = 96 + 3 = 99 B/token → 768/99.
        let asymptote = 768.0 / 99.0;
