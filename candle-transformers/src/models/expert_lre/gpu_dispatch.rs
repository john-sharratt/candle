    ...
    pub fn build(inner: &ExpertCacheInner, device: &CudaDevice) -> Option<Self> {
            ...
            match candle::tensor_assert::drain(&d) {
                Ok(all) => {
                    ...
                    if bad > 0 {
                        ...
                        tracing::info!(
                            slots = w.len(), examined = elems, min = lo, max = hi,
        ...
        if let (Some(k), Some(dk)) = (cols, down_cols) {
            ...
            if let Some((row, e, name, want, have)) = first {
                tracing::error!(
                    mismatched, total_weights = 3 * total,
                    layer_row = row, expert = e, proj = name,
                    tiling_implies = want, actually_allocated = have,
...
mod tests {
    ...
    fn make_inner(
        ...
        ExpertCacheInner {
            ...
            last_used: vec![0; n],
            generation: 0,
            geometry: Arc::new(ZoneGeometry::new(n, full_zone(n).slot_base(n.saturating_sub(1)))),
