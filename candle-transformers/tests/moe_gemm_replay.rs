...
fn analyse_operand(
    ...
    let finite_lo = vals.iter().copied().filter(|v| v.is_finite()).fold(f32::INFINITY, f32::min);
    let finite_hi = vals
        .iter()
        .copied()
        .filter(|v| v.is_finite())
...
fn analyse_tiles(
    ...
    let starts = r.typed::<i32>("tile_b_start")?;
    let cnts = r.typed::<i32>("tile_b_cnt")?;
    let n = launch_tiles.min(experts.len()).min(starts.len()).min(cnts.len());

    let rng = |v: &[i32]| -> (i32, i32) {
        v[..n].iter().fold((i32::MAX, i32::MIN), |(lo, hi), &x| (lo.min(x), hi.max(x)))
    };
    ...
    if bad_expert.is_empty() && bad_span.is_empty() {
        ...
        for e in &guilty {
            ...
            let lo = v.iter().copied().filter(|x| x.is_finite()).fold(f32::INFINITY, f32::min);
            let hi = v
                .iter()
                .copied()
                .filter(|x| x.is_finite())
...
fn the_captured_moe_gemm_replays_identically() -> Result<()> {
    ...
    let stacked_dev = cu.memcpy_stod(&stacked_bytes).map_err(candle::Error::wrap)?;
    ...
    let saw_nan: usize = r.get("saw.nan")?;
    let saw_inf: usize = r.get("saw.inf")?;
    println!("capture saw:    nan={saw_nan} inf={saw_inf} (min={} max={})",
        r.get::<String>("saw.min")?, r.get::<String>("saw.max")?);
