//! Integration tests for the tool-provenance KV/Q dataset.
//!
//! On first run, `ensure_fixtures()` generates synthetic `TokenSignature` data
//! and writes it to `tests/tool_provenance_data/`.  Subsequent runs reuse the
//! pre-generated files.  No real model or GPU is required.
//!
//! # Scenario taxonomy (per tool, 8 tools total)
//!
//!   - 6 positive cases  (all 96 tokens strongly associated with the target tool)
//!   - 4 boundary cases  (48 hit + 48 miss tokens)
//!   - 4 negative cases  (all tokens from a different tool's concept space)
//!   - 2 no-tool cases   (generic conversational concept)
//!
//! Total: 8 × 16 = 128 scenarios, 96 tokens each.

use std::path::{Path, PathBuf};
use std::sync::OnceLock;

use candle_conversation::provenance::{ProvenanceFile, SigEntry, TokenSignature};
...
const ENTRY_BYTES_PER_TOKEN: usize = 48; // 3 depths × 16 bytes
...
/// Returns `(user_prompt, assistant_prompt)` for the given scenario.
/// `wrong_tool` is the tool whose concept is stored in the KV data for negative
/// cases — passed through to the assistant_prompt `<tool_call>` format string.
...
fn fixture_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/tool_provenance_data")
}
...
fn generate_fixtures(dir: &Path) {
    ...
    let _ = std::fs::remove_file(dir.join("signatures.prov"));
    let _ = std::fs::remove_file(dir.join("MANIFEST.json"));

    let pf = ProvenanceFile::open(dir.join("signatures.prov")).unwrap();
...
fn open_fixtures() -> (Manifest, ProvenanceFile) {
    let dir = fixture_dir();
...
#[test]
fn all_scenarios_loadable() {
    ensure_fixtures();
    let (manifest, _pf) = open_fixtures();
    assert_eq!(manifest.version, 1);
    assert_eq!(manifest.scenarios.len(), 128);
}
...
#[test]
fn all_scenarios_have_correct_token_count() {
    ensure_fixtures();
    let (manifest, _pf) = open_fixtures();
    for s in &manifest.scenarios {
        assert_eq!(s.token_count, TOKENS_PER_CHUNK as u16, "scenario {}", s.id);
    }
}

#[test]
fn all_scenarios_have_non_empty_prompts() {
    ensure_fixtures();
    let (manifest, _pf) = open_fixtures();
    for s in &manifest.scenarios {
...
fn tool_scenarios_have_system_prompt_with_tool_json() {
    ...
    for s in manifest.scenarios.iter().filter(|s| s.case_type != CaseType::NoTool) {
        ...
        assert!(
            ...
            "scenario '{}': system_prompt missing tool JSON for '{tool}'",
            s.id
        );
    }
}
...
fn positive_assistant_prompts_reference_correct_tool() {
    ...
    for s in manifest.scenarios.iter().filter(|s| s.case_type == CaseType::Positive) {
        ...
        assert!(
            ...
            "scenario '{}': assistant_prompt calls wrong tool (expected '{tool}')",
            s.id
        );
    }
}
...
#[test]
fn sig_entry_byte_offsets_are_sequential() {
    ensure_fixtures();
    let (manifest, _pf) = open_fixtures();
    let entry_size = TOKENS_PER_CHUNK as u64 * ENTRY_BYTES_PER_TOKEN as u64;
    for (i, s) in manifest.scenarios.iter().enumerate() {
        let expected = i as u64 * entry_size;
        assert_eq!(
            s.byte_offset, expected,
            "scenario '{}' at {i}: byte_offset={}",
            s.id, s.byte_offset
        );
    }
}
...
#[test]
fn read_entry_returns_correct_token_count_for_every_scenario() {
    ensure_fixtures();
...
#[test]
fn all_real_tool_names_covered() {
    ensure_fixtures();
    let (manifest, _pf) = open_fixtures();
    for &tool in TOOLS {
        assert!(
...
#[test]
fn turn_ids_are_unique_and_sequential() {
    ensure_fixtures();
    let (manifest, _pf) = open_fixtures();
    for (i, s) in manifest.scenarios.iter().enumerate() {
        assert_eq!(s.turn_id, i as u64, "scenario '{}' at {i}", s.id);
    }
}

#[test]
fn three_depths_are_symmetric_in_generated_data() {
    ensure_fixtures();
    let (manifest, pf) = open_fixtures();
