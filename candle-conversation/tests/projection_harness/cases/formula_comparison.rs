...
use candle_conversation::projection::{ContentResolver, DepthWeights, PerDepthScores, ScoreFormula, SectionId};
...
fn formula_and_depth_weight_comparison() {
    ...
    let equal_weights = DepthWeights { syntactic: 1.0, semantic: 1.0, pragmatic: 1.0 };
    ...
    for (name, formula) in formulas {
        ...
        for (probe_tool, resolver) in &resolvers {
            let intra = resolver.section_score(h.tool_section_ids[*probe_tool], *formula, &equal_weights);
            let inter: Vec<f32> = TOOLS.iter()
        ...
        println!("{:<16} {:>10.4} {:>12.4} {:>9.4} {:>9.1} {:>9.1}",
    ...
    let weight_configs: &[(&str, DepthWeights)] = &[
        ("syn=1 sem=1 prag=1", DepthWeights { syntactic: 1.0, semantic: 1.0, pragmatic: 1.0 }),
        ("syn=0 sem=1 prag=2", DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 2.0 }),
        ("syn=0 sem=0 prag=1", DepthWeights { syntactic: 0.0, semantic: 0.0, pragmatic: 1.0 }),
        ("syn=1 sem=0 prag=0", DepthWeights { syntactic: 1.0, semantic: 0.0, pragmatic: 0.0 }),
        ("syn=0 sem=1 prag=0", DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 0.0 }),
    ];

    for (fname, formula) in &[("count", ScoreFormula::Count), ("mean", ScoreFormula::Mean)] {
        println!("\n── Depth-weight sensitivity [{fname}] ─────────────────────────────────");
        println!("{:<24} {:>10} {:>12} {:>9}", "weights", "min_ratio", "mean_ratio", "max_ratio");
        ...
        for (wname, weights) in weight_configs {
            ...
            println!("{:<24} {:>10.4} {:>12.4} {:>9.4}", wname, min_r, mean_r, max_r);
        }
    }

    // Per-tool count breakdown: show intra count, best-inter count, ratio.
    println!("\n── Per-tool count breakdown (equal weights) ───────────────────────────────");
    println!("{:<14} {:>12} {:>14} {:>9} {:>14}", "tool", "intra_count", "best_inter_count", "ratio", "best_inter_tool");
    for (probe_tool, resolver) in &resolvers {
        let intra = resolver.section_score(h.tool_section_ids[*probe_tool], ScoreFormula::Count, &equal_weights);
        let (best_inter_tool, best_inter_score) = TOOLS.iter()
            .filter(|&&t| t != *probe_tool)
            .map(|&t| (t, resolver.section_score(h.tool_section_ids[t], ScoreFormula::Count, &equal_weights)))
            .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
            .unwrap();
        println!("{:<14} {:>12.0} {:>14.0} {:>9.4} {:>14}",
            probe_tool, intra, best_inter_score, ratio(intra, best_inter_score), best_inter_tool);
    }

    // Show hit_rate (count * mean / sum) which normalises for corpus size.
    println!("\n── Hit-rate (count×mean/sum) comparison ───────────────────────────────────");
    println!("{:<14} {:>12} {:>14} {:>9}", "tool", "intra_rate%", "inter_mean_rate%", "ratio");
    for (probe_tool, resolver) in &resolvers {
        ...
        let hit_rate = |s: &candle_conversation::projection::TurnScores| -> f32 {
            if s.sum > 0.0 { s.count * s.mean / s.sum } else { 0.0 }
        };
        let intra_rate = (hit_rate(&scores_intra.syn) + hit_rate(&scores_intra.sem) + hit_rate(&scores_intra.prag)) / 3.0;

        let inter_rates: Vec<f32> = TOOLS.iter()
            .filter(|&&t| t != *probe_tool)
            .map(|&t| {
        ...
        println!("{:<14} {:>12.4} {:>14.4} {:>9.4}",
            probe_tool, intra_rate * 100.0, inter_mean_rate * 100.0, ratio(intra_rate, inter_mean_rate));
    }
    println!();
}
...
fn hit_rate_score(scores: &PerDepthScores, weights: &DepthWeights) -> f32 {
    ...
    (hr(&scores.syn) * weights.syntactic
        ...
        + hr(&scores.prag) * weights.pragmatic) / w
}
...
fn depth_weight_grid_sweep() {
    ...
    type Metric = Box<dyn Fn(&Harness, &crate::resolver::HarnessResolver, &str, &DepthWeights) -> f32>;
    ...
    for (mname, score_fn) in metrics {
        ...
        rows.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap().then(b.1.partial_cmp(&a.1).unwrap()));

        println!("\n── Grid sweep [{mname}] — top 20 by min_ratio ──────────────────────────");
        println!("{:<6} {:<6} {:<6} {:>10} {:>12}", "syn", "sem", "prag", "min_ratio", "mean_ratio");
        for &(min_r, mean_r, s, e, p) in rows.iter().take(20) {
            println!("{:<6.1} {:<6.1} {:<6.1} {:>10.4} {:>12.4}", s, e, p, min_r, mean_r);
        }

        // Also show the single-band results explicitly.
        println!("\n  Single-band results:");
        for &(label, s, e, p) in &[("syn", 1.0_f32, 0.0_f32, 0.0_f32), ("sem", 0.0, 1.0, 0.0), ("prag", 0.0, 0.0, 1.0)] {
            if let Some(&(min_r, mean_r, _, _, _)) = rows.iter().find(|&&(_, _, rs, re, rp)| rs == s && re == e && rp == p) {
...
fn content_type_calibration_sweep() {
    ...
    type MetricFn = Box<dyn Fn(&Harness, &crate::resolver::HarnessResolver, &str, &DepthWeights) -> f32>;
    ...
    for (mname, score_fn) in &metrics {
        ...
        for &(min_r, mean_r, s, e, p) in rows.iter().take(5) {
            println!("{:<6.2} {:<6.2} {:<6.2} {:>10.4} {:>12.4}", s, e, p, min_r, mean_r);
        }
    ...
    println!("\n── Cross-metric summary ─────────────────────────────────────────────────────");
    println!("{:<14} {:>10} {:>12}  best weights (syn/sem/prag)", "metric", "min_ratio", "mean_ratio");
    for &(name, min_r, mean_r, s, e, p) in &best_per_metric {
        println!("{:<14} {:>10.4} {:>12.4}  {:.2}/{:.2}/{:.2}", name, min_r, mean_r, s, e, p);
    }
    ...
    if let Some(&(name, min_r, mean_r, s, e, p)) = best {
        ...
        if let Some(line) = alpha_line { println!("{line}"); }
...
fn score_threshold_and_weight_calibration() {
    ...
    let resolvers: Vec<_> = manifest.scenarios.iter()
    ...
    let weight_configs: &[(&str, DepthWeights)] = &[
        ...
        ("sem+prag",    DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 1.0 }),
        ("sem_heavy",   DepthWeights { syntactic: 0.0, semantic: 2.0, pragmatic: 1.0 }),
        ("prag_heavy",  DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 2.0 }),
        ("prag_only",   DepthWeights { syntactic: 0.0, semantic: 0.0, pragmatic: 1.0 }),
        ("sem_only",    DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 0.0 }),
    ];
    ...
    for &(wname, weights) in weight_configs {
        ...
        for (scen, resolver) in &resolvers {
            ...
            let intra = resolver.section_score(
                h.tool_section_ids[probe_tool], formula, &weights);
            ...
            for &other in TOOLS.iter().filter(|&&t| t != probe_tool) {
                let inter = resolver.section_score(
                    h.tool_section_ids[other], formula, &weights);
        ...
        if gap > 0.0 {
            println!("  ✓ Clean gap: {gap:.2}  (min_intra={min_intra:.2} − max_inter={max_inter:.2})");
        } else {
            println!("  ✗ Overlap:  {:.2}  (min_intra={min_intra:.2} < max_inter={max_inter:.2})", -gap);
        }
        println!("  Recommended threshold: {threshold:.2}  →  TP={:.0}%  FP={:.0}%",
            tp_rate * 100.0, fp_rate * 100.0);

        results.push(WeightResult {
            name: wname, weights, gap, threshold, min_intra, max_inter, tp_rate, fp_rate });
    }

    // ── fine grid sweep for gap maximisation ──────────────────────────────────
    println!("\n── Fine grid sweep — maximise gap (min_intra − max_inter) ─────────────");
    println!("{:<10} {:<6} {:<6} {:>8} {:>9} {:>9} {:>8} {:>6}",
    ...
    for &e in &levels {
        for &p in &levels {
            if e + p == 0.0 { continue; }
            let w = DepthWeights { syntactic: 0.0, semantic: e, pragmatic: p };
            ...
            for (scen, resolver) in &resolvers {
                ...
                for &other in TOOLS.iter().filter(|&&t| t != probe_tool) {
                    inter_sorted.push(resolver.section_score(h.tool_section_ids[other], formula, &w));
                }
            }
            ...
            let gap = min_i - max_i;
            let thresh = if gap > 0.0 { max_i + gap * 0.5 } else { pct(&inter_sorted, 0.90) * 1.05 };
    ...
    for &(gap, e, p, min_i, max_i, thresh, tp, _fp) in grid_rows.iter().take(15) {
        ...
        println!("{:<28} {:>8.2} {:>9.2} {:>9.2} {:>8.2} {:>5.0}%",
            tag, gap, min_i, max_i, thresh, tp * 100.0);
    }
    ...
    if let Some(best) = grid_rows.first() {
        ...
        println!("  Gap:       {gap:.2}  (min_intra={:.2} − max_inter={max_i:.2})", best.3);
        println!("  Threshold: {thresh:.2}  → TP={:.0}%  FP={:.0}%", tp * 100.0, fp * 100.0);
        println!();
        println!("  depth_weights:");
...
fn multi_layer_calibration_sweep() {
    ...
    const LAYERS: &[(&str, &str, &str, &[&str])] = &[
        ("code_reading_provenance_real_data",         "code_reading_provenance_data",         "code_reading",
         &["decode_step","kv_arena","attention_fwd","quant_block","bdp_scan","moe_route","prefill_run","rope_enc"]),
        ("static_analysis_provenance_real_data",      "static_analysis_provenance_data",      "static_analysis",
         &["cache_rs","arena_rs","compress_rs","scan_rs","scheduler_rs","projection_rs","engine_rs","config_rs"]),
        ("dependency_analysis_provenance_real_data",  "dependency_analysis_provenance_data",  "dependency_analysis",
         &["cache_deps","arena_deps","compress_deps","scan_deps","scheduler_deps","projection_deps","engine_deps","config_deps"]),
        ("architectural_analysis_provenance_real_data","architectural_analysis_provenance_data","architectural_analysis",
         &["paged_kv","quant_policy","bdp_retrieval","moe_predict","wave_batch","three_tier","o1_theorem","proj_schema"]),
        ("critical_analysis_provenance_real_data",    "critical_analysis_provenance_data",    "critical_analysis",
         &["kv_frag","quant_drift","bdp_collision","sched_block","mem_pressure","dtype_mismatch","attn_overflow","moe_imbalance"]),
        ("bug_analysis_provenance_real_data",         "bug_analysis_provenance_data",         "bug_analysis",
         &["chunk_oob","q4_sign","kv_misalign","sink_scale","mask_skip","arena_leak","dtype_cast","flash_oob"]),
        ("daily_history_provenance_real_data",        "daily_history_provenance_data",        "daily_history",
         &["day_kv","day_quant","day_bdp","day_moe","day_proj","day_calib","day_bugfix","day_batch"]),
        ("dream_log_provenance_real_data",            "dream_log_provenance_data",            "dream_log",
         &["dream_distrib","dream_neural","dream_stream","dream_sinks","dream_prefetch","dream_cluster","dream_dynwin","dream_fedkv"]),
    ];
    ...
    let weight_configs: &[(&str, DepthWeights)] = &[
        ("equal",      DepthWeights { syntactic: 1.0, semantic: 1.0, pragmatic: 1.0 }),
        ("syn_only",   DepthWeights { syntactic: 1.0, semantic: 0.0, pragmatic: 0.0 }),
        ("sem_only",   DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 0.0 }),
        ("prag_only",  DepthWeights { syntactic: 0.0, semantic: 0.0, pragmatic: 1.0 }),
        ("syn+sem",    DepthWeights { syntactic: 1.0, semantic: 1.0, pragmatic: 0.0 }),
        ("syn+prag",   DepthWeights { syntactic: 1.0, semantic: 0.0, pragmatic: 1.0 }),
        ("sem+prag",   DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 1.0 }),
        ("prag_heavy", DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 2.0 }),
    ];
    ...
    for &(real_dir, synth_dir, display, items) in LAYERS {
        ...
        let section_ids: Vec<SectionId> = (1u32..=items.len() as u32)
            .map(SectionId::new)
            .collect();
        let item_to_sid: HashMap<&str, SectionId> = items.iter().copied()
            .zip(section_ids.iter().copied())
            .collect();

        let probes: Vec<&crate::corpus::LayerScenario> = manifest.scenarios.iter()
            .filter(|s| {
                s.item.is_some()
                    && matches!(s.case_type, LayerCaseType::Positive | LayerCaseType::Boundary)
            })
            .collect();
        ...
        for probe in &probes {
            ...
            let corpus: Vec<(SectionId, Vec<SigEntry>)> = items.iter().copied()
                .zip(section_ids.iter().copied())
                .map(|(item, sid)| {
                    let entries: Vec<SigEntry> = manifest.scenarios.iter()
                        .filter(|s| {
            ...
            let scores_by_sid: HashMap<SectionId, _> = scanner.section_scores()
                .iter()
                .map(|(&sid, &s)| (sid, s))
                .collect();
            probe_score_cache.push(ProbeScores { probe_sid, scores_by_sid });
        }
        ...
        for &(wname, weights) in configs_to_sweep {
            ...
            for ps in &probe_score_cache {
                for (&sid, &s) in &ps.scores_by_sid {
                    ...
                    if sid == ps.probe_sid { intra.push(score); } else { inter.push(score); }
                }
            }
            ...
            let ratio = if mean_inter > 0.0 { mean_intra / mean_inter } else { 1.0 };
            ...
            configs.push(ConfigResult {
                ...
                p10_intra, p90_inter, rob_gap,
                threshold, tp, fp,
            });
        }

        // Choose: highest ratio; tie-break by number of active bands (more = better).
        let chosen = configs.iter().enumerate()
            .max_by(|(_, a), (_, b)| {
                a.ratio.partial_cmp(&b.ratio)
        ...
        println!("\n── {display} {} ──", if is_real { "(real)" } else { "(synth)" });
        println!("  {:<12} {:>7} {:>11} {:>11} {:>10} {:>10} {:>10} {:>10} {:>5} {:>5}",
            "weights", "ratio", "mean_intra", "mean_inter",
            "p10_intra", "p90_inter", "rob_gap", "threshold", "TP%", "FP%");
        println!("  {}", "-".repeat(103));
        ...
        let (yn, ye, yp) = if total > 0.0 {
            (ch.weights.syntactic / total, ch.weights.semantic / total, ch.weights.pragmatic / total)
        } else {
            (0.333_f32, 0.333_f32, 0.334_f32)
        };
        println!("  → chosen: {}  threshold={:.2}  syn:{:.3} sem:{:.3} prag:{:.3}",
    ...
    for r in &layer_results {
        ...
        let (yn, ye, yp) = if total > 0.0 {
            (c.weights.syntactic / total, c.weights.semantic / total, c.weights.pragmatic / total)
        } else {
            (0.333_f32, 0.333_f32, 0.334_f32)
        };
        println!("  {:<26}  {:<12} {:>7.3} {:>10.2} {:>4.0}% {:>4.0}%  {:.3}/{:.3}/{:.3}  ({})",
            r.name, c.name, c.ratio, c.threshold,
            c.tp * 100.0, c.fp * 100.0,
            yn, ye, yp, src);
    }
    println!("{}", "=".repeat(110));
...
fn calibrated_threshold_derivation() {
    let layers: &[(&str, &str, &str, &[&str], DepthWeights)] = &[
        ("code_reading_provenance_real_data", "code_reading_provenance_data", "code_reading",
         &["decode_step","kv_arena","attention_fwd","quant_block","bdp_scan","moe_route","prefill_run","rope_enc"],
         DepthWeights { syntactic: 0.0, semantic: 3.0, pragmatic: 4.0 }),
        ("static_analysis_provenance_real_data", "static_analysis_provenance_data", "static_analysis",
         &["cache_rs","arena_rs","compress_rs","scan_rs","scheduler_rs","projection_rs","engine_rs","config_rs"],
         DepthWeights { syntactic: 1.0, semantic: 0.0, pragmatic: 0.0 }),
        ("dependency_analysis_provenance_real_data", "dependency_analysis_provenance_data", "dependency_analysis",
         &["cache_deps","arena_deps","compress_deps","scan_deps","scheduler_deps","projection_deps","engine_deps","config_deps"],
         DepthWeights { syntactic: 1.0, semantic: 0.0, pragmatic: 3.0 }),
        ("architectural_analysis_provenance_real_data", "architectural_analysis_provenance_data", "architectural_analysis",
         &["paged_kv","quant_policy","bdp_retrieval","moe_predict","wave_batch","three_tier","o1_theorem","proj_schema"],
         DepthWeights { syntactic: 3.0, semantic: 2.0, pragmatic: 0.0 }),
        ("critical_analysis_provenance_real_data", "critical_analysis_provenance_data", "critical_analysis",
         &["kv_frag","quant_drift","bdp_collision","sched_block","mem_pressure","dtype_mismatch","attn_overflow","moe_imbalance"],
         DepthWeights { syntactic: 0.0, semantic: 1.0, pragmatic: 0.0 }),
        ("bug_analysis_provenance_real_data", "bug_analysis_provenance_data", "bug_analysis",
         &["chunk_oob","q4_sign","kv_misalign","sink_scale","mask_skip","arena_leak","dtype_cast","flash_oob"],
         DepthWeights { syntactic: 1.0, semantic: 0.0, pragmatic: 0.0 }),
        ("daily_history_provenance_real_data", "daily_history_provenance_data", "daily_history",
         &["day_kv","day_quant","day_bdp","day_moe","day_proj","day_calib","day_bugfix","day_batch"],
         DepthWeights { syntactic: 1.0, semantic: 3.0, pragmatic: 0.0 }),
        ("dream_log_provenance_real_data", "dream_log_provenance_data", "dream_log",
         &["dream_distrib","dream_neural","dream_stream","dream_sinks","dream_prefetch","dream_cluster","dream_dynwin","dream_fedkv"],
         DepthWeights { syntactic: 3.0, semantic: 1.0, pragmatic: 2.0 }),
    ];
    ...
    for &(real_dir, synth_dir, display, items, weights) in layers {
        let real_path = std::path::PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/"))
            .join(real_dir).join("MANIFEST.json");
        let (dir, is_real) = if real_path.exists() { (real_dir, true) } else { (synth_dir, false) };
        let (manifest, pf) = load_layer_fixtures(dir);

        let section_ids: Vec<SectionId> = (1u32..=items.len() as u32).map(SectionId::new).collect();
        let item_to_sid: HashMap<&str, SectionId> = items.iter().copied()
            .zip(section_ids.iter().copied()).collect();
        ...
        for probe in &probes {
            ...
            let mut scanner = BdpScanner::new();
            scanner.scan_sections(&pf, &probe_syn, &probe_sem, &probe_prag, &corpus)
                .expect("scan_sections failed");

            for (&sid, &s) in scanner.section_scores().iter() {
                let score = weights.combine(
                    s.syn.pick(formula), s.sem.pick(formula), s.prag.pick(formula));
                if sid == probe_sid { intra.push(score); } else { inter.push(score); }
            }
        }
        ...
        println!("  mean   {:>10.1}  {:>10.1}  ratio={:.3}", mean_intra, mean_inter, ratio);

        summary.push(LayerThreshold {
            name: display.to_string(), threshold, mean_intra, mean_inter, ratio, tp, fp, is_real,
        });
    }
    ...
    println!("{:<28} {:>10} {:>10} {:>10} {:>7} {:>7} {:>7}",
        "layer", "threshold", "mean_intra", "mean_inter", "ratio", "TP%", "FP%");
    println!("{dash}");
    for lt in &summary {
        let src = if lt.is_real { "real" } else { "synth" };
        println!("  {:<26}  {:>10.2}  {:>10.1}  {:>10.1}  {:>7.3}  {:>5.0}%  {:>5.0}%  ({})",
            lt.name, lt.threshold, lt.mean_intra, lt.mean_inter,
            lt.ratio, lt.tp * 100.0, lt.fp * 100.0, src);
    }
    println!("{sep}");
    ...
    for lt in &summary {
        println!("  {:<30}  score_threshold: {:>10.2}   # TP={:.0}% FP={:.0}% (calibrated 2026-05-16)",
            lt.name, lt.threshold, lt.tp * 100.0, lt.fp * 100.0);
    }
    println!();
}
...
fn cross_corpus_provenance_sweep() {
    ...
    for &(real_dir, synth_dir, _, _) in LAYERS {
        let real_path = std::path::PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/"))
            .join(real_dir).join("MANIFEST.json");
        let dir = if real_path.exists() { real_dir } else { synth_dir };
        layer_data.push(load_layer_fixtures(dir));
    }

    // ── Build per-layer corpus ───────────────────────────────────────────────
    // Global SectionId: layer_idx * 8 + item_idx + 1  (1-based, 1..=64)
    let corpus_per_layer: Vec<Vec<(SectionId, Vec<SigEntry>)>> = LAYERS.iter().enumerate()
        .map(|(li, &(_, _, _, items))| {
        ...
        for (ii, &item) in items.iter().enumerate() {
            ...
            for s in layer_data[li].0.scenarios.iter().filter(|s| {
                ...
                probes.push(ProbeDef {
                    layer_idx: li,
                    probe_sid,
                    sig_entry: SigEntry { byte_offset: s.byte_offset, token_count: s.token_count },
                });
            }
        }
    }
    ...
    for (pi, probe) in probes.iter().enumerate() {
        if pi % 100 == 0 { eprintln!("  probe {pi}/{n_probes}"); }

        let (probe_syn, probe_sem, probe_prag) = layer_data[probe.layer_idx].1
        ...
        for (li, corpus) in corpus_per_layer.iter().enumerate() {
            scanner
                .scan_sections(&layer_data[li].1, &probe_syn, &probe_sem, &probe_prag, corpus)
                .expect("scan_sections failed");
            for (&sid, &s) in scanner.section_scores().iter() {
                scores.insert(sid, s);
            }
        }
        cached.push(CachedScores { layer_idx: probe.layer_idx, probe_sid: probe.probe_sid, scores });
    }
    eprintln!("[cross_corpus] scanning complete; sweeping {} weight configs...", {
        ...
        for a in 0u32..=4 {
            for b in 0u32..=4 {
                for c in 0u32..=4 {
                    ...
                    if seen.insert(key) {
                        cfgs.push(DepthWeights { syntactic: a as f32, semantic: b as f32, pragmatic: c as f32 });
                    }
                }
            }
        }
    ...
    for &weights in &weight_configs {
        ...
        for cs in &cached {
            let mut scored: Vec<(SectionId, f32)> = cs.scores.iter()
                .map(|(&sid, &s)| {
                    let score = weights.combine(
                        s.syn.pick(formula), s.sem.pick(formula), s.prag.pick(formula));
                    (sid, score)
    ...
    results.sort_unstable_by(|a, b|
        b.total_mrr.partial_cmp(&a.total_mrr).unwrap_or(std::cmp::Ordering::Equal));

    fn w_label(w: &DepthWeights) -> String {
        let t = w.syntactic + w.semantic + w.pragmatic;
        if t <= 0.0 { return "0/0/0".into(); }
        format!("{:.2}/{:.2}/{:.2}", w.syntactic / t, w.semantic / t, w.pragmatic / t)
    }

    // Abbreviate layer names to 9 chars for table columns.
    let col: Vec<&str> = LAYERS.iter().map(|&(_, _, d, _)| &d[..d.len().min(9)]).collect();
    ...
    println!("MRR = Mean Reciprocal Rank (1/rank).  Rank 1 = probe's item is top result out of 64.");
    println!("{sep}");

    print!("{:<16}", "syn/sem/prag");
    for c in &col { print!("  {:>9}", c); }
    println!("  {:>9}", "TOTAL");
    println!("{dash}");

    for r in &results {
        print!("{:<16}", w_label(&r.weights));
        for &m in &r.per_layer_mrr { print!("  {:>9.4}", m); }
    ...
    print!("{:<16}", "syn/sem/prag");
    for c in &col { print!("  {:>8}", c); }
    println!("  {:>8}", "TOTAL");
    println!("{}", "-".repeat(110));

    for r in &results {
        print!("{:<16}", w_label(&r.weights));
        for &t in &r.per_layer_top1 { print!("  {:>7.1}%", t * 100.0); }
