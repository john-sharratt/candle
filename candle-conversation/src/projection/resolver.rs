    ...
    pub fn score_belief_groups(
        ...
        for group in &layer.groups {
            ...
            if group.policy.scan.member_normalization && per_file_fused.len() > 1 {
                ...
                let normed_members = {
                    let mut cache = self.normalization.lock().unwrap();
                    // The peaks are already band values — each folder's turns
                    // were normalized on their own probe lengths above — so the
                    // member pass states them at the reference length itself,
                    // where the rate conversion is the identity. Passing a probe
                    // length here would rescale scores that carry none.
                    let band_tokens = cache.config().probe_t_ref;
                    let normed = cache.normalize(&member_scope, &peaks, band_tokens);
                    if let (true, Some(source)) =
                        (observe.teaches(&group.policy.tags), observe.source())
                    {
                        cache.observe(&member_scope, source, &peaks, band_tokens);
                    }
                    normed
                };
