...
impl ToolBelief {
    ...
    ///
    /// Deliberately **unbounded**. The accumulator is a contraction — for the
    /// leader the step is `s' = (1-β)s + fresh`, settling at `fresh/β` rather
    /// than running away — but `fresh` is a normalized score with no ceiling of
    /// its own (a lock-on "rides above" the 0–1000 band by design), so the
    /// settled value tracks the input: ~82,850 per projection settles near
    /// ~207,000.
    ///
    /// **It must not be clamped here.** Clamping the accumulator erases the
    /// margin the selector ranks on: with several tools past a 5,000 ceiling,
    /// `datetime` (143,035) and `aead_encrypt` (461) both read 5,000, tied, and
    /// the budget's three seats went in arbitrary order — a live "what time is
    /// it?" turn then answered "there's no clock tool available in my available
    /// tools", because the tool that would have answered it lost a coin-flip.
    ///
    /// Nor can a bound sit anywhere a score is read BACK: `PriorBelief::
    /// from_selection` seeds the next reprojection's RelLeak from the persisted
    /// selection's score, so a clamp upstream of the readout re-enters the
    /// accumulator one projection later and the ties return. The only bound is
    /// on the tile a human reads — zend's `ProjectTile::SCORE_CAP`.
...
mod tests {
    ...
    ///
    /// A belief reported in production as climbing 29,100 → 100,309 → 143,035
    /// was converging on `fresh/β`, not diverging; this pins that it cannot do
    /// otherwise, at a magnitude small enough to read the bound off directly.
    #[test]
    ...
    /// **The accumulator keeps its margin however far past a readout bound both
    /// beliefs ride.**
    ///
    /// Clamping inside `update` tied `datetime` (143,035) with `aead_encrypt`
    /// (461) at 5,000 apiece, and a live turn then answered "there's no clock
    /// tool available" because the seats went in arbitrary order. Two beliefs an
    /// order of magnitude apart in evidence, both past the 5,000 the readout is
    /// bounded at, must still rank.
    #[test]
    fn a_lock_on_keeps_its_margin_past_the_readout_bound() {
        const READOUT_BOUND: f32 = 5_000.0;
        ...
        assert!(
            strong > READOUT_BOUND && weak > READOUT_BOUND,
            "both should exceed the readout bound for this test to mean anything: \
             {strong} / {weak}"
        );
        assert!(
            strong > weak * 2.0,
