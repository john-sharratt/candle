...
| **Virtual filesystem** | `file_{write,read,edit,list,delete,present}` | Web chat only | Per-session in-memory; 10 MiB cap; `file_read` returns the whole file unless given `start_line` / `end_line` |
