...
pub struct ListRequest {
    /// Directory to list, e.g. `src/util`. Defaults to "" — the project root.
    /// Must name a real directory (or the root); a file path does not resolve.
...
impl Tool for FileList {
    ...
    const DESCRIPTION: &'static str =
        "List one directory's immediate contents, as visible to this session: \
         the project's working directory plus anything written or edited during \
         the session, which shadows the file of the same path on disk. Omit \
         `path` to list the project root; give a directory path to list inside \
         it. A listing is one level deep — a subdirectory appears as its own \
