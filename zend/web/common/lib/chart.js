...
/* Series colours arrive as custom properties — `color: 'var(--l-action)'` — so a
 * chart follows the theme without every caller hard-coding hex.
 *
 * A canvas cannot read one. Assigning `var(--x)` to `strokeStyle` is an invalid
 * colour, and the spec says an invalid assignment is IGNORED rather than
 * throwing: the context keeps whatever was set last, which here is the gridline
 * grey. Every series on every chart therefore drew in the same faint grey, and
 * silently — there is no error to notice.
 *
 * So resolve on the way in. Cached, because `getComputedStyle` is not free and
 * a hovered chart redraws per frame; keyed by theme, because the whole point of
 * a custom property is that its value changes when the theme does. */
...
function resolve(c) {
  ...
  if (!m) return c;
  const key = themeKey() + '|' + c;
  if (!COLORS.has(key)) COLORS.set(key, cssvar(m[1]) || '#888888');
  return COLORS.get(key);
}

/* The same colour at a given alpha, for area fills. Every theme token is
 * `#rrggbb`, so appending two hex digits is exact; anything else falls back to
 * the solid colour rather than producing an invalid string that would be
 * ignored the same silent way. */
function withAlpha(c, aa) {
  const v = resolve(c);
  if (/^#[0-9a-f]{6}$/i.test(v)) return v + aa;
  if (/^#[0-9a-f]{3}$/i.test(v)) return '#' + v[1] + v[1] + v[2] + v[2] + v[3] + v[3] + aa;
  return v;
}
...
function draw(spec) {
  ...
  if (opts.y2) {
    ...
    for (let i = 0; i <= 4; i++) {
      const yv = y2max * i / 4;
      g.fillStyle = resolve(opts.y2Color) || faint;
  ...
  (opts.markers || []).forEach((m) => {
    if (m.y > ymax) return;
    g.strokeStyle = resolve(m.color); g.lineWidth = 1.2; g.setLineDash([5, 4]);
  ...
  opts.series.filter((s) => s.type === 'bar').forEach((s) => {
    ...
    g.fillStyle = resolve(s.color); g.globalAlpha = 0.85;
  ...
  opts.series.filter((s) => s.type !== 'bar').forEach((s) => {
    ...
    if (s.type === 'area') {
      ...
      grad.addColorStop(0, withAlpha(s.color, '55')); grad.addColorStop(1, withAlpha(s.color, '0a'));
      g.fillStyle = grad; g.fill();
    }
    g.beginPath(); g.moveTo(X(xs[0]), yf(s.data[0]));
    for (let i = 1; i < xs.length; i++) g.lineTo(X(xs[i]), yf(s.data[i]));
    g.strokeStyle = resolve(s.color); g.lineWidth = s.lw || 1.8; g.lineJoin = 'round';
  ...
  if (spec._mx != null && spec._mx >= padL && spec._mx <= padL + plotW && opts.tip) {
    ...
    rows.forEach((r, i) => {
      const yy = ty + 11 + i * 15;
      if (r.c) { g.fillStyle = resolve(r.c); g.fillRect(tx + 9, yy - 3, 7, 3); }
      g.fillStyle = resolve(r.c) || ink;
      g.fillText(r.t, tx + (r.c ? 22 : 9), yy);
    });
    opts.series.forEach((s) => {
      const yf = s.axis === 'r' ? Y2 : Y;
      g.fillStyle = resolve(s.color);
...
function drawStack(spec) {
  ...
  bars.forEach((b, i) => {
    ...
    keys.forEach((k) => {
      ...
      g.fillStyle = resolve(opts.color(k)); g.globalAlpha = 0.92;
  ...
  if (spec._mx != null && opts.tip) {
    ...
    if (idx >= 0 && best < (plotW / n) * 0.9) {
      ...
      rows.forEach((r, i) => {
        const yy = ty + 11 + i * 15;
        if (r.c) { g.fillStyle = resolve(r.c); g.fillRect(tx + 9, yy - 3, 7, 3); }
        g.fillStyle = resolve(r.c) || cssvar('--ink');
        g.fillText(r.t, tx + (r.c ? 22 : 9), yy);
      });
    }
  }
