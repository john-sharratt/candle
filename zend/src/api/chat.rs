...
pub async fn completions(
    ...
    // The composer dials drive the dialogue layer's section-tree selectors —
    // the projection emits the matching thinking-effort / response-length
    // directive sections (and the `/no_think` node) on this and every
    // subsequent turn until the dials change.
    let selection = dial_selection(req.effort, req.verbosity, req.think);
    let messages = req.messages;
    if req.stream {
        stream_sse(
            session, messages, max_tokens, conv_id, model, id, created, selection,
        )
        .await
    } else {
        collect_completion(
...
async fn stream_sse(
    ...
    let token_stream = session
        .submit(messages, max_tokens, conv_id, selection)
        .await;
