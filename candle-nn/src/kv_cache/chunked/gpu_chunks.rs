...
impl GpuChunksGuard<'_> {
    ...
    /// Diagnostic: re-serialize every chunk's header + record from the CURRENT
    /// host windows and a FRESH arena snapshot, and byte-compare against the
    /// cached buffer the kernel actually reads. `None` = cache consistent; a
    /// report names each stale chunk with the cached-vs-fresh bytes at the
    /// first differing u64 — the embedded pointer the kernel would deref.
    pub(crate) fn audit_against(
        ...
        let cbs = token_slice_serialized_size(n_kv_head, head_dim);
        if self.inner.chunk_byte_size != cbs || cbs == 0 {
            return None;
        }
        let n = self.inner.buf.len() / cbs;
        if n == 0 {
            return None;
        }
        let rec_bytes = record_bytes(n_kv_head, head_dim);
        ...
        for (i, cw) in chunks.iter().take(n).enumerate() {
            ...
            match resident {
                Some(addr) => {
                    if got_ptr != addr {
                        report.push_str(&format!(
                            "chunk {i}: kvheads {got_ptr:#x} != resident meta {addr:#x}; "
                        ));
                    }
                }
                None => {
                    let inline = base + (records_off + i * rec_bytes) as u64;
                    if got_ptr != inline {
                        report.push_str(&format!(
                            "chunk {i}: kvheads {got_ptr:#x} != inline {inline:#x}; "
                        ));
                        continue;
                    }
                    scratch.fill(0);
                    write_record_for_chunk(&mut scratch, cw, n_kv_head, head_dim, arena_info);
                    let r0 = records_off + i * rec_bytes;
                    let cached = &buf[r0..r0 + rec_bytes];
                    if cached != scratch.as_slice() {
                        for off in (0..rec_bytes).step_by(8) {
                            let end = (off + 8).min(rec_bytes);
                            if cached[off..end] != scratch[off..end] {
                                let a = u64::from_le_bytes(
                                    cached[off..off + 8].try_into().unwrap_or([0; 8]),
                                );
                                let b = u64::from_le_bytes(
                                    scratch[off..off + 8].try_into().unwrap_or([0; 8]),
                                );
                                report.push_str(&format!(
                                    "chunk {i}: record byte {off} cached {a:#x} fresh {b:#x}; "
                                ));
                                break;
                            }
                        }
