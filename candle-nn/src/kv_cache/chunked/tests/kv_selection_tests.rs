...
//!
//! These tests load the binary dump produced by
//! `quantized_llama::tests::test_dump_kv_cache_data` and run a CPU
//! implementation of the per-block cosine-distance format-selection algorithm.
//!
//! The dump file lives at:
//!   `candle-nn/src/kv_cache/chunked/tests/data/qwen3-kv-data.bin`
//!
//! If the file is absent the tests are silently skipped, so they never block
//! a clean-room CI build.  To produce the file, run:
//!
...
use crate::kv_cache::chunked::sampled_selection::{
    cpu_palette4_reduce, cpu_parallel_kernel_map, cpu_parallel_kernel_range, qrel_threshold,
    SampleFormat, DEFAULT_CALIBRATION_ARENA_CHUNKS, DEFAULT_REPORT_ARENA_CHUNKS, ERROR_MARGIN_ABS,
...
/// Path to the pre-generated binary dump, relative to this crate's root.
...
const CANDIDATES: &[(BlockFormat, fn(&[f32; 32]) -> [f32; 32])] = &[
    ...
    (BlockFormat::Q0_M, round_trip_q1_a),
...
/// Return path to the dump file, or None if absent.
...
fn block_format_from_sample(fmt: SampleFormat) -> BlockFormat {
    match fmt {
        ...
        SampleFormat::Q1_A => BlockFormat::Q1_A,
...
fn test_candidate_list_compression_curve() {
    ...
    let (header, mut chunks) = load_dump(&path).expect("failed to load dump");
    ...
    let v_profile_start = Instant::now();

    // ── Profile V cosine distance CDF per format ──
    // Compute cosine distance for each V block × each V format to derive
    // data-driven thresholds (matching test_qproj_k_format_selection approach).
    let v_profile_fmts = [
        BlockFormat::Q2_0,
        BlockFormat::Q3_0,
        BlockFormat::Q4_0,
    ...
    let _v_n_fmts = v_profile_fmts.len();
    let mut v_errors_by_fmt: Vec<Vec<f32>> = cpu_parallel_kernel_map(&v_profile_fmts, |fmt| {
        ...
        for chunk in &chunks {
            for b in 0..num_blocks {
                ...
                if end <= chunk.v.len() {
                    let blk: [f32; SELECT_BLOCK] = chunk.v[start..end].try_into().unwrap();
                    let recon = fmt.apply_quant(&blk);
                    let dist = cosine_distance(&blk, &recon);
                    errs.push(dist);
                }
            }
        }
    ...
    for errs in v_errors_by_fmt.iter_mut() {
        errs.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    }
    let v_fmt_idx =
        |f: BlockFormat| -> usize { v_profile_fmts.iter().position(|&x| x == f).unwrap() };
    let v_pctile = |fmt: BlockFormat, p: f64| -> f32 {
        let e = &v_errors_by_fmt[v_fmt_idx(fmt)];
        if e.is_empty() {
            return 0.0;
        }
        let idx = ((e.len() as f64 * p) as usize).min(e.len() - 1);
        e[idx]
    };

    // Print V CDF for key formats
    println!(
        "V cosine distance CDF (from {} blocks):",
        v_errors_by_fmt[0].len()
    );
    for &fmt in &[
        BlockFormat::Q2_0,
        BlockFormat::Q3_0,
        ...
        println!(
            "  {:<5}: p05={:.6} p10={:.6} p25={:.6} p50={:.6} p75={:.6} p90={:.6} p95={:.6}",
            fmt,
            v_pctile(fmt, 0.05),
            v_pctile(fmt, 0.10),
            v_pctile(fmt, 0.25),
    ...
    // Use production V cosine thresholds from chunk_ops.rs (KV_SELECT_V_COSINE_THRESHOLDS).
    // The auto-derived approach from format error CDF percentiles produces thresholds that
    // are too tight: sink-protected chunks (threshold/8) end up with nearly all blocks at F16,
    // poisoning entire heads via worst-case-per-head reduction.  Production thresholds are
    // tuned so that even sink chunks mostly avoid F16.
    let v_auto_thresholds = V_COSINE_THRESHOLDS_PROPOSED;
    println!("V cosine thresholds (production):");
    for i in 0..11 {
        println!("  C{i}: {:.6}", v_auto_thresholds[i]);
    }

    let v_profile_time = v_profile_start.elapsed();
    ...
    let sealed_chunks = chunks.len() / header.num_layers; // chunks per head per layer
    let total_chunks = sealed_chunks + 1; // +1 for unsealed tail
    let sealed_frac = sealed_chunks as f64 / total_chunks as f64;
    let unsealed_frac = 1.0 / total_chunks as f64;
    ...
    let levels: Vec<LevelResult> = cpu_parallel_kernel_range(11, |i| {
            ...
            gpu_quality_agg.as_ref()
        {
            ...
            let v_sample: Vec<SampleFormat> = v_fmts
                ...
                .map(sample_format_from_block)
                .collect();
            let (cpu_k_pal4_bpe, _) = cpu_palette4_reduce(&k_sample, blocks_per_head);
            let (cpu_v_pal4_bpe, _) = cpu_palette4_reduce(&v_sample, blocks_per_head);
            assert!(
        ...
        let k_bpe = sealed_frac * k_quant_bpe + unsealed_frac * 16.0;
        let v_bpe = sealed_frac * v_quant_bpe + unsealed_frac * 16.0;
        let cr = 16.0 / ((k_bpe + v_bpe) / 2.0);
        let k_bpe_p4 = sealed_frac * k_pal4_bpe + unsealed_frac * 16.0;
        let v_bpe_p4 = sealed_frac * v_pal4_bpe + unsealed_frac * 16.0;
    ...
    let fmt_pct = |counts: &[usize; 16], total: usize, fmt: BlockFormat| {
        if total == 0 {
            format!("{:>5}", "-")
        } else {
            format!(
    ...
    println!("  Per-head K format distribution (%) for each compression mode, with fixed H/L thresholds and SNR.");
    println!();
    let k_grid_formats = [
        BlockFormat::F16,
        BlockFormat::Q8_KS,
    ...
    let mut hk = "  Mode | Tier     |   K_hi   |   K_lo   | H_SNR | L_SNR |   CR  |".to_string();
    ...
    for i in 0..11 {
        ...
        for fmt in &k_grid_formats {
            line.push_str(&format!(
                " {}",
                fmt_pct(&levels[i].k_head_dist, levels[i].k_total_heads, *fmt)
            ));
        }
        println!("{line}");
    }
    ...
    println!("  Per-head V format distribution (%) for each compression mode, with fixed H/L thresholds and SNR.");
    println!();
    let v_grid_formats = [
        BlockFormat::F16,
        BlockFormat::Q8_0,
    ...
    let mut hv = "  Mode | Tier     |   V_hi   |   V_lo   | H_SNR | L_SNR |   CR  |".to_string();
    ...
    for i in 0..11 {
        ...
        for fmt in &v_grid_formats {
            line.push_str(&format!(
                " {}",
                fmt_pct(&levels[i].v_head_dist, levels[i].v_total_heads, *fmt)
            ));
        }
        println!("{line}");
    }
    ...
    // ── Table 2: Quality metrics ──
    println!("\n{sep80}");
    println!("Table 2 -- Quality metrics (sink-aware, all blocks)");
    println!(
        "  SNR_dB  = 10*log10(sig/noise)  NRMSE = sqrt(noise/sig)  cos_p95 = 95th pct cosine dist"
    );
    println!();
    let h2 = "  Mode  Tier        K_SNR     K_NRMSE  K_cos_p95    V_SNR     V_NRMSE  V_cos_p95    head_CR  CR_step";
    println!("{h2}");
    println!("  {}", "-".repeat(h2.len() - 2));
    let mut prev_cr = 1.0f64;
    for i in 0..11 {
        let c = ph_cr(i);
        let step = c - prev_cr;
        let snr_fmt = |x: f64| {
        ...
        println!(
            "  C{i}  {:<9}  {} {:>9.5} {:>11.7}   {} {:>9.5} {:>11.7}  {:>6.2}x  {:>+6.2}x",
            LEVEL_TIER[i],
            snr_fmt(k(i).snr_db()),
            k(i).nrmse(),
            k(i).p95(),
    ...
    for i in 0..11 {
        ...
        println!(
            "  C{i:<3}  {:<9} {:>5.2}x   {:>5.2}x     {:>5.2}x   {:>8.3}  {:>8.3}  {:>8.3}  {:>8.3}  {:>5.1}%",
            LEVEL_TIER[i],
            cr(i),
            ph_cr(i),
            levels[i].cr_p4,
    ...
    for i in 0..11 {
        let lv = &levels[i];
        println!(
            "  C{i:<3}  {:<9} {:>6.1}  {:>6.1}  {:>6.1}   {:>6.1}  {:>6.1}  {:>6.1}   {:>.2e}  {:>.2e}  {:>.2e}  {:>.2e}  {:>.2e}  {:>.2e}",
            LEVEL_TIER[i],
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        println!("  GPU diagnostic check (3 levels)...");
        ...
        for &level in &diag_levels {
            ...
            let (gpu_k_bpe, gpu_v_bpe) = run_gpu_bpe(
                level,
                K_MAGWEIGHT_THRESHOLDS[level],
                v_auto_thresholds[level],
            );
...
fn test_qproj_k_format_selection() {
    ...
    // ── V cosine distance profiling per format ──────────────────────────────
    // Profile V sub-blocks across all formats to get empirical error CDFs,
    // then auto-derive V thresholds the same way we do for K.
    println!("\nProfiling V cosine distance across all V blocks and formats...");

    // V candidate formats (subset — skip F16 since V uses BF16 as max)
    let v_profile_fmts = [
        BlockFormat::Q2_0,
        BlockFormat::Q3_0,
        BlockFormat::Q4_0,
    ...
    let v_n_fmts = v_profile_fmts.len();
    let mut v_errors_by_fmt: Vec<Vec<f32>> = vec![Vec::new(); v_n_fmts];
        ...
        for h in 0..n_kv_head {
            for t in 0..chunk_size {
                ...
                for b in 0..num_sub_blocks {
                    ...
                    for (fi, &fmt) in v_profile_fmts.iter().enumerate() {
                        let recon = fmt.apply_quant(&blk);
                        let dist = cosine_distance(&blk, &recon);
                        v_errors_by_fmt[fi].push(dist);
                    }
                }
            }
        }
    }
    let v_n_blocks = v_errors_by_fmt[0].len();
    println!(
        "Profiled {} V sub-blocks × {} formats = {} measurements.",
        v_n_blocks,
        v_n_fmts,
        v_n_blocks * v_n_fmts
    );

    // Sort V errors for percentile computation.
    for errs in v_errors_by_fmt.iter_mut() {
        errs.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    }

    // ── Table A-V: V cosine distance percentiles per format ─────────────────
    let pctile_v = |v: &[f32], p: f64| -> f64 {
        if v.is_empty() {
            return 0.0;
        }
        let idx = ((v.len() as f64 * p) as usize).min(v.len() - 1);
    ...
    println!(
        "\n--- V cosine distance percentiles per format ({} blocks) ---",
        v_n_blocks
    );
    println!(
        "  {:<6}  {:>5}  {:>10}  {:>10}  {:>10}  {:>10}  {:>10}  {:>10}  {:>10}",
        "Format", "BPE", "p25", "p50", "p75", "p90", "p95", "p99", "max"
    );
    println!("  {}", "-".repeat(100));
    for (fi, &fmt) in v_profile_fmts.iter().enumerate() {
        let e = &v_errors_by_fmt[fi];
        println!("  {:<6}  {:>5.1}  {:>10.6}  {:>10.6}  {:>10.6}  {:>10.6}  {:>10.6}  {:>10.6}  {:>10.6}",
            fmt, fmt.bits_per_elem(),
            pctile_v(e, 0.25), pctile_v(e, 0.50), pctile_v(e, 0.75),
            pctile_v(e, 0.90), pctile_v(e, 0.95), pctile_v(e, 0.99),
            *e.last().unwrap_or(&0.0) as f64);
    }

    // Auto-derive V cosine distance thresholds from CDFs.
    // Strategy: match K's CDF approach — anchor to format error percentiles.
    let v_fmt_idx =
        |f: BlockFormat| -> usize { v_profile_fmts.iter().position(|&x| x == f).unwrap() };
    let v_auto_thresholds: [f32; 10] = [
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q8_0)], 0.05) as f32, // C0 — Q8 p05
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q8_0)], 0.50) as f32, // C1 — Q8 p50
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q4_1)], 0.25) as f32, // C2 — Q4_1 p25
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q4_0)], 0.25) as f32, // C3 — Q4_0 p25
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q4_0)], 0.50) as f32, // C4 — Q4_0 p50
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q3_0)], 0.25) as f32, // C5 — Q3_0 p25
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q3_0)], 0.50) as f32, // C6 — Q3_0 p50
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q3_0)], 0.50) as f32, // C7 — Q3_0 p50 (same primary, Q2 promo)
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q3_0)], 0.75) as f32, // C8 — Q3_0 p75 (Q2 promo active)
        pctile_v(&v_errors_by_fmt[v_fmt_idx(BlockFormat::Q3_0)], 0.90) as f32, // C9 — Q3_0 p90 (Q2 promo active)
    ];
    let v_derivation = [
        "Q8_0 p05", "Q8_0 p50", "Q4_1 p25", "Q4_0 p25", "Q4_0 p50", "Q3_0 p25", "Q3_0 p50",
        "Q3_0 p50", "Q3_0 p75", "Q3_0 p90",
    ];
    println!("\nAuto-derived V cosine distance thresholds:");
    println!("  {:<5}  {:>12}  {}", "Level", "Primary", "Derivation");
    println!("  {}", "-".repeat(60));
    for i in 0..10 {
        println!(
            "  C{i}     {:>12.6}  {}",
            v_auto_thresholds[i], v_derivation[i]
        );
    }
