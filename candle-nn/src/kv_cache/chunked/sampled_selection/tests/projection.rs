...
//!
//! These tests load real KV cache dumps and run a CPU implementation of the
//! per-block cosine-distance format-selection algorithm.
//!
//! Data files (both live at candle-nn/src/kv_cache/chunked/tests/data/):
//!   kv_cache_r16_dump.bin — Qwen3-30B-A3B v4 dump (K + V + Q captured in R16 mode).
//!                           Preferred for Q-relevance threshold scaling.
//!   qwen3-kv-data.bin     — Qwen3-30B-A3B v3 dump (K + V only, no Q; fallback).
//!   llama-kv-data.bin     — Llama-3.2-3B  (diversity baseline, sequential tokens).
//!
//! If a file is absent it is silently skipped.  To regenerate:
//!
//!   # v4 R16 dump (preferred — includes Q vectors):
//!   cargo test --release --features cuda --lib --package candle-transformers \
//!     quantized_qwen3_moe::tests::kv_dump::test_dump_r16_kvq_data -- --ignored --nocapture
//!
//!   # v3 float dump (fallback — K + V only):
//!   cargo test --release --features cuda --lib --package candle-transformers \
...
use crate::kv_cache::chunked::sampled_selection::{
    cpu_palette4_reduce, cpu_parallel_kernel_map, cpu_parallel_kernel_range,
    k_threshold_scaled_rust, model_compression_from_surface, sample_error_surface,
    sample_error_surface_cpu, select_smallest_passing, KvErrorThresholdFactors, SampleFormat,
    SampleSide, DEFAULT_CALIBRATION_ARENA_CHUNKS, DEFAULT_REPORT_ARENA_CHUNKS, ERROR_MARGIN_ABS,
    LLAMA_KV_FACTORS, PRODUCTION_K_QREL_HIGH_THRESHOLDS as K_QREL_HIGH_THRESHOLDS,
...
const COSINE_THRESHOLDS: [f32; 11] = [
    0.0001, 0.0002, 0.0005, 0.0010, 0.0020, 0.0050, 0.0100, 0.0200, 0.0500, 0.1000, 0.2000,
];
...
/// v4 R16 dump — includes Q vectors for proper Q-relevance threshold scaling.
const QWEN3_KVQ_DUMP_REL_PATH: &str = "src/kv_cache/chunked/tests/data/kv_cache_r16_dump.bin";
/// v3 float dump — K + V only, used as fallback when the R16 dump is absent.
...
/// Fast-path sample sizes for the heavy offline report tests.
const TABLE_SAMPLE_CHUNKS: usize = 28;
const GPU_MATCH_SAMPLE_CHUNKS: usize = 48;
const QPROJ_SAMPLE_CHUNKS: usize = 48;
const THRESHOLD_SWEEP_CHUNKS: usize = 12;
const THRESHOLD_SWEEP_STEPS: usize = 24;
const THRESHOLD_GRID_STEPS: usize = 8;

#[test]
fn cpu_smoke() {
    let n_batch = 2;
    let n_head = 2;
    let head_dim = 128; // production head_dim; small values make palette overhead dominate
    ...
    let winners = select_smallest_passing(&surface, 0.02, &candidates, None);
    let summary =
        model_compression_from_surface(&surface, &winners, &candidates, None).expect("model");

    assert!(summary.ideal_cr >= summary.palette4_cr);
    assert!(summary.head_cr > 1.0);
    assert!(summary.palette4_cr > 1.0);
}

#[test]
fn cpu_no_pass_uses_least_error_quant_fallback() {
    let mut block = [0.0f32; SELECT_BLOCK];
    for (i, x) in block.iter_mut().enumerate() {
        *x = ((i as f32) * 0.173).sin() * 1.7 + ((i as f32) * 0.071).cos() * 0.35;
    }

    let candidates = [SampleFormat::Q0, SampleFormat::Q8KS];
    let k_fmt = select_best_passing_format(&block, &candidates, -1.0, true);
    let v_fmt = select_best_passing_format(&block, &candidates, -1.0, false);

    assert_eq!(k_fmt, SampleFormat::Q8KS);
    assert_eq!(v_fmt, SampleFormat::Q8KS);
}
...
fn gpu_palette4_reduction_does_not_invent_float_formats_from_uniform_quant_input() {
    ...
    let (_backing, gpu_inputs) = PagedSelectionGpuInputs::from_f32_chunks(
        &chunk_refs,
        &chunk_refs,
        n_head * head_dim,
        n_head,
    ...
    .expect("stage paged GPU inputs");

    let candidates: Vec<SampleFormat> = candidate_formats()
        .into_iter()
    ...
    assert!(
        k_palette_rows
            ...
            .all(|fmt| !fmt.is_float()),
        "K fused selection invented a float format from quant-only candidate list"
    );
    assert!(
        v_palette_rows
            .iter()
            .flat_map(|row| row.iter())
            .all(|fmt| !fmt.is_float()),
        "V fused selection invented a float format from quant-only candidate list"
    );
}
...
fn gpu_no_pass_uses_least_error_quant_fallback() {
    ...
    let candidates = vec![SampleFormat::Q0, SampleFormat::Q8KS];
    // Use 0.0 thresholds: when q_spread=0 (no Q-capture data), the kernel falls back to
    // sqrt(lo * hi) = sqrt(0) = 0.0. Real quantization error is always > 0, so no format
    // passes → fallback = last candidate = Q8KS. (Threshold -1.0 was wrong here because
    // sqrt((-1)*(-1))=1.0 lets Q0 pass at threshold=1.0 when q_spread=0.)
    let (k_formats, v_formats, _k_sidx, _v_sidx, _k_amax, _v_amax) = gpu_inputs
        .select_block_formats(&candidates, &candidates, 0.0, 0.0, 0.0, 0.0)
        .expect("GPU least-error fallback selection");

    assert!(k_formats.iter().all(|fmt| *fmt == SampleFormat::Q8KS));
    assert!(v_formats.iter().all(|fmt| *fmt == SampleFormat::Q8KS));
}
...
fn gpu_quant_only_selector_never_returns_float_formats() {
    ...
    assert!(
        !candidates.is_empty(),
        "quant-only candidate ladder must not be empty"
    );
    ...
    assert!(
        k_palette_rows
            ...
            .flat_map(|row| row.iter())
            .all(|fmt| !fmt.is_float()),
        "K fused selection returned a float format from quant-only candidate list"
    );
    assert!(
        v_palette_rows
            .iter()
            .flat_map(|row| row.iter())
            .all(|fmt| !fmt.is_float()),
        "V fused selection returned a float format from quant-only candidate list"
    );
}
...
// Shared projection tests reuse the central sampled-selection format enum
// and the legacy comparison threshold ladder from the main module.
type BlockFormat = SampleFormat;

#[allow(dead_code)]
fn summarize_top_formats(counts: &[usize; 23], total: usize, top_n: usize) -> String {
    if total == 0 {
        return "-".to_string();
    }
...
fn select_format_with_error(block: &[f32; SELECT_BLOCK], threshold: f32) -> (BlockFormat, f32) {
    ...
    (
        candidates.first().copied().unwrap_or(BlockFormat::Q8_0),
        0.0,
    )
}
...
fn test_kv_format_selection_statistics() {
    ...
    let (header, loaded_chunks) = load_dump(&path).expect("failed to load dump");
    let total_chunks = loaded_chunks.len();
    let chunks = evenly_sampled_chunks(&loaded_chunks, TABLE_SAMPLE_CHUNKS);
    println!(
        "Loaded dump: {} layers, {} kv-heads, chunk_size={}, head_dim={}",
        header.num_layers, header.n_kv_head, header.chunk_size, header.head_dim
    );
    println!(
        "Using representative subset: {} / {} float chunks",
        chunks.len(),
        total_chunks
    );
...
fn test_compression_mode_table() {
    ...
    let level_stats: Vec<(ComponentStats, ComponentStats)> =
        cpu_parallel_kernel_map(&COSINE_THRESHOLDS, |threshold| {
            (
...
/// Returns `(median, spread, is_fallback)` for the relevance signal used during
/// threshold scaling.  `is_fallback = true` when no chunk has Q data and the
/// K-energy proxy is used instead of the real Q·K relevance.  Callers should
/// honour the flag by clamping the IQR multiplier to ≤ 1.0 so that the
/// proxy can only *tighten* the threshold for clearly high-magnitude blocks
/// — never relax it for blocks that look low-magnitude but might still be
/// strongly attended to.
fn compute_q_relevance_stats(
    chunks: &[ChunkData],
    ...
    let n = rels.len();
    let q1 = rels[(n as f64 * 0.25) as usize];
    let median = rels[(n as f64 * 0.50) as usize];
    let q3 = rels[((n as f64 * 0.75) as usize).min(n - 1)];
    let spread = (q3 - q1) * 2.0 / std::f32::consts::LN_2;
    Some((median, spread, is_fallback))
}

/// Accumulated statistics for the proposed-curve test.
#[derive(Default, Clone)]
struct CurveStat {
    counts: [usize; 23], // indexed by BlockFormat::table_index()
    bpe_sum: f64,
    cos_sum: f64,
...
fn test_proposed_compression_curve() {
    ...
    let all: Vec<[CurveStat; 4]> = cpu_parallel_kernel_range(11, |i| {
        curve_level_stats(
            &chunks,
            num_blocks,
...
/// Map a CUDA kernel format tag back to the shared sampled-selection format type.
#[cfg(feature = "cuda")]
fn tag_to_block_format(tag: i32) -> BlockFormat {
    BlockFormat::from_cuda_tag(tag)
}
...
fn ladder_distance(a: BlockFormat, b: BlockFormat) -> usize {
    ...
    let rank = |f: BlockFormat| match f {
        ...
        BlockFormat::Q1_A => 18,
        BlockFormat::Q0_X => 19,
        BlockFormat::Q0_M2 => 20,
        BlockFormat::Q0_M4 => 21,
    };
...
fn test_cuda_selection_matches_cpu() {
    ...
    let path = match dump_path() {
        ...
        None => {
            println!(
                "SKIP: no Qwen3 dump found (tried kv_cache_r16_dump.bin and qwen3-kv-data.bin)"
            );
            return;
        }
    };
    let (header, loaded_chunks) = match load_dump(&path) {
        Some(v) => v,
        None => {
    ...
    let mut cpu_counts = vec![[[0usize; 23]; 2]; 10];
    let mut cuda_counts = vec![[[0usize; 23]; 2]; 10];
...
fn test_cuda_per_head_matches_cpu() {
    ...
    let mut cpu_head_dist = vec![[0usize; 23]; 10];
    let mut gpu_head_dist = vec![[0usize; 23]; 10];
...
/// Extract the b-th 32-element block from a token-major dump as a dim-major
/// block (32 tokens of one channel within a (head, palette) sub-band).
///
/// `dump_sequence_*_chunks` and the v3/v4 dump readers produce
/// `[head][palette][token][sub_dim]` token-major.  The CUDA selection kernel,
/// however, indexes blocks dim-major within each (head, palette) sub-band:
/// each block is 32 tokens of a single channel.  This helper performs the
/// transposition so test-side per-block quality metrics see the same data
/// the kernel selected formats for.
///
/// Block indexing matches the kernel's iteration order — `b` runs
/// 0..(n_kv_head × N_PALETTE × sub_head_dim) = 0..blocks_per_chunk, with
/// inner axis = channel within sub-band, then palette, then head.
#[inline]
...
fn cpu_select_k_qproj(
    ...
    // All-zero or all-positive → keep the cheapest caller-provided candidate.
    ...
    let threshold =
        k_threshold_scaled_rust(threshold_lo, threshold_hi, relevance, q_median, q_spread);
...
fn test_cuda_r16_qproj_matches_cpu() {
    ...
    println!(
        "Loaded v4 dump: {} layers, n_kv_head={}, chunk_size={}, head_dim={}, {} sampled / {} total chunks",
        header.num_layers,
        header.n_kv_head,
        header.chunk_size,
        header.head_dim,
    ...
    println!(
        "\n  {:4}  {:>9}  {:>9}  {:>10}  {:>8}",
        "Mode", "Decisions", "Mismatch", "Mismatch%", "BPE-Tie"
    );
    println!("  {}", "─".repeat(76));
    for level in 0..10 {
        let pct = 100.0 * level_mismatches[level] as f64 / level_total[level].max(1) as f64;
        ...
        println!(
            "  C{level}   {:>8}  {:>8}  {:>9.4}%  {:>7}  {status}",
            level_total[level], level_mismatches[level], pct, level_bpe_ties[level],
        );
        for m in &level_first_mismatches[level] {
            println!("{m}");
        }
...
fn block_format_from_kv(fmt: KvFormat) -> BlockFormat {
    match fmt {
        KvFormat::Float(candle::DType::F16) => BlockFormat::F16,
        KvFormat::Float(candle::DType::BF16) => BlockFormat::BF16,
        KvFormat::Quantized(QuantFormat::Q8_KS) => BlockFormat::Q8_KS,
        KvFormat::Quantized(QuantFormat::Q8_1) => BlockFormat::Q8_1,
        KvFormat::Quantized(QuantFormat::Q8_0) => BlockFormat::Q8_0,
        KvFormat::Quantized(QuantFormat::Q4_KS) => BlockFormat::Q4_KS,
        KvFormat::Quantized(QuantFormat::Q4_1) => BlockFormat::Q4_1,
        KvFormat::Quantized(QuantFormat::Q4_0) => BlockFormat::Q4_0,
        KvFormat::Quantized(QuantFormat::Q3_1) => BlockFormat::Q3_1,
        KvFormat::Quantized(QuantFormat::Q3_0) => BlockFormat::Q3_0,
        KvFormat::Quantized(QuantFormat::Q2_1) => BlockFormat::Q2_1,
        KvFormat::Quantized(QuantFormat::Q2_A) => BlockFormat::Q2_A,
        KvFormat::Quantized(QuantFormat::Q2_S) => BlockFormat::Q2_S,
        KvFormat::Quantized(QuantFormat::Q2_0) => BlockFormat::Q2_0,
        KvFormat::Quantized(QuantFormat::Q1_S) => BlockFormat::Q1_S,
        KvFormat::Quantized(QuantFormat::Q0_V) => BlockFormat::Q0_V,
        KvFormat::Quantized(QuantFormat::Q1_A) => BlockFormat::Q1_A,
        KvFormat::Quantized(QuantFormat::Q0_X) => BlockFormat::Q0_X,
        KvFormat::Quantized(QuantFormat::Q0_M2) => BlockFormat::Q0_M2,
        KvFormat::Quantized(QuantFormat::Q0_M4) => BlockFormat::Q0_M4,
        KvFormat::Quantized(QuantFormat::Q0) => BlockFormat::Q0,
...
fn collect_formats_and_stats(
    ...
    k_candidates: &[BlockFormat],
    v_candidates: &[BlockFormat],
    q_rel_stats: Option<(f32, f32, bool)>,
    _gpu_selector: Option<&GpuSelector>,
) -> (
    [CurveStat; 2],
    [CurveStat; 2],
    [CurveStat; 2],
    ...
    // Block slicing must be dim-major to match the CUDA selection kernel,
    // which sees each block as 32 tokens of one channel within a (head,
    // palette) sub-band.  The dump is token-major `[h][p][t][sub_d]`, so
    // `chunk.k[b*32..(b+1)*32]` would extract 32 channels of one token
    // (the transpose) and produce nonsense quant errors.
    let elems_per_chunk = blocks_per_chunk * SELECT_BLOCK;
    // v_low_threshold is only consumed by the CUDA selection kernel; silence the
    // unused-arg warning on the CPU-only build.
    #[cfg(not(feature = "cuda"))]
    let _ = v_low_threshold;
    let qrel_mask_start = Instant::now();
    let mut qrel_hi_mask = Vec::with_capacity(chunks.len() * blocks_per_chunk);
    for chunk in chunks {
        for b in 0..blocks_per_chunk {
            if elems_per_chunk <= chunk.k.len() {
                let blk =
                    dim_major_block_from_token_major(&chunk.k, b, n_kv_head, blocks_per_chunk);
                let is_high = if let Some((median, _spread, _)) = q_rel_stats {
                    let rel = if let Some(q) = chunk.q.as_ref() {
                        if elems_per_chunk <= q.len() {
                            let q_blk =
                                dim_major_block_from_token_major(q, b, n_kv_head, blocks_per_chunk);
    ...
    if let Some(gpu) = _gpu_selector {
        ...
        let (k_selected, v_selected, _k_selected_sidx, _v_selected_sidx, _k_amax, _v_amax) = gpu
        ...
        for chunk in chunks {
            for b in 0..blocks_per_chunk {
                ...
                if elems_per_chunk <= chunk.v.len() {
                    let blk =
                        dim_major_block_from_token_major(&chunk.v, b, n_kv_head, blocks_per_chunk);
    ...
    for chunk in chunks {
        ...
        // CPU path is V-monolithic — fall back to the strict V-high bound.
        // The GPU path above passes both v_high/v_low to the kernel.
        let vt_base = v_high_threshold;
        ...
        for b in 0..blocks_per_chunk {
            ...
            if elems_per_chunk <= chunk.k.len() {
                ...
                qrel_bucket = q_rel_stats.map(|_| is_high);
                if q_rel_stats.is_some() {
                ...
                let kt = if let Some((median, spread, is_fallback)) = q_rel_stats {
                    if is_fallback {
                        // K-energy fallback: cap multiplier at 1.0 so the K² proxy
                        // can only TIGHTEN the threshold for clearly high-magnitude
                        // blocks; low-magnitude blocks fall back to the geometric
                        // mean rather than the relaxed end of the range.  Without
                        // a real Q signal we can't be confident that low K-energy
                        // means low attention, so we err toward keeping quality.
                        if spread <= 1.0e-8 {
                            (kt_lo * kt_hi).sqrt()
                        } else {
                            let z = (q_relevance - median) / spread;
                            let mult = (-z).exp().min(1.0);
                            let base = (kt_lo * kt_hi).sqrt();
                            (base * mult).clamp(kt_hi, kt_lo)
                        }
                    } else {
                        k_threshold_scaled_rust(kt_lo, kt_hi, q_relevance, median, spread)
                    }
                } else {
                    kt_base
                };
...
fn test_threshold_sweep() {
    ...
    let (header, loaded_chunks) = load_dump(&path).expect("failed to load dump");
    let total_chunks = loaded_chunks.len();
    let chunks = evenly_sampled_chunks(&loaded_chunks, THRESHOLD_SWEEP_CHUNKS);
    let elems = header.n_kv_head * header.chunk_size * header.head_dim;
    let num_blocks = elems / SELECT_BLOCK;
    let candidates = level_candidates();
    ...
    let sweep_thresholds: Vec<f32> = {
        ...
        let n_steps = THRESHOLD_SWEEP_STEPS;
        for i in 0..=n_steps {
    ...
    println!(
        "Sweeping {} threshold points across {} sampled / {} total chunks ({} blocks/chunk)...",
    ...
    // ── Section 1: K-sweep ──
    let k_results: Vec<(f32, f64, f64, f64, f64)> =
        cpu_parallel_kernel_map(&sweep_thresholds, |t| {
            let (bpe, snr, nrmse, p95) = sweep_single_side(&chunks, num_blocks, t, k_cands, true);
            (t, bpe, snr, nrmse, p95)
        });

    // ── Section 2: V-sweep ──
    let v_results: Vec<(f32, f64, f64, f64, f64)> =
        cpu_parallel_kernel_map(&sweep_thresholds, |t| {
    ...
    let coarse_thresholds: Vec<f32> = {
        ...
        let log_min = -6.0f64;
        let log_max = -0.5f64; // 10^(-0.5) ≈ 0.316
        let n_steps = THRESHOLD_GRID_STEPS;
        for i in 0..=n_steps {
            let log_val = log_min + (log_max - log_min) * i as f64 / n_steps as f64;
            v.push(10.0f64.powf(log_val) as f32);
        }
    ...
    let k_bpe_at: Vec<f64> = cpu_parallel_kernel_map(&coarse_thresholds, |t| {
        sweep_single_side(&chunks, num_blocks, t, k_cands, true).0
    });
    let v_bpe_at: Vec<f64> = cpu_parallel_kernel_map(&coarse_thresholds, |t| {
        sweep_single_side(&chunks, num_blocks, t, v_cands, false).0
    });
    ...
    let representative_levels = [0usize, 3, 6, 9];
    ...
    println!("{sep}");
    let v_display_step = (coarse_thresholds.len() / 8).max(1);
    let v_display_indices: Vec<usize> = (0..coarse_thresholds.len())
        .step_by(v_display_step)
        .collect();
    ...
    println!();
    let k_display_step = (coarse_thresholds.len() / 12).max(1);
    for (ki, &kt) in coarse_thresholds.iter().enumerate().step_by(k_display_step) {
        print!("  {:>12.8}", kt);
        for &vi in &v_display_indices {
            let combined_bpe = (k_bpe_at[ki] + v_bpe_at[vi]) / 2.0;
            let cr = 16.0 / combined_bpe;
...
fn test_candidate_list_compression_curve() {
    ...
    // Load Qwen3 as primary; fall back to Llama if Qwen3 absent.
    // If the preferred path (R16 dump) exists but has 0 chunks, fall through
    // to the regular qwen3 dump so the tables still have data.
    let (header, mut chunks, primary_name) = 'load_primary: {
        if let Some(p) = qwen3_path.as_ref() {
            let (h, c) = load_dump(p).expect("failed to load Qwen3 dump");
            let name = p
                .file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("qwen3-kv-data.bin")
                .to_string();
            if !c.is_empty() {
                break 'load_primary (h, c, name);
            }
            // Preferred dump is empty — try the regular Qwen3 dump as fallback.
            if let Some(fallback) = dump_path_for(QWEN3_DUMP_REL_PATH).filter(|fp| fp != p) {
                if let Some((fh, fc)) = load_dump(&fallback) {
                    if !fc.is_empty() {
                        let fname = fallback
                            .file_name()
                            .and_then(|n| n.to_str())
                            .unwrap_or("qwen3-kv-data.bin")
                            .to_string();
                        break 'load_primary (fh, fc, fname);
                    }
                }
            }
            break 'load_primary (h, c, name);
        }
        let p = llama_path.as_ref().unwrap();
        let (h, c) = load_dump(p).expect("failed to load llama-kv-data.bin");
        let name = p
            .file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("llama-kv-data.bin")
            .to_string();
        (h, c, name)
    };
    let mut data_sources = vec![primary_name];

    // Load Llama as a separate secondary dataset — different n_kv_head, so processed independently.
    let (llama_hdr, mut llama_chunks) = if qwen3_path.is_some() {
    ...
    // Pre-round both datasets through F16 to match GPU arena precision.
    ...
    for chunk in llama_chunks.iter_mut() {
        for x in chunk.k.iter_mut() {
            *x = f16::from_f32(*x).to_f32();
        }
        for x in chunk.v.iter_mut() {
    ...
    let (llama_blocks_per_head, llama_blocks_per_chunk) = llama_hdr
        .as_ref()
        .map(|lh| {
            let bph = (lh.chunk_size * lh.head_dim) / SELECT_BLOCK;
            (bph, lh.n_kv_head * bph)
        })
        .unwrap_or((0, 0));
    let has_llama = !llama_chunks.is_empty();
    ...
    let q_rel_stats = compute_q_relevance_stats(&chunks, blocks_per_chunk);
    let llama_q_rel_stats = if has_llama {
        compute_q_relevance_stats(&llama_chunks, llama_blocks_per_chunk)
    } else {
        None
    };
    ...
    #[cfg(feature = "cuda")]
    let (
        _gpu_selection_generation,
        _gpu_backing,
        gpu_selector,
        _llama_gpu_generation,
        _llama_gpu_backing,
        llama_gpu_selector,
    ): (
        Option<candle::quantized::pinned_staging::Generation>,
        Option<crate::kv_cache::ChunkedKvBacking>,
        Option<GpuSelector>,
        Option<candle::quantized::pinned_staging::Generation>,
        Option<crate::kv_cache::ChunkedKvBacking>,
        Option<GpuSelector>,
        ...
        Ok(candle::Device::Cuda(cuda_dev)) => {
            ...
            // Prefer the R16+Q path when every chunk has Q data — the GPU
            // selection kernel reads block_r16->q[] for per-block relevance
            // and threshold scaling, which is what production sees during
            // prefill/decode.  Falls back to the F32-only path otherwise.
            let q_chunks: Option<Vec<&[f32]>> =
                chunks.iter().map(|chunk| chunk.q.as_deref()).collect();
            let result = if let Some(q_chunks) = q_chunks.as_ref() {
                ...
                PagedSelectionGpuInputs::from_f32_chunks(
                    &k_chunks,
                    &v_chunks,
                    blocks_per_chunk,
                    header.n_kv_head,
            ...
            let (backing, gpu) = match result {
                Some((b, g)) => (Some(b), Some(g)),
                None => (None, None),
            };
            // Build a separate GPU selector for the Llama secondary dataset so the
            // "(ll)" column uses the same CUDA kernel path as the Llama-only run.
            let (ll_gen, ll_back, ll_gpu) = if has_llama && !llama_chunks.is_empty() {
                let ll_stager = candle::quantized::pinned_staging::PinnedStager::new(&cuda_dev);
                let ll_gen = ll_stager.begin_generation();
                let ll_k: Vec<&[f32]> = llama_chunks.iter().map(|c| c.k.as_slice()).collect();
                let ll_v: Vec<&[f32]> = llama_chunks.iter().map(|c| c.v.as_slice()).collect();
                let ll_q: Option<Vec<&[f32]>> =
                    llama_chunks.iter().map(|c| c.q.as_deref()).collect();
                let ll_n_kv_head = llama_hdr.as_ref().map_or(0, |h| h.n_kv_head);
                let ll_result = if let Some(ll_q) = ll_q.as_ref() {
                    PagedSelectionGpuInputs::from_f32_chunks_with_q(
                        &ll_k,
                        &ll_v,
                        ll_q,
                        llama_blocks_per_chunk,
                        ll_n_kv_head,
                        DEFAULT_REPORT_ARENA_CHUNKS,
                        Some(&ll_gen),
                        &cuda_dev,
                ...
                match ll_result {
                    Some((b, g)) => (Some(ll_gen), Some(b), Some(g)),
                    None => (None, None, None),
                }
            } else {
                (None, None, None)
            };
            (Some(generation), backing, gpu, ll_gen, ll_back, ll_gpu)
        }
    ...
    #[cfg(not(feature = "cuda"))]
    let llama_gpu_selector: Option<GpuSelector> = None;
    ...
    struct LevelResult {
        ...
        k_cr_p4: f64,
        v_cr_p4: f64,
        k_cr_p4_ll: Option<f64>,
        v_cr_p4_ll: Option<f64>,
        k_head_dist: [usize; 23],
        v_head_dist: [usize; 23],
        k_pal4_dist: [usize; 23],
        v_pal4_dist: [usize; 23],
        k_pal4_dist_ll: Option<[usize; 23]>,
        v_pal4_dist_ll: Option<[usize; 23]>,
        k_total_pal4_blocks_ll: usize,
        v_total_pal4_blocks_ll: usize,
        k_head_dist_hi: [usize; 23],
        k_head_dist_lo: [usize; 23],
        v_head_dist_hi: [usize; 23],
        v_head_dist_lo: [usize; 23],
        k_total_heads: usize,
        v_total_heads: usize,
    ...
    // Scale the shared production thresholds by the per-model factors so the
    // offline report matches what the live runtime sees.  Returns the 5-tuple
    // expected by `collect_formats_and_stats`:
    //   (k_threshold, v_high_threshold, v_low_threshold, k_high_threshold, k_low_threshold)
    let factored_thresholds = |i: usize, f: KvErrorThresholdFactors| -> (f32, f32, f32, f32, f32) {
        (
            K_QREL_HIGH_THRESHOLDS[i] * f.k_hi,
            PRODUCTION_V_QREL_HIGH_THRESHOLDS[i] * f.v_hi,
            PRODUCTION_V_QREL_LOW_THRESHOLDS[i] * f.v_low,
            K_QREL_HIGH_THRESHOLDS[i] * f.k_hi,
            K_QREL_LOW_THRESHOLDS[i] * f.k_low,
        )
    };
    ...
    let levels: Vec<LevelResult> = cpu_parallel_kernel_range(11, |i| {
        ...
        // Primary dataset (Qwen3) uses Qwen3-MoE per-model factors.
        let (k_thr, v_hi_thr, v_lo_thr, k_hi_thr, k_lo_thr) =
            factored_thresholds(i, QWEN3_MOE_KV_FACTORS);
        ...
        let gpu_quality_agg = if let Some(gpu) = gpu_selector.as_ref() {
            ...
            let v_sample: Vec<SampleFormat> = v_fmts
                ...
                .collect();
            Some(
                gpu.aggregate_quality_metric_formats_gpu(&k_sample, &v_sample, None)
        ...
        let mut k_pal4_dist = [0usize; 23];
        let mut v_pal4_dist = [0usize; 23];
        ...
        // Per-dataset pal4 CRs (K and V separately)
        let k_cr_p4 = 16.0 / k_pal4_bpe;
        let v_cr_p4 = 16.0 / v_pal4_bpe;

        // Llama secondary pass — uses same GPU selector as standalone Llama run for consistency.
        // Llama-specific factors so the report tracks whatever the Llama trait override produces.
        let mut ll_k_pal4_dist: Option<[usize; 23]> = None;
        let mut ll_v_pal4_dist: Option<[usize; 23]> = None;
        let mut ll_k_total_pal4_blocks: usize = 0;
        let mut ll_v_total_pal4_blocks: usize = 0;
        let (k_cr_p4_ll, v_cr_p4_ll) = if has_llama {
            let ll_n_kv_head = llama_hdr.as_ref().map_or(0, |h| h.n_kv_head);
            let (ll_k_thr, ll_v_hi_thr, ll_v_lo_thr, ll_k_hi_thr, ll_k_lo_thr) =
                factored_thresholds(i, LLAMA_KV_FACTORS);
            let (_, _, _, ll_k_fmts, ll_v_fmts, _, _) = collect_formats_and_stats(
                &llama_chunks,
                llama_blocks_per_chunk,
                ll_n_kv_head,
                ll_k_thr,
                ...
                &candidates[i].1,
                llama_q_rel_stats,
                llama_gpu_selector.as_ref(),
            );
            let (k_ll_pal4_bpe, k_ll_p4_fmts) =
                cpu_palette4_reduce(&ll_k_fmts, llama_blocks_per_head);
            let (v_ll_pal4_bpe, v_ll_p4_fmts) =
            ...
            ll_v_total_pal4_blocks = v_ll_p4_fmts.len();
            (Some(16.0 / k_ll_pal4_bpe), Some(16.0 / v_ll_pal4_bpe))
        } else {
            (None, None)
        };

        let quality_start = Instant::now();
        let (k_q_w1, k_q_p4) = compute_quality_w1_p4(
            &chunks,
            blocks_per_chunk,
            header.n_kv_head,
            &k_w1_fmts,
        ...
        let (v_q_w1, v_q_p4) = compute_quality_w1_p4(
            &chunks,
            blocks_per_chunk,
            header.n_kv_head,
            &v_w1_fmts,
        ...
        let cr_w1 = 16.0 / ((k_quant_bpe + v_quant_bpe) / 2.0);
        ...
        LevelResult {
            ...
            _v_bpe: v_raw_bpe,
            cr_w1,
            k_bpe_w1: k_quant_bpe,
            v_bpe_w1: v_quant_bpe,
            k_cr_p4,
            v_cr_p4,
            k_cr_p4_ll,
            v_cr_p4_ll,
            ...
            k_pal4_dist,
            v_pal4_dist,
            k_pal4_dist_ll: ll_k_pal4_dist,
            v_pal4_dist_ll: ll_v_pal4_dist,
            k_total_pal4_blocks_ll: ll_k_total_pal4_blocks,
            v_total_pal4_blocks_ll: ll_v_total_pal4_blocks,
            k_head_dist_hi,
            k_head_dist_lo,
            v_head_dist_hi,
            v_head_dist_lo,
    ...
    let w1_cr = |i: usize| levels[i].cr_w1;
    let k_pal4_cr = |i: usize| levels[i].k_cr_p4;
    let v_pal4_cr = |i: usize| levels[i].v_cr_p4;
    ...
    match q_rel_stats {
        Some((median, spread, false)) if has_q_relevance_capture => println!(
            "  Q-relevance IQR stats: median={:.6}  spread={:.6}  (IQR-standardised exponential K threshold)",
            median, spread,
        ),
        Some((median, spread, _)) => println!(
            "  Q-relevance IQR stats (K-energy fallback, conservative — multiplier capped at 1.0): \
             median={:.6}  spread={:.6}",
            median, spread,
        ),
        None => println!("  Q-relevance stats: unavailable"),
    }
    println!(
    ...
    let fmt_pct = |counts: &[usize; 23], total: usize, fmt: BlockFormat| {
        if total == 0 {
            format!("{:>4}", "-")
        } else {
            format!(
    ...
    for i in 0..11 {
        ...
        let v_cands = candidates[i]
            ...
            .join(", ");
        println!(
            "  C{i}: K=[{k_cands}]  V=[{v_cands}]  blk={:.2}x  w1={:.2}x  K_p4={:.2}x  V_p4={:.2}x",
            cr(i),
            w1_cr(i),
            k_pal4_cr(i),
            v_pal4_cr(i),
    ...
    // Column lists are the union of K (resp. V) candidates across all 11
    // compression modes, ordered by descending table_index so the layout is
    // least-aggressive on the left → most-aggressive on the right.
    let union_grid = |get: &dyn Fn(&(Vec<BlockFormat>, Vec<BlockFormat>)) -> &Vec<BlockFormat>| {
        let mut seen = [false; 23];
        let mut out: Vec<BlockFormat> = Vec::new();
        for level in &candidates {
            for &fmt in get(level) {
                if !seen[fmt.table_index()] {
                    seen[fmt.table_index()] = true;
                    out.push(fmt);
                }
            }
        }
        out.sort_by_key(|f| std::cmp::Reverse(f.table_index()));
        out
    };
    let k_grid_formats = union_grid(&|c| &c.0);
    let v_grid_formats = union_grid(&|c| &c.1);

    // Helper: render one K distribution table for a given dataset label and dist accessor.
    let render_k_table = |label: &str, cr_col: &str, _cr_fn: &dyn Fn(usize) -> f64| {
        println!("\n{sep80}");
        println!("Table 1a ({label}) -- K palette-4 block format distribution (%)");
        println!("  p4_CR column shows the palette-4 compression ratio for this dataset.");
        println!();
        let mut hk = format!("  Mode | H_SNR | L_SNR  | {cr_col:<6} |");
        let mut hk2 = format!("       |       |        |        |");
        for fmt in &k_grid_formats {
            hk.push_str(&format!(" {:>4}", fmt.grid_label()));
            hk2.push_str(&format!(" {:>4}", ""));
        }
        println!("{hk}");
        println!("{hk2}");
    ...
    let render_v_table = |label: &str, cr_col: &str, _cr_fn: &dyn Fn(usize) -> f64| {
        println!("\n{sep80}");
        println!("Table 1b ({label}) -- V palette-4 block format distribution (%)");
        println!("  p4_CR column shows the palette-4 compression ratio for this dataset.");
        println!();
        let mut hv = format!("  Mode | H_SNR | L_SNR  | {cr_col:<6} |");
        let mut hv2 = format!("       |       |        |        |");
        for fmt in &v_grid_formats {
            hv.push_str(&format!(" {:>4}", fmt.grid_label()));
            hv2.push_str(&format!(" {:>4}", ""));
        }
        println!("{hv}");
        println!("{hv2}");
    ...
    // Table 1a Qwen3
    render_k_table("Qwen3", "p4_CR", &k_pal4_cr);
    for i in 0..11 {
        let mut line = format!(
            "  C{i:<2} | {} | {} | {:>5.2}x |",
    ...
    // Table 1a Llama
    if has_llama {
        render_k_table("Llama", "p4_CR", &|i| levels[i].k_cr_p4_ll.unwrap_or(0.0));
        for i in 0..11 {
            let cr = levels[i].k_cr_p4_ll.unwrap_or(0.0);
            let mut line = format!(
                "  C{i:<2} | {} | {} | {:>5.2}x |",
                fmt_db(levels[i].k_snr_hi),
                fmt_db(levels[i].k_snr_lo),
    ...
    // Table 1b Qwen3
    render_v_table("Qwen3", "p4_CR", &v_pal4_cr);
    for i in 0..11 {
        let mut line = format!(
            "  C{i:<2} | {} | {} | {:>5.2}x |",
    ...
    // Table 1b Llama
    if has_llama {
        render_v_table("Llama", "p4_CR", &|i| levels[i].v_cr_p4_ll.unwrap_or(0.0));
        for i in 0..11 {
            let cr = levels[i].v_cr_p4_ll.unwrap_or(0.0);
            let mut line = format!(
                "  C{i:<2} | {} | {} | {:>5.2}x |",
                fmt_db(levels[i].v_snr_hi),
                fmt_db(levels[i].v_snr_lo),
    ...
    println!("  {}", "-".repeat(100));
    for i in 0..11 {
        let gain = levels[i].cr_p4 / levels[i].cr_w1;
        println!(
            "  C{i:<2} {:<9} {:>5.2}x   {:>5.2}x     {:>5.2}x   {:>8.3}  {:>8.3}  {:>8.3}  {:>8.3}  {:>5.1}%",
            LEVEL_TIER[i],
            cr(i),
            w1_cr(i),
            levels[i].cr_p4,
    ...
    for i in 0..11 {
        let lv = &levels[i];
        println!(
            "  C{i:<2} {:<9} {:>6.1}  {:>6.1}  {:>6.1}   {:>6.1}  {:>6.1}  {:>6.1}   {:>.2e}  {:>.2e}  {:>.2e}  {:>.2e}  {:>.2e}  {:>.2e}",
            LEVEL_TIER[i],
...
/// GPU-accelerated threshold calibration.
///
/// For each compression level, finds the magnitude-weighted K threshold that
/// produces the same per-head K BPE as the cosine baseline, and a more
/// aggressive V threshold targeting 20% lower BPE.  Run this separately from
/// `test_candidate_list_compression_curve` — it takes ~60 s on its own.
///
/// Run with:
///   cargo test --release --features cuda --lib \
///     sampled_selection::tests::projection::test_threshold_calibration \
///     -- --ignored --nocapture
#[test]
#[ignore]
fn test_threshold_calibration() {
    ...
    let sep = "=".repeat(115);
    #[cfg(feature = "cuda")]
    let fmt_ms = |d: std::time::Duration| format!("{:>8.1} ms", d.as_secs_f64() * 1_000.0);
    ...
    let (header, mut chunks) = match load_dump(&path) {
        Some(v) => v,
        None => {
            println!("SKIP: failed to parse dump at {:?}", path);
            return;
        }
    };
    for chunk in chunks.iter_mut() {
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        let (_calibration_generation, _calibration_backing, gpu) = if !calibration_chunks.is_empty()
        {
            ...
            match candle::Device::cuda_if_available(0) {
                Ok(candle::Device::Cuda(cuda_dev)) => {
                    let stager = candle::quantized::pinned_staging::PinnedStager::new(&cuda_dev);
                    let generation = stager.begin_generation();
                    let result = PagedSelectionGpuInputs::from_f32_chunks(
                        &k_chunks,
                        &v_chunks,
                        blocks_per_chunk,
                        header.n_kv_head,
        ...
        let run_gpu_bpe = |level: usize, k_thr: f32, v_thr: f32| -> (f64, f64) {
            ...
            let v_sample_candidates: Vec<SampleFormat> = candidates[level]
                ...
                .map(sample_format_from_block)
                .collect();
            let (k_selected, v_selected, _k_sidx_cal, _v_sidx_cal, _k_amax_cal, _v_amax_cal) = gpu
                .select_block_formats(
                    &k_sample_candidates,
                    &v_sample_candidates,
                    k_thr,
        ...
        let (_diag_generation, _diag_backing, diag_gpu) = if !diag_chunks.is_empty() {
            ...
            match candle::Device::cuda_if_available(0) {
                Ok(candle::Device::Cuda(cuda_dev)) => {
                    ...
                    .ok();
                    let (backing, g) = match result {
                        Some((b, g)) => (Some(b), Some(g)),
                        None => (None, None),
                    };
                    (Some(generation), backing, g)
                }
                _ => (None, None, None),
            }
        } else {
        ...
        for &level in &diag_levels {
            ...
            // GPU now uses real quant/dequant + normalised max_abs_error; the CPU
            // reference still uses the old cosine/magnitude metric with thresholds
            // calibrated for that metric.  Until PRODUCTION_* thresholds are
            // re-derived for the new metric, the CPU/GPU comparison is informational
            // only — do not assert.
            println!(
                "    C{level}: K_bpe cpu={cpu_k_bpe:.3} gpu={gpu_k_bpe:.3} err={k_bpe_err:.4} (info)  V_bpe cpu={cpu_v_bpe:.3} gpu={gpu_v_bpe:.3} err={v_bpe_err:.4} (info)",
            );
        }
        let diag_time = diag_start.elapsed();
        println!("  GPU diagnostic: printed (thresholds pending re-calibration)");
        println!();
...
///
/// Block extraction is dim-major to match the GPU selection kernel — see
/// `dim_major_block_from_token_major` for the layout rationale.
#[allow(dead_code)]
...
///
/// Block extraction is dim-major to match the GPU selection kernel's view —
/// `w1_fmts[b]` / `p4_fmts[b]` were chosen for the dim-major block at index
/// `b`, so the SNR test must apply them to the same dim-major block.
fn compute_quality_w1_p4(
    ...
    for chunk in chunks {
        let data = if is_key { &chunk.k } else { &chunk.v };
        for b in 0..blocks_per_chunk {
            if elems_per_chunk <= data.len() && blk_idx < w1_fmts.len() {
                let blk = dim_major_block_from_token_major(data, b, n_kv_head, blocks_per_chunk);
...
fn reduce_per_head_with_dist(
    ...
    let mut total_bpe = 0.0f64;
    let mut total_blocks = 0usize;
    let mut head_counts = [0usize; 23]; // indexed by table_index (0..16)
    for chunk_fmts in block_fmts.chunks(blocks_per_chunk) {
        for head_fmts in chunk_fmts.chunks(blocks_per_head) {
            let worst = head_fmts
                .iter()
                ...
                let fmt = if is_key {
                    if let Some(q) = q_data {
                        if end <= q.len() {
                            ...
                            cpu_select_k_qproj(&blk, &q_blk, candidates, t, t, 0.0, 0.0)
                        } else {
                            select_format_from_candidates_k(&blk, candidates, t)
                        }
                    } else {
...
fn select_formats_k_qproj_with_errors(
    ...
    let _ = (relevance_lo, relevance_hi);
    ...
    let fallback_fmt = candidate_formats()
        .into_iter()
        .filter(|fmt| *fmt != BlockFormat::BF16)
        .next()
        .unwrap_or(sink_fmt);
    ...
    for b in 0..num_blocks {
        ...
        // Compute block-local relevance: ‖Q̂·K_block‖² / ‖K_block‖²
        ...
        // Relevance gates removed — thresholds are conservative enough on their own.
        let _ = relevance;
...
fn test_qproj_k_format_selection() {
    ...
    let (header, loaded_chunks) = load_dump(&path).expect("failed to load R16 dump");
    let total_chunks = loaded_chunks.len();
    let chunks = evenly_sampled_chunks(&loaded_chunks, QPROJ_SAMPLE_CHUNKS);
    println!(
        "Loaded R16 dump: {} layers, {} kv_heads, head_dim={}, chunk_size={}, {} sampled / {} total chunks",
        header.num_layers,
        header.n_kv_head,
        header.head_dim,
        header.chunk_size,
    ...
    let subspace_k: usize = 16; // number of principal components in the fast representative run
    ...
    let subspaces: Vec<Vec<f32>> = cpu_parallel_kernel_range(num_layers * n_kv_head, |lh| {
        compute_q_subspace(&q_vecs_by_lh[lh], head_dim, subspace_k)
    });
    // Free Q vectors
    drop(q_vecs_by_lh);
