...
impl KvCache {
    ...
    /// Reconcile sealed chunks in all caches to their storage policies.
    ///
    /// This re-quantizes sealed chunks after kernel execution.
    /// Uses batch-optimized path when all caches share the same backing (typical case),
    /// taking only a single lock instead of one per sequence.
    ///
    /// Returns the total number of chunks migrated across all caches.
    /// Reconcile sealed KV chunks within a lookback window across a batch of caches.
    ///
    /// Accepts caches from multiple layers in a single call.  Caches are expected
    /// in layer-major order (all sequences for layer 0, then all for layer 1, …).
    /// Consecutive runs sharing the same backing are detected by pointer equality
    /// and each backing's `reconcile_multi` is called once per run.
    ///
    /// Only blocks whose last token falls within `[seq_len - lookback_tokens, seq_len)`
    /// are inspected per backing:
    ///
    /// - `lookback_tokens = 1`         — decode: O(1) per sequence, fires only on
    ///   chunk-boundary steps (31/32 steps are free).
    /// - `lookback_tokens = prefill_n` — prefill: covers all blocks sealed by the
    ///   current prefill batch.
    /// - `lookback_tokens = usize::MAX` — full scan across all sealed blocks.
    ///
    /// Returns the total number of chunks migrated across all caches.
    pub fn reconcile_sealed_batch(
        caches: &[&mut KvCache],
        ...
        // Each layer has its own BlockTableState (via new_shared) but all layers share
        // BackingInner and its bg_quantizer.  Group caches by per-layer state identity
        // so we can enqueue every layer's work items onto the shared bg_quantizer in one
        // pass, then call bg_join() once — letting the bg thread batch all N_layers ×
        // N_sequences blocks into a single GPU quantization pass.
        struct LayerGroup {
            backing: super::chunked::ChunkedKvBacking,
            policy: super::StoragePolicy,
            compression: Option<super::chunked::CompressionPolicy>,
            batch_data: Vec<(usize, usize)>,
            seen_batch: ahash::AHashSet<usize>,
        }

        let mut groups: Vec<(usize, LayerGroup)> = Vec::new();

        for kv in caches.iter() {
            if let CacheStorage::Chunked(c) = &kv.k.storage {
                let state_ptr = std::sync::Arc::as_ptr(&c.backing.state) as usize;
                if let Some((_, g)) = groups.iter_mut().find(|(ptr, _)| *ptr == state_ptr) {
                    if g.seen_batch.insert(c.batch_idx) {
                        g.batch_data.push((c.batch_idx, kv.k.current_seq_len));
                    }
                } else {
                    let mut seen_batch = ahash::AHashSet::new();
                    seen_batch.insert(c.batch_idx);
                    groups.push((
                        state_ptr,
                        LayerGroup {
                            backing: c.backing.clone(),
                            policy: c.storage_policy(),
                            compression: c.compression_policy.clone(),
                            batch_data: vec![(c.batch_idx, kv.k.current_seq_len)],
                            seen_batch,
                        },
                    ));
                }
        ...
        let layer_args: Vec<(
            &super::chunked::ChunkedKvBacking,
            super::StoragePolicy,
            Option<&super::chunked::CompressionPolicy>,
            &[(usize, usize)],
        )> = groups
            .iter()
            .map(|(_, g)| {
                (
                    &g.backing,
                    g.policy,
                    g.compression.as_ref(),
                    g.batch_data.as_slice(),
                )
            })
            .collect();

        backing.reconcile_all_layers(&layer_args, lookback_tokens)
    }
