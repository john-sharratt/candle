...
mod tests {
    ...
    fn declaring_grows_coverage_and_the_report_names_the_region() {
        ...
        assert!(!hits(A + 0x2_1000, 16), "one past the end is not a hit");
    }
    ...
    fn adjacent_and_overlapping_spans_merge_into_one_region() {
        ...
        assert!(
            hits(base + 0x9000, 1),
            "the disjoint one is still protected"
        );
    }
    ...
    fn a_fragmented_set_is_searched_correctly_at_every_gap_and_edge() {
        ...
        for i in 0..n {
            ...
            assert!(
                !hits(b + width as u64, 1),
                "first byte of the gap after {i}"
            );
    ...
    fn releasing_below_a_floor_drops_clips_and_keeps_the_right_regions() {
        ...
        assert!(
            hits(base + 0x8000, 1),
            "the wholly-above region is untouched"
        );
    }
