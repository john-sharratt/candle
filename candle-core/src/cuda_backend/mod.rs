...
impl CudaStorage {
    ...
    pub fn add_at_indices_mut(
        ...
        match &mut self.slice {
            CudaStorageSlice::F32(dst) => {
                let func =
                    device.get_or_load_func("add_at_indices_f32", &kernels::ADD_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
            ...
            CudaStorageSlice::F16(dst) => {
                let func =
                    device.get_or_load_func("add_at_indices_f16", &kernels::ADD_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
            ...
            CudaStorageSlice::BF16(dst) => {
                let func =
                    device.get_or_load_func("add_at_indices_bf16", &kernels::ADD_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
            ...
            CudaStorageSlice::F64(dst) => {
                let func =
                    device.get_or_load_func("add_at_indices_f64", &kernels::ADD_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
    ...
    pub fn sub_at_indices_mut_with_values(
        ...
        let indices_dev = device.memcpy_stod(indices)?;
        let values_dev = device.memcpy_stod(values)?;
        let num_indices = indices.len();
        let cfg = LaunchConfig::for_num_elems(num_indices as u32);
        ...
        match &mut self.slice {
            CudaStorageSlice::F32(dst) => {
                let func = device.get_or_load_func(
                    "sub_at_indices_with_values_f32",
                    &kernels::SUB_AT_INDICES_WITH_VALUES,
                )?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
            ...
            CudaStorageSlice::F16(dst) => {
                let func = device.get_or_load_func(
                    "sub_at_indices_with_values_f16",
                    &kernels::SUB_AT_INDICES_WITH_VALUES,
                )?;
                let mut builder = func.builder();
                builder.arg(dst);
            ...
            CudaStorageSlice::BF16(dst) => {
                let func = device.get_or_load_func(
                    "sub_at_indices_with_values_bf16",
                    &kernels::SUB_AT_INDICES_WITH_VALUES,
                )?;
                let mut builder = func.builder();
                builder.arg(dst);
            ...
            CudaStorageSlice::F64(dst) => {
                // Convert f32 values to f64 for the kernel
                let values_f64: Vec<f64> = values.iter().map(|&v| v as f64).collect();
                let values_dev_f64 = device.memcpy_stod(&values_f64)?;

                let func = device.get_or_load_func(
                    "sub_at_indices_with_values_f64",
                    &kernels::SUB_AT_INDICES_WITH_VALUES,
                )?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                builder.arg(&values_dev_f64);
                barg!(builder, num_indices);
                unsafe { builder.launch(cfg) }.w()?;
            }
            _ => crate::bail!(
    ...
    pub fn mul_at_indices_mut(
        ...
        match &mut self.slice {
            CudaStorageSlice::F32(dst) => {
                let func =
                    device.get_or_load_func("mul_at_indices_f32", &kernels::MUL_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
            ...
            CudaStorageSlice::F16(dst) => {
                let func =
                    device.get_or_load_func("mul_at_indices_f16", &kernels::MUL_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
            ...
            CudaStorageSlice::BF16(dst) => {
                let func =
                    device.get_or_load_func("mul_at_indices_bf16", &kernels::MUL_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
            ...
            CudaStorageSlice::F64(dst) => {
                let func =
                    device.get_or_load_func("mul_at_indices_f64", &kernels::MUL_AT_INDICES)?;
                let mut builder = func.builder();
                builder.arg(dst);
                builder.arg(&indices_dev);
                barg!(builder, num_indices);
    ...
    /// Get the maximum absolute value in a range [start, end) on GPU.
    /// This is optimized for checking only the most recently added KV cache layer
    /// without transferring the entire cache to CPU. Returns a single f32 scalar.
    pub fn max_abs_in_range(
        &self,
        layout: &Layout,
        start: usize,
        end: usize,
        ...
        // Allocate output buffer on GPU (initialized to 0)
        let output = device.alloc_zeros::<f32>(1)?;

        // Calculate launch configuration
        let range_size = (end - start).min(total_elems - start);
        let threads = 256;
        let blocks = ((range_size + threads - 1) / threads).min(1024);
        let cfg = LaunchConfig {
        ...
        match &self.slice {
            CudaStorageSlice::F32(src) => {
                let func = device.get_or_load_func(
                    "max_abs_in_range_f32",
                    &kernels::MAX_ABS_IN_RANGE,
                )?;
                let mut builder = func.builder();
                builder.arg(src);
                builder.arg(&output);
                barg!(builder, start);
                barg!(builder, end);
                barg!(builder, total_elems);
                unsafe { builder.launch(cfg) }.w()?;
            }
            CudaStorageSlice::F16(src) => {
                let func = device.get_or_load_func(
                    "max_abs_in_range_f16",
                    &kernels::MAX_ABS_IN_RANGE,
                )?;
                let mut builder = func.builder();
                builder.arg(src);
            ...
            _ => crate::bail!(
                "max_abs_in_range is only supported for float types (f16, bf16, f32, f64)"
            ),
        }

        // Copy single scalar result back to CPU
        let result = device.memcpy_dtov(&output)?;
        Ok(result[0])
    }
