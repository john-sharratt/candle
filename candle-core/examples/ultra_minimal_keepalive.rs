...
fn main() -> Result<()> {
    ...
    println!(
        "Scalar add:        {:.3} µs",
        start.elapsed().as_micros() as f64 / test_iters as f64
    );
    ...
    let strategies = vec![
        ...
        (
            "Strategy 1: Scalar (1 elem)",
            strategy1_us,
            strategy1_us - baseline_us,
        ),
        (
            "Strategy 2: 64-elem sum",
            strategy2_us,
            strategy2_us - baseline_us,
        ),
        (
            "Strategy 3: 256-elem sum",
            strategy3_us,
            strategy3_us - baseline_us,
        ),
        (
            "Strategy 4: Persistent add",
            strategy4_us,
            strategy4_us - baseline_us,
        ),
        (
            "Strategy 5: 2x2 matmul",
            strategy5_us,
            strategy5_us - baseline_us,
        ),
        (
            "Strategy 6: Sync only",
            strategy6_us,
            strategy6_us - baseline_us,
        ),
    ];
    ...
    for (name, time, overhead) in &strategies {
        ...
        println!(
            "  {:30} {:8.2} µs  ({:+7.2} µs){}",
            name, time, overhead, marker
        );
    }

    println!("\n💡 Best Strategy: {}", best.0);
    println!(
        "   Overhead: {:+.2} µs ({:+.3}% change)",
        best.2,
        (best.2 / baseline_us) * 100.0
    );
