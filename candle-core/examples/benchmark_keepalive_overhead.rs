...
fn main() -> Result<()> {
    ...
    println!(
        "ping_aggressive() alone: {:.2} µs per call\n",
    ...
    println!("=== SUMMARY ===");
    println!(
        "Baseline inference:       {:.2} µs per token",
        baseline_per_iter
    );
    println!(
        "With ping():              {:.2} µs per token (+{:.2} µs, +{:.2}%)",
        ping_per_iter,
        ping_per_iter - baseline_per_iter,
        ((ping_per_iter - baseline_per_iter) / baseline_per_iter) * 100.0
    );
    println!(
        "With ping_aggressive():   {:.2} µs per token (+{:.2} µs, +{:.2}%)",
        aggressive_per_iter,
        aggressive_per_iter - baseline_per_iter,
        ((aggressive_per_iter - baseline_per_iter) / baseline_per_iter) * 100.0
    );
    ...
    if ping_per_iter - baseline_per_iter < 10.0 {
        println!(
            "   Use ping() - overhead is negligible ({:.2} µs)",
            ping_per_iter - baseline_per_iter
        );
    ...
    println!(
        "   Keepalive overhead: {:.2} µs ({:.3}% of fixed performance)",
        ping_per_iter - baseline_per_iter,
