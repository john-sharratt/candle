...
fn test_memory_bandwidth(device: &Device) -> Result<()> {
    ...
    for (size, label) in sizes {
        ...
        println!(
            "{}: {:.2} ms, {:.1} GB/s",
            label, avg_time_ms, bandwidth_gb_s
        );
    }
...
fn test_model_layer_simulation(device: &Device) -> Result<()> {
    ...
    for _ in 0..5 {
        ...
        let _attn_out = qkv.narrow(1, 0, hidden_size)?.matmul(&w_o.t()?)?;
        let gate_up = input.matmul(&w_gate_up.t()?)?;
        let _ffn_out = gate_up
            .narrow(1, 0, intermediate_size)?
            .matmul(&w_down.t()?)?;
    }
    ...
    let total_flops = (
        ...
        2 * batch_size * intermediate_size * hidden_size
        // Down
    ) as f64;
    ...
    println!(
        "  Bandwidth-bound achievable: {:.2} TFLOPS",
        bandwidth_bound_flops
    );
    ...
    if is_memory_bound {
        ...
        println!(
            "      With AI={:.2}, you can achieve at most {:.2} TFLOPS.",
    ...
    println!(
        "  Q5_K weight bandwidth: {:.1} GB/s (vs {:.1} GB/s F32)",
