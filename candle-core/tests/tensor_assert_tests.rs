...
fn half_precision_inputs_are_examined_in_their_own_width() -> Result<()> {
    let d = dev()?;
    for (dt, name) in [(DType::F16, "test::f16"), (DType::BF16, "test::bf16")] {
...
fn an_inf_poisons_exactly_its_own_quant_block_and_no_other() -> Result<()> {
    ...
    assert_eq!(
        f.inf, 0,
        "the Inf becomes NaN through the scale, not an Inf"
    );
    assert!(f.is_bad());
    Ok(())
}
...
fn the_order_ticket_names_which_site_went_bad_first() -> Result<()> {
    ...
    assert!(
        a < b,
        "first bad site must carry the lower ticket ({a} vs {b})"
    );
