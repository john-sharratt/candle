...
engine is mostly a **new schema, a new API surface, and two genuinely new subsystems** — the
tick loop, and the world binding that gives a body somewhere to stand — over machinery that
exists and is in production in zend.
...
```
   ...
   └──────────┘  └────┬─────┘  └──────┬───────┘  └────────┬─────────┘
    second-class      │ first-class   │                    │
              ...
              ┌────────────────────────────────────────────────┐
              │  npc-map   (peer, no conversation deps)       │
              │  places · bodies · what each can make out     │
              └────────────────────────────────────────────────┘
```
...
|---|---|---|---|---|
| `perception` | `Sequence{recent, historical_top_k}` | self-local | fast | **change only**; the present is a section, Part V |
...
| `world` | `TopK` | **cross-timeline** | slow | shared authored facts; **never the event record** |
...
retrievable across NPCs. That exception is also the one line of this schema that could undo
a world's social design in a single change — see Part V-bis.
...
Two things are neither memory nor stream, and both are **environment-placed** in the sense
of Part V — present exactly once, never scored, rewritten only when they change:

- **The percept.** What is true where the body is standing, this instant.
- **The current mission and task.** The NPC's own state, effectively its TODO list.

Neither is a system-prompt section, because the system prompt is built once per conversation
and is the KV prefix. Both are superseding turns in their own bands (Part V), which puts
...
new in them. Those turns are not heartbeats — they carry the thing that keeps a long run on
course, and what they carry is **written by whoever authored the task**, not composed in
code.

A continuance generated by the engine is one generic string for every task in the world.
Every NPC gets identical prodding, acts on it identically, and it reads as emergence — the
*affordances, never recommendations* failure, except firing on every idle turn for every NPC
and appearing in no log as anything but a heartbeat. A chronicle task and a portrait task
should read forward differently, and the only honest source of that difference is the task
document.

So the continuance is a field of the task, and the idle turn is **assembled from the TODO
structure the way a place description is assembled from a map** — authored English in the
data, a pure function turning it into prose, the same bytes every time. A task with no
continuance is refused at load, because the fallback for a missing one is precisely the
generic string being avoided, and it would install itself silently.

One property this buys, beyond keeping the task in view: the idle turn is **externally
authored**, so an NPC drifting inward is pulled back by something it did not write. A
continuance derived from the NPC's own state would be self-echo wearing an anchor's clothes
— the Part X-bis runaway loop with extra steps.
...
### Two NPCs in one room are not an interaction

An interaction is a **private, scoped channel** — an operator addressing the mind from
outside the fiction, a player addressing the character inside it. Co-located NPCs need none
of it: one speaks, the act lands in the place, and everyone standing there perceives it.
The room is the channel.

This matters at scale rather than in principle. Sixteen NPCs sharing a building would be 240
pairwise forks under an interaction-per-pair reading of `interlocutor: {kind: "npc"}`, all
to model something the act stream already carries. It is the mind document's own result one
level down: a conversation is a *reading* of the stream, not a thread in anybody's head.

`kind: "npc"` is therefore reserved for a genuinely private NPC-to-NPC channel — a message,
a call — and is never how two characters standing together talk.

### Speech is delivered by place and directed by address

These are different things and collapsing them loses a real social signal. An utterance
lands in the place; everybody there receives it. Who it was aimed at is a property of the
utterance, not a filter on who receives it. So one event has three true readings:
...
Being told something and watching somebody else be told it are different facts, and an NPC
that could not distinguish them is one for whom a room of people is a room of noise. The
world records the whole utterance; each reader narrows it on the way out — which is the
parallax property of the mind document's Part VII, obtained for free.
...
### Three ways of knowing, and they are different mechanisms

An earlier version of this document had one perception concept: events pushed to
`/perceive`, landing as turns in the `perception` layer. That conflates three things which
behave differently, live in different places, and cost differently.

| | Question | Shape | Home |
|---|---|---|---|
| **Memory** | what is this place? | static, shared, never changes | the immutable core — the CoW prefix, Part II |
| **Percept** | what is true here, now? | a pure function of world state, no cursor | a **superseding** turn in the window, below |
| **Change** | what happened while I was not looking? | ordered, per-reader cursor, place-scoped | the `perception` layer, as turns |

Only the third is events. Getting this wrong is expensive in both directions: a percept
written as turns accumulates twelve stale readings of one room, and a change stream written
as a section loses the ordering that makes it a stream.

**The percept is not a memory to retrieve, so it is not scored.** An NPC that fails to
notice the room it is standing in because something scored higher is broken, not selective.
It is structurally resident the way the immutable core is — and this is a third residency
class the architecture did not previously name:

| Class | Scored | Refreshed | Placed by |
|---|---|---|---|
| Immutable core (the lens) | never | never | the CoW prefix |
| **Environment-placed** | **never** | **every tick** | `ConversationEnvironment` |
| Gathered substrate | under budget | accumulates | provenance |

`SelectionRule::Named` is the closest existing primitive — it is explicitly
score-independent — but it names a *collection member*. Environment placement puts a
**block** at a fixed position with no scoring at all. That mechanism is what "deterministic
provenance" means, and it is what the environment owns.

### Supersession is how a point in time lives in an append-only window

A system prompt is built once per conversation and is the KV prefix; rebuilding it each turn
to update a percept would invalidate the whole suffix and cost far more than it saved. So
the percept is not a system-prompt section. It is a **turn that replaces the previous one of
its kind** — `EventKind::Situation`, sharing one band, so exactly one is ever resident and it
is always the current one.

That is environment placement in practice: written by the environment rather than selected
by provenance, present exactly once, never competing for budget, and always in the most
recent position where attention weights it highest — which is correct for the present and
wrong for anything else.

Supersession was previously described here as a wart, "the one place the engine departs from
pure append-only", introduced to stop stale maps accumulating. That had it backwards. The
mechanism is right and the map was the wrong thing to put in it: a picture that cannot
survive tokenization. **A situation is the honest use of the band the map was occupying.**

The NPC's current mission and task sit in the same middle row by the same mechanism, in
their own band.

### Push and pull, and why both

The world simulation is authoritative and the engine is its consumer, but the direction of
travel differs by what is being carried.

**Change is pushed.** After each world tick the environment computes every NPC's delta and
pushes it, and that push is **one batched prefill across every NPC at once** — the mind
document's central asymmetry, actually delivered. Pulling at gather time would serialise it:
each NPC would compute its own delta inside its own gather, one at a time, on the decode
path.

**State is pulled.** The percept is a pure function of world state, so the environment reads
it rather than being told it. This is what keeps two sources of truth from existing for
where a body is standing.

One property falls out and is worth the design depending on: **an unchanged situation
produces byte-identical tokens.** A percept is deterministic, so an NPC in a quiet room
where nothing happened re-renders exactly what is already resident and there is nothing to
prefill. An NPC nothing happened to costs nothing, without needing to block.

Nothing is dropped at emission. The mind document is explicit that a filter removing
contradicting evidence before it lands makes a delusion permanent; salience biases the
gather and the fade, never the write.

### Injection is between turns *and* between tool calls

Events do not wait for turn boundaries. They are injected at every boundary the NPC's own
execution offers — including between tool invocations within one turn — in the same position
and the same shape as a tool result. This is the `<system-reminder>` discipline Claude Code
uses, and it has two consequences worth stating:

- **The world can move underneath a multi-step plan**, and the NPC finds out mid-plan rather
  than at the end. This is the mind document's *perception never blocks on action* at finer
  grain than that document imagined.
- **A mutation from one tool call is visible to the next injection in the same turn**, because
  the world is one shared state behind one lock.

The percept must therefore be computable at any instant, which it is — it is a pure function
with no cursor. The change cursor advances at injection points rather than tick boundaries.

### The batch endpoint remains, for worlds the engine does not host

```
POST /v1/npc/{npc_id}/perceive     { "events": [ … ] }
```

For an NPC bound to a hosted world (Part V-bis) this is **refused**, not merged: two sources
of truth for where a body is standing is the same bug as two world simulators. For an NPC
whose world lives entirely in an external game, it is the only path, and it stays a batch
endpoint with no single-event variant — a per-event endpoint would silently destroy the
prefill batching that makes perception cheap.

**ASCII maps are gone.** They were a `kind: "map"` event carrying a zoom band, a fenced
block and a legend, superseding the previous map for that band. Two things were wrong with
them. Tokenization destroys column alignment — the content of a 2D map is the alignment, and
it is not represented in the token stream — and East Asian Ambiguous width makes box-drawing
characters render at two cells in some terminals, so the picture is not even stable for a
human. A place is a graph and a graph is described in prose (Part V-bis).

Their *band* is not gone. `kind: "situation"` occupies it, for the reason above: what a map
was reaching for — one current reading of where you are, replacing the last — is exactly
right, and only the pictures were wrong.

---

## Part V-bis — The world binding

`npc-map` is a standalone crate holding places, the bodies in them, and what each body can
make out from where it stands. It has no dependency on the conversation stack and no
knowledge of models, so it tests in under a second with no GPU — which is what makes it a
citizen of the harness's pure tier (Part XIV).

| Module | Answers |
|---|---|
| `schema` · `load` · `validate` | what a map file says, and what must be true before anything walks it |
| `describe` | the building an NPC remembers — the immutable core's world content |
| `perceive` | what is true here, now — the environment-placed percept |
| `witness` | what one body could make out — the `perception` layer's turns |
| `stream` | the whole record, for readers that are not bodies |
| `world` | who is where, what is claimed, what just happened |
| `route` | how to get there, and how far it is |

### The world layer holds facts, never the record

The `world` layer in Part III is cross-timeline and unmasked — the one place enumeration
across NPCs is correct. That makes it look like the natural home for `stream`, and wiring
it there would be a single line that undoes the entire social design: every NPC could read
every other NPC's work off a shared layer without moving.

> **The `world` layer holds authored shared facts. It never holds the event record.**
> `stream` feeds the dispatch board, persistence and telemetry. `witness` feeds NPCs.

### Scope is decided in one place

What one body can make out is a single function — its own room, plus what it can see into —
and it is a type rather than a filter buried in a loop so that it can be asserted about
directly. The property is stated over every pair of places in the building: an event reaches
a body only if it happened where that body is standing, or somewhere the map says that place
can see. A further sense would widen that one function and nothing else.

This is the mind document's *scoping by observability, not relevance* with an implementation
rather than a prose rule. It composes with the interaction mode envelope (Part IV) rather
than duplicating it: reach decides whether an act arrives at all, mode decides which channel
of it an operator gets.

### Movement is measured in stops

There is no physics and no metres. A journey is measured in the places somebody would
actually pause, and a stop is a tick, which is a turn.

Concretely, in the six-level building the vault ships: anywhere on your own level is **one**
stop, the lift ride however many levels is **one**, and the far corner of the top level to
the command room is **three** — against nine doorways. That is a compression of the
building, not a flattening of it, and the three stops are the three a person would name:
*you go to the lift, you ride it, you walk to the room.* Cutting a journey there rather than
at every doorway is what makes each pause worth a turn. Nobody reconsiders halfway down a
corridor; everybody reconsiders at a lift.

**One primitive moves a body.** `place` emits the events and settles the journey; the leg
advance is a caller of it, and so is a game that owns its own movement. Battle Cities walks
a tile grid at its own pace and reports where the body ended up; a hosted world advances a
leg per tick. **How long a journey takes is the binding's business, not the map's.**

**The outcome is an event, not a return value**, because by the time a journey has succeeded
or failed the body has had turns in between. Every journey that begins is answered exactly
once — arrived, or lost with a reason — since one dropped in silence leaves a body waiting
for ever. And a journey answered by its own destination *succeeded*, however it got there.

Instant travel, where a world offers it, goes to exactly one authored destination. A
shortcut that went anywhere would dissolve the building; one that goes to the room everybody
must reach most often leaves every other distance intact, because every trip out is still
walked.

### Claims are compare-and-swap, not optimistic versioning

Part VII's arbiter has world-mutating acts carry the world-version they reasoned over, to be
reconciled or discarded at commit if stale. For **claim-shaped** mutations — taking a
station, holding a character, reserving a task — that is the wrong instrument. The world
performs the check and the claim in one step and answers with a typed refusal that names
the holder.

That is strictly better than a stale-version discard, because it is actionable: *this is
held by Maker-07* is something an NPC can act on, and *your premise is stale* is not.
...
except claim-shaped acts, which are compare-and-swap with a typed refusal (Part V-bis).

### Availability is a product of location

The table above says "always", and for a bare NPC in an unmodelled world that is right. In a
world with places, it is not: **what an NPC can do is computed from where its body is.**
Tools hang on the things standing in a room — a terminal, a board, a table — so walking away
takes them with you, a corridor offers none, and a tool the NPC is not standing next to is
never offered and cannot be reasoned about wrongly. A few affordances belong to the area
rather than a thing in it; instant travel is one.

Two consequences:

- `GET /v1/tools` remains the registry. **What an NPC may call this turn is a different
  question** and is answered per-NPC, from its location.
- Calibration is unaffected. A tool is calibrated once, at registration; location filters
  *availability*, not the examples that taught selection.

The same rule governs who may be addressed. The percept already names who is in the room, so
the addressee vocabulary is in context by construction, and addressing somebody who is not
there is refused rather than silently delivered. Talking *about* an absent person is
undirected speech with their name in the words.

Each part carries its own provenance text — what it is and what its tools do — which is
injected alongside the tools it offers. That text is authored beside the part, once, so
every room holding one describes it identically.
...
### It is off, not defaulted off, for a bound world

There is a third case, and it is a refusal rather than a default. An NPC bound to a hosted
world (Part V-bis) has a **deterministic** simulator already: the world knows where every
body is, what is claimed, and what just happened, and it computes those answers rather than
inventing them. A language model narrating what happened next on top of that is a second
arbiter over one state, and the two will disagree — silently, in prose, at the exact points
that matter.

So for a bound NPC the simulator is unavailable and enabling it is refused. **A deterministic
world wins outright**, which is the answer to the last of Part XVI's open questions for every
...
`npc-map` is a new peer crate rather than anything relocated. It depends on nothing in the
conversation stack — no model, no substrate, no projection — which is what lets it be
exercised exhaustively without a GPU. `npcd` depends on it; `candle-conversation` does not.
The direction of that arrow is the point: a world does not know it is being perceived by a
language model.
...
```
  ...
  Perception
    POST   /v1/npc/{id}/perceive            batched events — unbound NPCs only
    GET    /v1/npc/{id}/percept             what it would perceive now (introspection)
    GET    /v1/npc/{id}/tools               what is within reach where it stands

  Worlds  (Part V-bis)
    POST   /v1/world                        load a map set, bind it
    GET    /v1/world/{id}/map               areas, places, what joins what
    GET    /v1/world/{id}/where             every body in it
    POST   /v1/world/{id}/tick              advance one moment
    GET    /v1/world/{id}/stream            the unfiltered record — never an NPC's input
...
drives itself. The loop — drain, gather, decode one step, fan out — is new code, and with it
the salience-gated tick, preemption on high-salience events, the heartbeat whose interval is
the NPC's idle metabolism, and the scheduler that batches across all NPCs with pending
events. The existing scheduler (`candle-conversation::scheduler`, 17.6k LOC, with admission
control) is the right substrate for it, but the loop above it is new.

Two amendments to the mind document's version of it, both from Part V:

- **The world clock is a separate clock.** Many NPCs share one world, so no NPC can drive
...
- **"Blocked costs nothing" becomes "a body nothing happened to costs nothing."** A high-pace
  ...
  there is nothing to prefill and the gate is on *what kind of turn this is*, not on whether
  to take one at all. That distinction is per-NPC — a Maker has a high pace floor, a guard at
  his post does not — and it is what pays for the floor.
...
6. **Perception + authoring.** The world binding and the three-way split (Part V), the
   environment-placed section, batched perceive for unbound NPCs, the state endpoints, the
   two-plane write discipline.
...
Movement answers it for one case and the answer may generalise: a tick is one *intent*, and
the intent is carried out at the world's pace over as many ticks as it costs. The NPC decides
where to go; it does not decide each doorway.

**RESOLVED — a deterministic world beats a narrated one.** Part XVI previously left open what
happens when an external world simulation and the environment simulator are both attached.
Where the world is hosted and deterministic (Part V-bis) the simulator is refused, not
defaulted off: two arbiters over one state disagree silently, in prose, at the points that
matter most. The question survives only between two *narrating* sources, which is a
configuration nobody has asked for.
