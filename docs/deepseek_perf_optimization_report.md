...
Blackwell 72 GB. Bench: `deepseek4::wave::tests::test_parallel_batched_forwarding` (StoryRewrite,
...
- **Correction:** my first instrumentation used device-synced spans, which *serialized* the pipeline and
  ...
  **attention path**, not MoE. (Instrumentation now supports both: sync helpers in `profile.rs`,
  ...
  `DeepSeekBatched::snapshot_profiles`.)
...
the seconds-scale microbench (`deepseek4::bench::run_decode`/`run_prefill` — correctness gate + isolated
...
### Method: a per-kernel selection microbench first (`deepseek4::select_bench`)
...
- **candle-transformers/src/models/deepseek4:** `hyper.rs` (mHC fusion + parity test), `attention.rs`
