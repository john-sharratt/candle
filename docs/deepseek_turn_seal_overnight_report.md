...
> `DeepSeekBatched::resume_sequence` as the composed resume path. Codec bumped `DSC1→DSC2`.
...
`candle-transformers/src/models/deepseek4/compressor.rs`, `kernel_attention.rs`
...
`candle-transformers/src/models/deepseek4/gallery.rs`, `wave.rs`
...
- `FloatGallery::snapshot()` / `from_snapshot(device, &snap)` — native durable form, no
  re-quant.
- `DeepSeekBatched::{seal_sequence, corpus_snapshot, corpus_restore}` — per-layer compose.
...
`candle-transformers/src/models/deepseek4/wave.rs`
...
- `DeepSeekBatched::{window_ring_snapshot, window_ring_restore}` + `WindowRingSnapshot`/
...
`candle-transformers/src/models/deepseek4/indexer.rs`
...
| Suite | Result |
|-------|--------|
| `deepseek4::*` non-ignored (incl. all new codec/close/query_band tests) | **37 passed, 0 failed** |
...
1. **The scheduler seals model-agnostically.** `gather_wide_sigs` reads only
   ...
   `DeepSeekBatched`, invisible to the scheduler.
...
- `wave.rs` — `DeepSeekBatched::{seal_sequence, corpus_snapshot, corpus_restore,
...
1. Add `ManagedBatchedModel::seal_artifacts` (default empty) + `DeepSeekBatched` override
