...
| [`expert_cache_design.md`](expert_cache_design.md) | **Built, and measured 12–87 % faster than the two-tier cache it replaces** (every config of `test_parallel_batched_forwarding`). VRAM ↔ RAM ↔ NVMe over an authoritative repacked pack file: the old tiers were mutually *exclusive*, which forced eviction to be a 68 GB/config D2H copy of read-only weights and made VRAM's floor a function of how much host RAM was allocated. With a cold tier that always holds a copy, eviction is a drop, the elastic partition's boundary is uncapped, and a restart skips the 42 s repack. §12 records what the build settled: the boundary quiesce the uncapped concession exposed, the 30 % regression the first warm-tier sizing caused and the three fixes for it, and the reload-cost term eviction gained. |
...
| [`tool-system.md`](tool-system.md) | Specifies all 95 server-registered tools, the client-specific tool semantics (Continue client-executed vs. web-chat server-executed), and the Hermes-style `<tools>`/`<tool_call>` prompt format. |
