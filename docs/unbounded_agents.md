...
**Content-addressed cache.** Each slot carries a `SlotProjectionCache` keyed by `hash(preceding_tokens ++ run_tokens)` where the preceding hash is folded incrementally as the assembler walks segments. Cache hits inject the captured per-layer sealed sequences via Arc-clone — no GPU work. Misses enqueue a prefill run into the existing batched prefill queue and capture the resulting K/V under the run's key after the forward pass completes. The cache lives on the slot, persists across reprojections, and drops with the slot.

**Steady-state cost is zero.** Two consecutive projections with the same selected sections produce the same rolling hash at every run boundary, so every run hits the cache and no GPU work runs. Projections that diverge produce hits up to the first divergent token and cache misses for every run downstream of it; the token cost of a miss is the structural template length of that run, typically single-digit tokens per boundary and dozens of tokens per full projection at most. Across slots, prefill runs batch into the standard `forward_batched` calls already used for user prefills and decode steps — structural prefills become first-class participants in the same pipeline with no new kernel.
...
**Structural segments at the barrier.** The probe-barrier cycle includes structural-segment resolution between gather and decode (§5): adjacent `Generated` segments between sealed content form prefill runs that either hit the per-slot content-addressed cache (Arc-clone, no GPU work) or batch into the existing prefill pipeline. The probe's Q vectors at structural positions reflect what the model is reaching for given the *current* scaffolding, not given the *original* scaffolding of some prior turn. Cached structural runs add no visible latency; cache misses overlap with the existing 3–10ms CPU provenance scan.
