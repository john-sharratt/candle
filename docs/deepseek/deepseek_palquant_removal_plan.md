...
- `latent_moe/paged.rs` (`arena_write_read_round_trip`): oracle updated — dims `< 448` round
...
- `latent_moe::paged` gate — **18 passed / 4 failed**. The 4 (`mirror_bit_exact_all_band_formats`,
...
- `latent_moe::paged` GPU gate — **21 passed / 0 failed** (was 18/4; +the new
...
- `latent_moe::wave::wave_paris` (3 sub-tests) — **3 passed / 0 failed**, all crisp
...
(`latent_moe::paged::tests`, `wave_paris`, prefill/decode parity) to be re-run on
the dev box.
