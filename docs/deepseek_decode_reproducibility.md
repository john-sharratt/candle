...
name-substituted story; `wave_paris` checks the answer is "Paris". Both pass
...
| Test | Where | Asks |
|---|---|---|
| `prefill_kernel_is_bitwise_repeatable` | `deepseek4/paged.rs` | the prefill kernel returns the same bits twice — **0.16 s, no model** |
...
| `decode_step_bitwise_probe` | `deepseek4/wave.rs` | both of the above, on the real model |
| `wave_decode_is_reproducible` | `deepseek4/wave.rs` | end-to-end token streams |
