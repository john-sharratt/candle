...
DeepSeek currently has **no substrate seal wiring at all** — `DeepSeekBatched` is only a
...
**Integration.** Give `DeepSeekBatched` a seal-time `indexer_wide_sigs(seq, range) ->
...
- `wave.rs` (`DeepSeekBatched`): seal hook exposing (window bytes, corpus entries,
...
**Artifact B — close partials** (`deepseek4/compressor.rs`, `kernel_attention.rs`)
...
**Artifact C — corpus persist/restore** (`deepseek4/gallery.rs`, `wave.rs`)
...
- `DeepSeekBatched::{seal_sequence, corpus_snapshot, corpus_restore}` — per-layer seal
...
`chunk_ops.rs`; `deepseek4/wave.rs`)
...
- `WindowRingSnapshot`/`WindowRingLayer { resident_len, kv }` — **no stored position**.
  `DeepSeekBatched::window_ring_snapshot` captures the dense resident tail;
...
**The resume path** (`deepseek4/wave.rs`)
- `DeepSeekBatched::resume_sequence(session, seq, corpus_snaps, window, base, total_tokens)`
...
**Artifact D — Indexer wide-Q source** (`deepseek4/indexer.rs`)
...
1. **`ManagedBatchedModel` seal hook** (`candle-transformers/.../batched_inference.rs`):
   ...
   overridden by `DeepSeekBatched` to call `seal_sequence` → `corpus_snapshot` →
...
2. **Per-token wide-Q capture** (`DeepSeekBatched`): a per-`(seq)` ring appended in
