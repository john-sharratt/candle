//! Continuous-batching demo for quantized Qwen3-0.6B via `BatchedInference`.
//!
...
//!
//! Not a CLI — a fixed harness. Downloads `unsloth/Qwen3-0.6B-GGUF` and runs.
//! For throughput across many sessions with output-validity checking, see
//! `quantized_qwen3`'s `test_parallel_batched_forwarding` instead; this is a
...
use candle_transformers::models::rope_schedule::{DeclaredScaling, RopePreset};
...
fn main() -> Result<()> {
    ...
    println!("Downloading Qwen3-0.6B...");
    let model_path = repo
    ...
    // Qwen3's production schedule: progressive YaRN over the file's plain
    // θ = 1e6 frequencies, as the engine runs it. This file declares no scaling.
    let schedule =
        RopePreset::qwen3().gqa_schedule(inv_freq, Some(1e6), DeclaredScaling::None, 40_960)?;
    ...
    let prompts = [
        prompt("Provide a detailed explanation of photosynthesis. Include the light-dependent and light-independent reactions, and the role of ATP and NADPH."),
