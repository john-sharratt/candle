...
fn main() -> Result<()> {
    ...
    let mut caches = model.create_kv_caches(4096);
    ...
    println!("   Final cache length: {}", caches.current_seq_len());
    ...
    for i in 0..max_gen {
        ...
        let logits = model.forward(&mut caches, &input, offset + i)?;
    ...
    let mut caches_full = model.create_kv_caches(4096);

    let full_start = std::time::Instant::now();
    let input_full = Tensor::new(tokens.as_slice(), &device)?.unsqueeze(0)?;
    let logits_full = model.forward(&mut caches_full, &input_full, 0)?;
